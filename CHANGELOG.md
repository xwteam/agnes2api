...
87 份源码 / 面板文件逐份抠掉注释再对拍，只有两份仍有差异，而那两处都是字符串里的
说明文字（上游事实表里一条「假设」的描述、面板产物内嵌的注释本身），
没有一处逻辑改动。从首个版本升级不需要迁移。
...
- **面板资源里的内部研发编号清零** —— 这是这次清理里唯一真正外泄的一块。
  ...
  与 `/admin/css/*.css` 的响应体发出去 —— 26 份文件里的 202 处编号，
  **每一个打开面板的访客都会收到**。清完之后又补了 4 处：上一轮的正则吃不到
  「数字后面还跟一个小写字母」那种形态，而那四处同样已经烧进了产物。
- **源码、测试与门禁脚本里的内部研发编号清零**：网关源码 520 处（62 份）、
  测试 2070 处（125 份）、门禁脚本 79 处（10 份）、推送前那份逐格表 156 处、
  仓根四份配置 7 处。**一处都不是删了事** —— 这些编号绝大多数在做指代，
  删掉编号那句话就指不到东西了，所以逐处换成说得清那件事本身的说法，
  行数一行没多一行没少，东亚宽度的分隔线逐条按原宽补齐。
- **出货文档里的内部研发编号清零**：40 份出货文档全集逐份重扫，共 116 处。
- **提交信息里的内部研发编号清零**：435 条提交全扫、222 条改写，只改信息不动树。
