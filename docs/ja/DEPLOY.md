...
| `TRUST_PROXY` | いいえ | 未設定（`X-Forwarded-For` を信頼**しない**） | ゲートウェイが実際に自分で管理するリバースプロキシの背後にある場合**のみ** `1` にしてください。設定すると `X-Forwarded-For` の先頭要素がクライアント IP としてログイン失敗イベントに記録されます。 |
...
`admin.login_failed` イベントに記録されるため、`X-Forwarded-For` を無条件に信頼すると、誰でも
総当たりの痕跡を任意の IP になすりつけられます。無効時は代わりに `CF-Connecting-IP` を使います
（Cloudflare ではプラットフォームが注入するため、クライアントは偽装できません）。どちらも取得
できない場合、このフィールドは正直に `null` として記録されます——**`"unknown"` という偽の値は
決して作りません**。実在の送信元と誤解されるためです。
