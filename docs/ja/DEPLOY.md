...
- **`list` と `delete` はさらに別々の枠で、それぞれ 1 日 1,000 回**です。読み取り・書き込みの
  ...
  インデックスキーはまさにそのために存在します。消費するのは 2 か所だけです: 1 日 48〜96 回の
  インデックス照合と、**プールが空のときのフォールバック走査**（インデックスは正常なのに
  生きたレコードが 1 件も読めない場合、手動でインポートされたレコードがインデックスに
  入っていないかを確認するため `list` を 1 回だけ実行します）。この走査は組み込みの
  **10 分**のバックオフに従うため（環境変数ではなく固定の定数）、プールが空の間でも
  isolate あたり 1 日最大 144 回です。
