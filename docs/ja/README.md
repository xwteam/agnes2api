...
> [!TIP]
> 上流は Agnes API key のプールが供給します。対話は `agnes-2.0-flash`、画像は `agnes-image-2.1-flash` と `agnes-image-2.0-flash`、動画は `agnes-video-v2.0`（タスク作成 + ポーリングの 2 段階）を使います。key プールは自己修復します —— 上流の `429`/`402` はそのキーをクールダウンさせ、`401`/`403` は永久に排除し、一時的な失敗が連続して閾値に達した場合も同じく排除します。期限切れとともに自動で戻る種類のものに人手の介入は要りません。
...
| 日付 | 更新内容 |
|------|----------|
| 2026-08-28 | v0.1.0 - 🎉 **最初のリリース**：4 プロトコルのゲートウェイ、レジストラー、管理パネルが一度に揃い、同じコードが Cloudflare Worker と Node / Docker の両ランタイムで動きます。4 本の受信プロトコルは同じ上流スケジューラ、同じ key プール、同じ失敗の切り分けを共有します。レジストラーの 2 本の一時メールボックス経路は厳密に対等です。パネルは 8 つのセクションでビルド手順は不要です。ドキュメントは 5 言語に各 1 部あります |
...
- 上流の `429`/`402` はそのキーを段階的なクールダウンに入れ、`401`/`403` は永久に排除し、一時的な失敗が連続して `MAX_STRIKES` に達した場合も排除します
...
- 上流のレスポンスは既定でストリームのまま転送します。非ストリーミングのリクエストでも内部ではイベントストリームを解いて、揃ってから一括で返します
...
2. **ストリーミング**：4 つのプロトコルすべてがストリーミングに対応します。`stream:false` のときもサービスは内部でイベントストリームを解き、揃ってから完全な JSON を一度に返します。上流がエラーになったり途中でストリームが切れたりした場合は、そのプロトコル自身のエラーイベントで終わります。正常な完了に見せかけることはありません。

3. **key プールの自己修復**：上流の `429`/`402` はキーをクールダウンさせ、`401`/`403` は永久に排除し、一時的な失敗が連続して上限に達した場合も排除します。使えるキーが 1 本も残っていないときは区別できる理由を添えて `503` を返します。同期の経路では、予算を使い切っても 1 本も応答しなかった場合に `504` を返します。
