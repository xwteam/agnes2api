...
> [!NOTE]
> `400` のよくある原因（これで全部ではありません）：Anthropic プロトコルに `text` 以外のコンテンツブロックがある、Gemini のパスにメソッド名が無い、動画タスク識別子の形が不正、管理エンドポイントのフィールドが未知または必須項目が無い、フィールド値が範囲外（メモが長すぎる / 一度に 200 本を超えるインポート / 不正な `op`）、クエリパラメータの形式が不正（使用量エンドポイントの `from` / `to`）。`409` は六つ：無効化もされておらず排除もされていない key を削除しようとした、プールサイズが画面で見た数と違う、レジストラーが無効、チャネルに認証情報が無い、補充が既に進行中（`tend_in_flight`）、レプリカ間の短いロックを他が保持している（`locked`）。`429` は二種類：外向きプローブ（単体 key の疎通確認、チャネル疎通テスト）は**識別子ごとにレート制限**され互いを塞ぎません。手動補充の枠のガードレール（最小間隔と 1 日の回数）は [REGISTRAR.md](REGISTRAR.md) の「四つのガードレール」にあります。
...
> [!WARNING]
> 削除は**取り消せません**：レコードの中の key 素材はそこで消え、どこにも残っていません。だからこれは前提条件を持つ唯一の書き込みです——**無効化済み、または（上流の 401/403 で）システムに排除済みの key だけが削除でき、どちらか一方を満たせば十分です**。どちらでもない key には `409` とトップレベルの `reason: "must_disable_first"` が返ります。
...
| パラメータ | 型 | 必須 | 説明 |
|----------|----|----|----|
| `expect` | number | はい | 画面で見たプールサイズ、0 以上の整数；食い違うとトップレベルの `reason: "pool_size_changed"` 付きの `409` で、一本も削除されません。 |
...
チャネルの疎通テスト：利用可能ドメインを列挙し、そのうえで**実際に認証情報を検証**します。方法はチャネルごとに異なり、一時メールボックスを作って削除する方式のチャネルでは呼び出しごとにアクティブメールボックス枠を 1 つ使います。key は受け取りません。

`domains` は**探れたドメインの個数**（整数）であって一覧ではありません。`credentials` は `accepted`（上流が認証情報を受け入れた）か `not_checked`（今回は検証していない）。`cleaned` は検証のために作成したものを確実に削除できたかで、`false` なら一時メールボックスが枠を占有したままです。上流のレスポンス本文は返しません。**認証情報が無効なら `ok: false` を返します。緑ではありません。**

`domains` が `0` の場合も `ok: true` ですが、そのチャネルは今プールを補充できません：補充の手順は使えるドメインが無いとそのまま失敗します。この場合 `credentials` は `not_checked` です。

失敗時は `200` + `{ "ok": false }` です。`reason` は 4 種類：`credentials_rejected`（上流が拒否、HTTP 401 / 403）、`rate_limited`（レート制限中、今回は何も測れていない、HTTP 429）、`upstream_error`（それ以外。5xx と「リクエストが最後まで届かなかった」場合を含む）、`not_attempted`（自分の設定すら読めず、**上流へのリクエストが 1 件も出ていない**。確認すべきはストレージ）。上流が実際に応答した場合のみ `status` が付きます——サーバーは値を捏造しません。
