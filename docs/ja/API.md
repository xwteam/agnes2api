...
| パラメータ | 型 | 必須 | 説明 |
|----------|----|----|----|
| `from` | string | いいえ | 区間の始点；解析できなければ Tier-2 の有効/無効に関わらず `400`。 |
| `to` | string | いいえ | 区間の終点；保持期間を超える分はクランプされ、`note` で説明されます。 |
...
**リクエストボディ**：このエンドポイントはボディを取りません。日付はパスに書き、UTC の `YYYY-MM-DD` でなければ `400` です。
