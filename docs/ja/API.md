...
レスポンスの `domains` は**探れたドメインの個数**（整数）であって、ドメインの一覧ではありません——このエンドポイントは上流のレスポンス本文を返しません。失敗時に付くのは 3 桁のステータスコードだけです（下記）。

**`ok: true` はアドレスに到達できてドメインを列挙できたことだけを意味し、認証情報が使えることは意味しません。** この手順はドメイン一覧を読むだけで、この手順で認証情報を確認しないメールサービスもあります——認証情報を間違えていても `ok: true` が返ります。実際に確認されるのは補充時のメールボックス作成の手順です。

`domains` が `0` の場合も `ok: true` ですが、そのチャネルは今プールを補充できません：補充の手順は使えるドメインが無いとそのまま失敗します。

失敗時は `200` + `{ "ok": false }` です。`reason` は 3 種類あります：`credentials_rejected`（上流がこのリクエストを拒否、HTTP 401 / 403）、`rate_limited`（上流がレート制限中、HTTP 429、今回は何も測れていない）、`upstream_error`（それ以外。5xx と「リクエストが最後まで届かなかった」場合を含む）。上流が実際に応答した場合は `status`（その応答の HTTP ステータスコード）が付きます。リクエストが届かなかった場合はこのフィールドが**ありません**——サーバーは値を捏造しません。
