...
画像生成、動画ジョブ作成、そして**すべての非ストリーミング対話**（4 プロトコルとも）は
同期タイムアウト予算 `UPSTREAM_SYNC_TIMEOUT_MS`（既定 120000 ミリ秒、
[デプロイガイド](DEPLOY.md#環境変数)を参照）で動きます。その総予算の中で試したすべての
...
この総予算がクライアントの最悪待ち時間であり、プールの大きさには依存しません。`504` の
時点でゲートウェイはどの key も罰していません。key が罰せられるのは、同一リクエスト内で
別の key が成功した場合だけです。
...
`storage.writable` は「key プールを置くストレージが本当に書き込めるか」を報告します。
起動時の一度きりのプローブと、実行中の実際の書き込みの両方で更新され、ヘルスチェック
自体は書き込みを行いません。書き込めない場合は **HTTP `503`** を返します:
...
イメージ内蔵の `HEALTHCHECK` はレスポンスが成功かどうかで判定するため、この状態の
コンテナは Docker から unhealthy と報告されます。元の例外はコンテナログにのみ出力し、
認証不要のこのエンドポイントには出しません。
...
ゲートウェイは転送前にタスク識別子の形状を検査し、**`A-Za-z0-9_- (1-128)` のみを
受け付けます**。前半が許可される文字集合、括弧内が長さの下限と上限です。一致しない
ものはすべて 400 になり、**上流へのリクエストは一度も発行されません**。400 の
レスポンスボディにはこの形状がそのまま含まれるので、それに合わせて識別子を
貼り直してください。

⚠️ タスク識別子の形状判定は実際の上流では未検証です。本リポジトリのテスト
フィクスチャにあるあの識別子から**外挿**した文字集合と長さ上限であり、そのまま
写したものではありません。上流が別の形状を発行した場合、ゲートウェイはそれを
上流に転送せず 400 を返します。そのときはリクエストパラメータをどう変えても
通らず、ゲートウェイ側を直すしかありません。
