...
- **Worker 形態會自動在牆鐘耗盡前收手，你不需要為此調參數。** 每次準備開始一次鑄 key
  之前，註冊機都會先算「剩餘牆鐘夠不夠完整跑完這一次」（判據是
  `CODE_TIMEOUT_MS × 通道數` 再加上嘗試間隔）；不夠就**根本不開始**，提前結束本輪、
  印出一條 `本轮墙钟预算不足以再完整跑完一次铸 key，提前收尾`，已經鑄好的 key 照常
  入池，剩餘名額留給下個排程週期。
  關鍵在於「不開始」而不是「跑到一半被砍」：被平台從中間中止時，那次正在用的臨時信箱
  來不及刪除就會殘留（YYDS 約 24 小時後隨 `expiresAt` 過期，MoeMail 按 1 小時 TTL 過期），
  而主動收尾能保證每一次嘗試都完整走完、信箱一定被刪掉。
  因此 **`MINT_BATCH` 在 Worker 上是「單輪上限」而不是「保證值」**：配大了不會出事，
  只是單輪可能鑄不滿。Node/Docker 沒有平台牆鐘上限，不啟用這個機制，`MINT_BATCH`
  會照常跑滿。
