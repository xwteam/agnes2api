...
### 除了 GATEWAY_TOKEN，還認面板簽發的對外 API 密鑰

上面四種傳遞方式，值除了填 `GATEWAY_TOKEN`，還可以填管理面板「API 密鑰」板塊簽發出來的**對外 API 密鑰**（`sk-` 開頭）。兩者是**疊加**關係，不是取代：
...
下面兩類請求會在**轉發上游之前**被本地判掉：既不消耗 key，也不消耗上游那層共享的限流額度。

- **無法映射成純文字的內容區塊** —— `/v1/messages`、`/v1/responses`、`:generateContent` 三條協議轉換請求內容時只認文字區塊，遇到 `image` / `input_image` / `inlineData` / `tool_use` 之類一律回傳 `400` 並點名區塊型別，**不會**靜默丟掉再照常回一段答案。`/v1/chat/completions` 原樣透傳，不做這層判斷。
- **轉達不了的生成參數** —— 同上三條協議裡，`tools` / `tool_choice`（不轉換工具呼叫）、Anthropic 的 `top_k` 與 Gemini 的 `topK`（上游請求內容沒有這一格）、Gemini 的 `candidateCount > 1`（只轉換第一條 candidate）一律回傳 `400` 並點名欄位。能安全轉發的是 `temperature`、`top_p`（Gemini 寫作 `topP`）與 `stop`（Anthropic 寫作 `stop_sequences`、Gemini 寫作 `stopSequences`）；各節請求內容表裡沒列出的其餘欄位不會轉發給上游。
...
> [!NOTE]
> `400` 的常見成因（不止這些）：Anthropic 協議裡出現非 `text` 內容區塊、Gemini 路徑缺少方法名、影片任務識別碼形狀非法、管理介面的請求內容欄位不認識或缺必填項、欄位值越界（備註過長 / 一次匯入超過 200 把 / `op` 不合法）、查詢參數格式非法（用量端點的 `from` / `to`）。`409` 有六種：刪一把既沒停用、也沒被系統剔除的 key，清空池時池大小與你看到的對不上，註冊機沒啟用，通道沒配憑證，補池已在途（`tend_in_flight`），跨副本短鎖被別人佔著（`locked`）。`429` 分兩類：對外探測（單把 key 驗活、通道連通性測試）**按識別碼限速**、互不牽連；手動補池的名額護欄（最小間隔與每日次數）見 [REGISTRAR.md](REGISTRAR.md) 的「四條護欄」。
...
傳 `"stream": true` 時回應為 `text/event-stream`，按官方最小事件序列依次攜帶：`response.created`、`response.output_item.added`、`response.content_part.added`、一個或多個 `response.output_text.delta`、`response.output_text.done`、`response.content_part.done`、`response.output_item.done`、`response.completed`。最後那個事件的 `response.output[]` 是完整的最終物件。上游串流中途斷開時改發 `response.failed`，而且**不會**再發 `response.completed`。
...
> [!IMPORTANT]
> 若 `content`（或 `system`）陣列裡出現無法對應到內部純文字格式的區塊——任何非 `text` 型別，例如 `image`、`tool_use`、`tool_result`——閘道會在轉發上游前直接回傳 `400`，而不是像早期版本那樣靜默丟棄該區塊。報文裡那句 `不支持的内容块类型: image（本网关仅支持 text）` 中的區塊型別會換成實際收到的值。**這條規則今天同樣適用於 `/v1/responses` 與 `:generateContent`**（早期版本只有本條協議執行它），逐條見「錯誤碼」一節。
...
> [!IMPORTANT]
> 路徑裡的方法名是白名單：本閘道只實作 `generateContent` 與 `streamGenerateContent`。`:countTokens`、`:embedContent` 以及任何拼錯的方法名一律回傳 `404`，**不會**被當成一次對話真發給上游。
...
串流的最後一幀是**終幀**：`parts` 為空，帶 `finishReason`（`STOP` / `MAX_TOKENS` / `SAFETY`）與 `usageMetadata`。用戶端靠它區分「說完了」和「被截斷 / 被安全攔了」。上游串流中途斷開時終幀的 `finishReason` 是 `OTHER`，且不帶 `usageMetadata`。
...
> [!WARNING]
> 刪除**不可撤銷**：記錄裡那把 key 材料就此消失，沒有任何地方還留著它。所以它是唯一一條帶前置條件的寫操作——**已停用、或已被系統剔除（上游 401/403）的 key 才刪得掉，兩者滿足其一即可**；兩者都不是的那把會回 `409` 加頂層 `reason: "must_disable_first"`。
...
| 參數 | 型別 | 必填 | 說明 |
|----|----|----|----|
| `expect` | number | 是 | 你在螢幕上看到的池大小，非負整數；對不上就 `409` 加頂層 `reason: "pool_size_changed"`，一把都不刪。 |
...
> [!NOTE]
> `unreadable: true` 說的是「儲存裡那張表讀不出來」，此時 `keys` 是空陣列、`version` 是 `null`——**它不等於「一把都沒有」**，面板必須按這一格分支繪製。`version` 是樂觀並行用的版本號，下面三條寫端點都要把它原樣帶回來。
...
| 參數 | 型別 | 必填 | 說明 |
|----|----|----|----|
| `name` | string | 是 | 1–64 個字元，用來在清單裡認出這把密鑰是誰在用；允許重名。 |
| `expiresAt` | number 或 null | 否 | 到期時刻（epoch 毫秒），必須是將來的時刻；`null` 或缺席 = 不過期。 |
...
| 參數 | 型別 | 必填 | 說明 |
|----|----|----|----|
| `version` | number | 是 | 你手上那份清單的版本號（`GET` 給的那個）。對不上就 `409` 加 `code: "stale_write"`，一個位元組都不寫。 |
| `name` | string | 否 | 同簽發。 |
| `disabled` | boolean | 否 | `true` = 停用。 |
| `expiresAt` | number 或 null | 否 | 同簽發；**明確傳 `null` 是「改成不過期」，缺席才是「這次不動它」**。 |
...
吊銷一把密鑰。成功是 `204`，沒有回應體。

> [!NOTE]
> **版本號走查詢參數 `?version=`，不走請求體**：`DELETE` 帶請求體在規範上合法，但中間代理與部分用戶端會把它丟掉，而一條被靜默丟掉的並行憑證等於「以為帶了保險、其實沒帶」。

它**沒有**「必須先停用才能刪」那道前置（上游 key 池那條有）：這裡刪掉的只是我們自己簽發的一條驗證記錄，重發一把是完全正常的操作，而吊銷一把已外洩的密鑰要越快越好。
...
一次清掉全部**已失效**的密鑰（已停用或已過期），仍然可用的一把都不動。

**請求體**：

| 參數 | 型別 | 必填 | 說明 |
|----|----|----|----|
| `version` | number | 是 | 同 `PATCH`；對不上就 `409`，一把都不刪。 |
...
> [!NOTE]
> 刪掉的這一批與面板上「已停用」「已過期」兩張統計卡之和逐條相同。
...
| 參數 | 型別 | 必填 | 說明 |
|----|----|----|----|
| `channel` | string | 否 | 只能是 `moemail` 或 `yyds`；不給就用設定裡選中的那條。 |
...
某一天的用量明細：按小時、按模型、按協議、按密鑰四張切片。**按密鑰那一維的鍵是簽發時的密鑰 id**，刪掉的密鑰仍以原始 id 留在歷史裡；主口令的用量歸到保留偽 id `master`。
