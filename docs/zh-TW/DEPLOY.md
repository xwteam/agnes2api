...
- **面板寫入操作的寫入側（P3c 新增）——「人點一下才發生」，不進上面那些穩態帳。**
  ...
  - **改一把 key**（停用 / 啟用 / 備註 / 清冷卻 / 清 strikes / 解除剔除 / 重設用量計數）：
    ...
    ⚠️ **括號裡最後那一項是本段唯一一個「面板上點不到」的**：重設用量計數
  ...
  - **清空 Key 池**（`/admin/api/keys/purge`，危險區第二顆按鈕）：**N 次 delete
    ...
    ⚠️ **免費檔的 delete 桶是每天 1,000 次**（與讀、寫、list 各自獨立）⇒ **N 接近 1,000
    時這一顆按鈕自己就打穿當天的刪除配額**，而池子越大它越貴。讀側是一次池快照
    （命中 isolate 快取時 0 次 get）加一次回讀；空池上再點一次是 0 次 delete、0 次 put。
  ⚠️⚠️ **上面「儲存一次設定」與「清空一把憑證」這兩條沒有每日上限，這是刻意的，不是漏了。**
  ...
  **管不到這兩條**。規劃餘量時按這一點來。
  ⚠️ 這幾筆與下面那三欄**不相加**：三欄說的是「沒有人操作時每天固定幾次」，
...
- **Tier-2 用量統計的寫側（P3d 新增，`USAGE_STATS_ENABLED` **預設關**）——它是本期唯一的新寫者。**
  ...
  **關著時這一段的寫量恆為 0**：不建任何記憶體累加器、一次 `storage.put` 都沒有。
  ...
  ⚠️ **這張表和上面那三欄一樣不是上界。** 正確的讀法是：**這張表裡帶硬閘的有兩項**——
...
③ **每個實例每天 13 次 put 是一道硬閘**：耗盡之後當天不再寫，**下一個 UTC 日自動恢復**，
   ...
   ⚠️ **這半句在 Worker 形態下結構上到不了**：那些累加器只在記憶體裡，而 Worker 的 isolate
   ...
   ⚠️⚠️ **Docker 形態下這道閘壓根不存在**，所以這句承諾在那一側無從談起：每天有沒有
   ...
   **別把它讀成「Docker 上撞到 13 次閘也不丟資料」——Docker 上沒有這道閘**；
   ...
   ⚠️ 這道閘**只在單個實例內部生效**——8 個 isolate 就是 8 份各自獨立的 13 次，
...
- **Tier-2 用量的讀取側——它燒的不是每天的讀取配額，是「一次呼叫能發多少條子請求」。**
  ...
  ⚠️ **別拿下面事件板塊那 48 次冷讀當佐證**——48 在上面兩種讀法下都合規，
  它對「60 行不行」一個字都沒有說。要在免費檔上把這一檔用踏實，只能先在真機上量一次。
- **Playground 的影片檔：一次任務最多 `1 + 60` 次上游請求**（1 次建任務 +
  最多 60 拍輪詢，`VIDEO_POLL_MAX_ATTEMPTS`）。⚠️ **這 61 倍只放大上游那邊的配額與 key
...
⚠️ **P3c 起註冊機那一族（`REGISTRAR_ENABLED` / `REGISTRAR_PRIMARY` / `REGISTRAR_FALLBACK` /
...
⚠️ **面板寫進去的憑據以明文落在 KV / `store.json` 裡**，與 P1 的「key 明文存檔」同一水位。
別以為這裡輸的是加密過的 secret。資料目錄 / KV namespace 請按憑據處置。

⚠️ **清掉網關口令而環境變數裡也沒有 `GATEWAY_TOKEN` 時**：當前行程還能跑，
...
⚠️ **「進階」摺疊區裡那個註冊後端位址（`AGNES_PLATFORM_URL`）不是普通設定項。**
