...
- **Tier-2 用量統計的寫側（P3d 新增，`USAGE_STATS_ENABLED` **預設關**）——它是本期唯一的新寫者。**
  ...
  **Tier-2 那 104 是這張表裡唯一一項有硬閘的**（下面第 ③ 條），其餘各項都沒有。
  ...
  ④ **兩種執行環境的行為完全相同，不做執行環境嗅探。** 落盤間隔是後端常數，兩邊同一個值、
     同一條程式碼路徑（請求收尾**同步等待寫完**，不是背景任務——背景任務在 Worker 上會被
     回應返回後的 isolate 停擺靜默截斷）。設計文件裡提過一個 `USAGE_FLUSH_INTERVAL_MS`
     環境變數，**它今天沒有接線，設了不起任何作用**；真要放開它，必須讓每天 13 次這道閘
     跟著間隔一起變，否則把間隔調小只會讓預算更早耗盡、當天剩下的時間完全不寫，
     比預設值更糟。
