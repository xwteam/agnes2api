...
- **寫入側合計，按 `registrar.enabled` 分三欄**（20 把 key、8 個並發 isolate 的口徑）：
  ...
  **`delete` 桶**另計：補池鎖每天釋放 48 次，那個桶今天幾乎閒置。
  ...
  ⚠️ **調密補池頻率不是等量放大，是跳檔**——這三筆全部按輪計費，而上面那條設定
  警告在 `TEND_INTERVAL_MS` 低於 10 分鐘時又給每一輪多加一次。舉一個**完全合法**
  的設定：`TEND_INTERVAL_MS=300000`（5 分鐘）⇒ 每天 288 輪 ⇒
  `80 + 96 + 288 + 288 + 288 = 1,040` 次／天，**已經打穿寫配額**。
  上面三欄都是按預設的 30 分鐘一輪算的，**別把它們當成與頻率無關的常數**。
