...
- **事件板塊（Task 6）的寫入側**：每個 isolate 每天最多寫入 `EVENT_WRITES_PER_DAY`（**12**）
  ...
  **8 個並發 isolate**估算（比本節其餘處用的 3 個更保守：這類個人自用閘道即便流量
  ...
  最小間隔閘。
- **事件板塊的讀取側**：面板輪詢不依賴索引，候選鍵數**有界**——首次開啟面板（或清空
  視圖後重新拉、或直接呼叫 API 不帶 `after`）是「冷讀」，回看 24 個時間窗 × 2 個槽位
  = 48 次 get；此後每次輪詢是「暖讀」，通常只需要 2 次 get（跨過時間窗邊界的那一刻
  是 4 次），**與部署跑了多久、歷史累積了多少事件無關**。持續開著一個分頁輪詢，
  暖讀側的上界約 23,000 次／天（約讀配額的 23%，實際因為只有少數輪詢會跨越窗口
  邊界，一般遠低於這個數）。**這筆帳要加在上面 key 池讀取側之上**：如果你的部署
  已經在池子讀配額上很緊張（見上文「3 個活躍 isolate 就用掉約 99.5%」那個場景），
  開啟事件板塊前建議先調大 `POOL_CACHE_TTL_MS` 或減少同時開啟的面板分頁數。
