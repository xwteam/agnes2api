...
| `POOL_TOUCH_INTERVAL_MS` | 否 | `21600000` | key 的「最後使用時間」最多多久寫入一次；`0` = 每次成功請求都寫入。它是純展示欄位、不參與排程，為它每次請求寫一次 KV 會把免費方案 1,000 次／天的寫入配額吃光，連冷卻與剔除都寫不進去。代價：面板「最後使用」的精度最粗到這個間隔。同一個間隔也管著面板上的用量計數（請求數 / 成功率）。**手工改小儲存裡的 `stats`（例如歸零）不會立刻反映到面板上**：還在執行的實例記著自己的寫入基線，會按基線繼續回報，最遲到這個實例被回收之後才一致；P3c 會提供一條經過 repo 的正式重設路徑。 |
