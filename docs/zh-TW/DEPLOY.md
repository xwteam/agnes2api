...
| `POOL_CACHE_TTL_MS` | 否 | `60000` | 每個 isolate／行程在記憶體裡快取一份 key 池快照，本值是它的存活時長；`0` = 關閉快取。**KV 讀取次數與請求數無關**，只取決於重新整理頻率，算法見下文「配額帳」。代價：其他 isolate 判定的冷卻／剔除，本 isolate 最多晚**本值 + 約 60 秒**才看到（後者是 KV 邊緣快取的預設 `cacheTtl`）；預設 60000 時上界約 **120 秒**。而且不只是「看得晚」：陳舊快照上的任意一次排程寫入會把整條記錄覆寫回去，**抹掉**其他 isolate 在這個視窗裡剛寫下的 `evicted` / `cooldownUntil`，那次判定要重新發生一遍。 |
| `POOL_TOUCH_INTERVAL_MS` | 否 | `21600000` | key 的「最後使用時間」最多多久寫入一次；`0` = 每次成功請求都寫入。它是純展示欄位、不參與排程，為它每次請求寫一次 KV 會把免費方案 1,000 次／天的寫入配額吃光，連冷卻與剔除都寫不進去。代價：面板「最後使用」的精度最粗到這個間隔。同一個間隔也管著面板上的用量計數（請求數 / 成功率）。**手工改小儲存裡的 `stats`（例如歸零）之後，面板可能先顯示為已歸零、隨後又被頂回舊值**：快照過一個 TTL 會讀到歸零後的值，而還在執行的實例記著自己的寫入基線，下一次真正寫入又會按基線寫回去，最遲到這個實例被回收之後才真正一致；P3c 會提供一條經過 repo 的正式重設路徑。 |
...
- **註冊機的寫入側（P3c Task 1 新增）——這一整段只在 `registrar.enabled` 為真時存在。**
  ...
  裡的 Cron，預設 `*/30 * * * *` = 每天 48 輪；Node 是 `TEND_INTERVAL_MS`）：
    ...
    - **補池事件寫入**：每輪最多一次 put。**決定寫不寫的是「這一輪的事件緩衝空不空」**
      ...
      於是是 0。⚠️ **「健康時是 0」有一個前提，必須說清楚**：`loadConfig` 有一條
    ...
    - **補池歷史（`tend:history`）**：每輪一次 get + 一次 put，**無條件**。
      單鍵、無扇出。
  ⚠️ **不要拿 `EVENT_WRITES_PER_DAY`（每 isolate 每天 12 次）去理解這三筆。**
...
- **事件板塊的讀取側（評審 C4/C4b 修復後的真實帳目，比早期草稿更保守）**：面板輪詢
  ...
  **但「上界有限」不等於「開銷與活躍度無關」**——評審 C4b 點出一個反直覺的結論：
    ...
    - **長期沒有新事件的「安靜」部署**（含剛部署完、尚未觸發過任何診斷事件）：
      ...
      ⚠️ **這個數是「穩態閒置界」，不是上界。** 它假設面板就那麼開著、沒有人動它；
...
- **`list` 與 `delete` 是另外兩個桶，各 1,000 次／天**，與讀、寫那兩個桶互不相通。穩態轉發
  一次 `list` 都不用——`pool:index` 索引鍵正是為此存在的。會消耗它的一共**三處**：每天
  48~96 次索引對帳（獨立的排程任務）；**池子為空時的兜底掃描**（索引合法卻一條活記錄都
  ...
  期間每個 isolate 每天最多 144 次，疊上索引對帳的 48~96 次仍有餘量。
...
| 變數 | 是否必填 | 預設值 | 說明 |
|---|---|---|---|
| `ADMIN_TOKEN` | 否 | 無（面板不啟用） | 管理介面的口令。**必須與 `GATEWAY_TOKEN` 不同**，且至少 24 位。另外**首尾不得有空白**：HTTP 請求標頭的值在傳輸層會被去掉首尾空白，而環境變數不會，帶空白的口令任何用戶端都送不出來。口令還必須只由**可列印 ASCII（0x20–0x7E）**組成，這條限制有三段不同性質的理由：① 漢字 / emoji / 零寬空格這類碼點大於 U+00FF 的字元，以及換行與 NUL，瀏覽器在設定請求標頭時**直接拋錯**——請求壓根送不出去，留著它只會裝出一棵 200 但永遠進不去的面板，而且服務端連一條 `admin.login_failed` 都不會有；② **除 TAB 外**的其餘控制字元（`0x01–0x08`、`0x0B`、`0x0C`、`0x0E–0x1F`、`0x7F`，共 29 個）瀏覽器送得出去，但會被 HTTP 解析器判成 `400`；③ **TAB（`0x09`）**與 `é` / `£` / 不間斷空格這類 `0x80–0xFF` 的字元**其實送得出去也能用**，攔它們是本網關的**穩健性取捨**而不是物理限制；**兩者的理由並不相同**：TAB 是**不可見字元**，貼進 `.env` 或 secret 裡沒人看得見（與首尾空白同類的可診斷性問題，只是那條是物理、這條是取捨）；`0x80–0xFF` 則是編碼口徑問題——環境變數按 UTF-8 解、HTTP 標頭值按 Latin-1 解，兩邊在這一段並不由規範保證一致（我們也只在 Node 上驗過，Cloudflare Workers 側未驗），而 RFC 9110 已把這段標為棄用。請用純 ASCII 口令。**中間帶空格是允許的**——`correct horse battery staple` 這類 passphrase 送得出去也送得到，而且在 24 位下限之下它往往比隨機串更好記、更不容易抄錯；首尾空白由上面那條規則管著。未設定或不合規時整棵 `/admin` 樹都不註冊，具體原因記在 `admin.token_rejected` 事件裡。 |
