...
| `POOL_CACHE_TTL_MS` | 否 | `60000` | 每个 isolate/进程在内存里缓存一份 key 池快照，本值是它的存活时长；`0` = 关闭缓存。**每天的 KV 读取次数 = 活跃 isolate 数 × (86400 ÷ 本值秒数) × (1 + 池中 key 数)，与请求数无关。** 代价：别的 isolate 判定的冷却/剔除，本 isolate 最多晚这么久才看到。 |
