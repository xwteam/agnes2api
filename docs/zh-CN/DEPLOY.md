...
| 变量 | 是否必填 | 默认值 | 说明 |
|---|---|---|---|
| `ADMIN_TOKEN` | 否 | 无（面板不启用） | 管理接口的口令。**必须与 `GATEWAY_TOKEN` 不同**，且至少 24 位。未设置或不合规时整棵 `/admin` 树都不注册。 |
| `TRUST_PROXY` | 否 | 未设置（**不**信任 `X-Forwarded-For`） | 只有在网关确实位于你自己的反代之后时才设成 `1`。设了之后，`X-Forwarded-For` 的首段会被当作客户端 IP 写进登录失败事件。 |
...
这条规则在启动时校验，**并且在每一个管理请求上复查**。若两者在运行中变成相同——例如用
`wrangler kv key put` 或直接编辑 `store.json` 手工把 `gatewayToken` 写成了管理口令——管理
接口会**立刻开始返回 `503`**，同时打一条 error 级的 `admin.token_conflict`。**网关转发不受
影响。** 把其中任一把口令改回去，管理接口会自行恢复：改存储里的 `gatewayToken`，在配置缓存下一次
刷新之后生效，不需要重启；改 `ADMIN_TOKEN` 则因为它是环境变量，需要重新部署（Worker）或重建
容器（Docker）。
...
事件，无脑信任 `X-Forwarded-For` 等于允许任何人把爆破痕迹嫁祸给任意 IP。关闭时改用
`CF-Connecting-IP`（Cloudflare 由平台注入，客户端伪造不了）；两者都拿不到时该字段如实记
`null`，**绝不伪造一个 `"unknown"`**——那会被当成一个真实来源。
