...
| 变量 | 是否必填 | 默认值 | 说明 |
|---|---|---|---|
| `ADMIN_TOKEN` | 否 | 无（面板不启用） | 管理接口的口令。**必须与 `GATEWAY_TOKEN` 不同**，且至少 24 位。另外**首尾不得有空白**：HTTP 请求头的值在传输层会被去掉首尾空白，而环境变量不会，带空白的口令任何客户端都送不出来。口令还必须只由**可打印 ASCII（0x20–0x7E）**组成：含汉字 / emoji / 零宽空格 / 控制字符的口令浏览器根本发不出去（`fetch` 设置请求头时直接抛错），留着它只会装出一棵 200 但永远进不去的面板，而且服务端连一条 `admin.login_failed` 都不会有。**中间带空格是允许的**——`correct horse battery staple` 这类 passphrase 送得出去也送得到，而且在 24 位下限之下它往往比随机串更好记、更不容易抄错；首尾空白由上面那条规则管着。未设置或不合规时整棵 `/admin` 树都不注册，具体原因记在 `admin.token_rejected` 事件里。 |
