...
- **Tier-2 用量统计的写侧（P3d 新增，`USAGE_STATS_ENABLED` **默认关**）——它是本期唯一的新写者。**
  ...
  **Tier-2 那 104 是这张表里唯一一项有硬闸的**（下面第 ③ 条），其余各项都没有。
  ...
  ④ **两种运行时的行为完全相同，不做运行时嗅探。** 落盘间隔是后端常量，两边同一个值、
     同一条代码路径（请求收尾**同步等待写完**，不是后台任务——后台任务在 Worker 上会被
     响应返回后的 isolate 停摆静默截断）。设计文档里提过一个 `USAGE_FLUSH_INTERVAL_MS`
     环境变量，**它今天没有接线，设了不起任何作用**；真要放开它，必须让每天 13 次这道闸
     跟着间隔一起变，否则把间隔调小只会让预算更早耗尽、当天剩下的时间完全不写，
     比默认值更糟。
