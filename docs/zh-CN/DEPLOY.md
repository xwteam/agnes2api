...
- **事件板块（Task 6）的写侧**：每个 isolate 每天最多落盘 `EVENT_WRITES_PER_DAY`（**12**）
  ...
  **8 个并发 isolate**估算（比本节其余处用的 3 个更保守：这类个人自用网关即便流量
  ...
  最小间隔闸。
- **事件板块的读侧**：面板轮询不依赖索引，候选键数**有界**——首次打开面板（或清空
  视图后重新拉、或直接调 API 不带 `after`）是"冷读"，回看 24 个时间窗 × 2 个槽位
  = 48 次 get；此后每次轮询是"暖读"，通常只需要 2 次 get（跨过时间窗边界的那一刻
  是 4 次），**与部署跑了多久、历史积累了多少事件无关**。持续开着一个标签页轮询，
  暖读侧的上界约 23,000 次/天（约读配额的 23%，实际因为只有少数轮询会跨越窗口
  边界，一般远低于这个数）。**这笔账要加在上面 key 池读侧之上**：如果你的部署已经
  在池子读配额上很紧张（见上文「3 个活跃 isolate 就用掉约 99.5%」那个场景），
  打开事件板块前建议先调大 `POOL_CACHE_TTL_MS` 或减少同时打开的面板标签页数。
