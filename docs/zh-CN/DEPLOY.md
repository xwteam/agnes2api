...
- **写侧合计，按 `registrar.enabled` 分三栏**（20 把 key、8 个并发 isolate 的口径）：
  ...
  **`delete` 桶**另计：补池锁每天释放 48 次，那个桶今天几乎闲置。
  ...
  ⚠️ **调密补池频率不是等量放大，是跳档**——这三笔全部按轮计费，而上面那条配置
  警告在 `TEND_INTERVAL_MS` 低于 10 分钟时又给每一轮多加一次。举一个**完全合法**
  的配置：`TEND_INTERVAL_MS=300000`（5 分钟）⇒ 每天 288 轮 ⇒
  `80 + 96 + 288 + 288 + 288 = 1,040` 次/天，**已经打穿写配额**。
  上面三栏都是按默认的 30 分钟一轮算的，**别把它们当成与频率无关的常数**。
