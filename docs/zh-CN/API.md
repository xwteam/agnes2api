...
概览页的一次取数：版本、服务端时钟、运行时、存储健康、池健康与 Tier-1 池级聚合。
...
| `limit` | number | 否 | 本页条数，默认 50，上限 200。 |
...
> [!IMPORTANT]
> 这里的 `counted` **不叫 `available`**：它的判据是「占名额数」，被停用的与正在冷却的 key 都算在里面，而这两种恰恰都不能打上游。真正的可用数是并列的那个 `fresh`。
