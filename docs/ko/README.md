...
| 날짜 | 변경 내용 |
|------|----------|
| 2026-09-09 | v0.2.0 - 🔧 **레지스트라 대수술**(파괴적 변경 포함): 두 메일함 채널이 "주 채널 + 자동 대체"에서 **둘 중 하나를 고르는** 방식으로 바뀌었습니다. `registrar.primary` / `registrar.fallback`은 `registrar.channel`로 합쳐졌고, 기존 설정도 읽으며 빠진 채널은 이름을 밝혀 알려 줍니다. 보충이 스스로를 잠가 버리는 일도 없어졌습니다 — 업스트림 속도 제한에 걸리면 그 라운드를 즉시 중단하고 단계별로 지수 백오프하며, **업스트림이 차단한 도메인을 기억**해 다음 라운드에서 그것들에 정원을 낭비하지 않습니다. 패널의 "연결 테스트"는 이제 도메인 목록만 읽는 것이 아니라 **실제로 자격 증명을 검증**합니다(키를 잘못 붙여 넣어도 예전에는 초록으로 나왔습니다). 그 밖에 문구가 구현보다 더 많이 주장하던 곳들을 바로잡았고, 각각 빨개지는 게이트를 붙였습니다 |
