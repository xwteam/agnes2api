...
> [!TIP]
> 업스트림은 Agnes API key 풀이 공급합니다. 대화는 `agnes-2.0-flash`, 이미지는 `agnes-image-2.1-flash`와 `agnes-image-2.0-flash`, 동영상은 `agnes-video-v2.0`(작업 생성 + 폴링 2단계)을 씁니다. key 풀은 스스로 회복합니다 —— 업스트림의 `429`/`402`는 해당 키를 쿨다운시키고, `401`/`403`은 영구히 제거하며, 일시적 실패가 연달아 임계값에 닿으면 그때도 제거합니다. 시간이 지나면 저절로 돌아오는 부류에는 사람 손이 필요 없습니다.
...
| 날짜 | 변경 내용 |
|------|----------|
| 2026-08-28 | v0.1.0 - 🎉 **첫 릴리스**: 4개 프로토콜 게이트웨이, 레지스트라, 관리 패널이 한 번에 갖춰졌고 같은 코드가 Cloudflare Worker와 Node / Docker 두 런타임에서 함께 돕니다. 네 갈래 인바운드 프로토콜은 같은 업스트림 스케줄러, 같은 key 풀, 같은 실패 원인 판별을 공유합니다. 레지스트라의 임시 메일함 경로 두 개는 엄격하게 대등합니다. 패널은 여덟 개 섹션이며 빌드 단계가 필요 없습니다. 문서는 5개 언어로 각각 한 벌씩 있습니다 |
...
- 업스트림의 `429`/`402`는 해당 키를 단계별 쿨다운에 넣고, `401`/`403`은 영구히 제거하며, 일시적 실패가 연달아 `MAX_STRIKES`에 닿으면 그때도 제거합니다
...
- 업스트림 응답은 기본적으로 스트림 그대로 전달합니다. 비스트리밍 요청이라도 내부에서는 이벤트 스트림을 풀어 다 모인 뒤 한 번에 돌려줍니다
...
2. **스트리밍**: 네 프로토콜 모두 스트리밍을 지원합니다. `stream:false`일 때도 서비스는 내부에서 이벤트 스트림을 풀고 다 모은 뒤 완전한 JSON을 한 번에 돌려줍니다. 업스트림이 오류를 내거나 도중에 스트림이 끊기면 해당 프로토콜 자신의 오류 이벤트로 끝맺으며, 정상 완료인 척하지 않습니다.

3. **key 풀 자가 회복**: 업스트림의 `429`/`402`는 키를 쿨다운시키고, `401`/`403`은 영구히 제거하며, 일시적 실패가 연달아 상한에 닿으면 그때도 제거합니다. 쓸 수 있는 키가 하나도 남지 않으면 구분 가능한 이유를 붙여 `503`을 돌려줍니다. 동기 경로에서는 예산을 다 쓰도록 한 개도 응답하지 않은 경우에 `504`를 돌려줍니다.
