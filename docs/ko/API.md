...
> [!NOTE]
> `400`의 흔한 원인(전부는 아닙니다): Anthropic 프로토콜의 `text` 아닌 콘텐츠 블록, Gemini 경로에 메서드 이름 없음, 비디오 작업 식별자 형태 오류, 관리 엔드포인트의 미지 필드나 누락된 필수 항목, 범위를 벗어난 필드 값(메모 과다 / 한 번에 200개 초과 임포트 / 잘못된 `op`), 잘못된 쿼리 파라미터(사용량의 `from` / `to`). `409`는 여섯 가지: 비활성화도 제거도 안 된 key 삭제, 풀 크기 불일치, 레지스트라 꺼짐, 채널 자격 증명 없음, 충원 진행 중(`tend_in_flight`), 복제본 간 잠금 점유(`locked`). `429`는 두 갈래: 아웃바운드 프로브(단일 key 확인, 채널 연결 테스트)는 **식별자별로 속도 제한**되어 서로를 막지 않고, 수동 충원의 횟수 가드레일(최소 간격과 하루 횟수)은 [REGISTRAR.md](REGISTRAR.md)의 "네 가지 가드레일"에 있습니다.
...
> [!WARNING]
> 삭제는 **되돌릴 수 없습니다**: 레코드 안의 key 재료는 거기서 사라지고 어디에도 남지 않습니다. 그래서 이것은 전제 조건을 가진 유일한 쓰기입니다 — **비활성화되었거나 시스템이 제거한(업스트림 401/403) key만 지울 수 있고, 둘 중 하나만 만족하면 됩니다**. 둘 다 아니면 `409`와 최상위 `reason: "must_disable_first"`가 돌아옵니다.
...
| 파라미터 | 타입 | 필수 | 설명 |
|--------|----|----|----|
| `expect` | number | 예 | 화면에서 본 풀 크기, 0 이상의 정수; 어긋나면 최상위 `reason: "pool_size_changed"`가 붙은 `409`이고 하나도 지우지 않습니다. |
...
두 파라미터는 **epoch 밀리초 정수만 받습니다**: 정수가 아니거나 음수면 무조건 `400`이고 서버는 '선의의 교정'을 하지 않습니다 — `2026-08-01` 같은 날짜 문자열은 잘못된 값으로 걸립니다. **`days`는 파라미터가 아닙니다**: 패널 버튼의 구간일 뿐 서버는 모르며 프런트는 `from` / `to`만 보냅니다.

**요청 본문**: 이 엔드포인트는 쿼리 파라미터만 받고 본문은 받지 않습니다.

| 파라미터 | 타입 | 필수 | 설명 |
|--------|----|----|----|
| `from` | number | 아니오 | 구간 시작. **epoch 밀리초 정수**. 기본값은 `to - 86400000`. 보존 기간 시작보다 이르면 그 지점으로 잘리고 `range.clamped`가 참이 됩니다. |
| `to` | number | 아니오 | 구간 끝. **epoch 밀리초 정수**. 기본값은 서버의 현재 시각. 현재보다 뒤면 현재로 잘리고 `range.clamped`가 참이 됩니다. |
