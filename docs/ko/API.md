...
게이트웨이는 전달하기 전에 작업 식별자의 형태를 검사하며 **`A-Za-z0-9_- (1-128)` 만
받아들입니다**. 앞부분은 허용되는 문자 집합이고, 괄호 안은 길이의 하한과 상한입니다.
일치하지 않으면 모두 400이 되고 **업스트림 요청은 한 번도 나가지 않습니다**. 400 응답
본문에 이 형태가 그대로 담기므로 그에 맞추어 식별자를 다시 붙여 넣으면 됩니다.
...
않고 400을 돌려줍니다. 그때는 요청 파라미터를 아무리 바꿔도 통하지 않으며 게이트웨이
쪽을 고쳐야 합니다.
