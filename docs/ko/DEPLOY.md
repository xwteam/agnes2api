...
- **`list`와 `delete`는 또 다른 두 개의 할당량이며 각각 하루 1,000회**로, 읽기·쓰기 할당량과는
  ...
  키가 바로 그 때문에 존재합니다. 이를 소비하는 곳은 두 군데뿐입니다: 하루 48~96회의 인덱스
  대조, 그리고 **풀이 비었을 때의 폴백 스캔**(인덱스는 정상인데 살아 있는 레코드를 하나도 읽지
  못할 때, 수동으로 가져온 레코드가 인덱스에 빠져 있는지 확인하려고 `list`를 한 번 실행합니다).
  이 스캔은 내장된 **10분** 백오프를 따르므로(환경 변수가 아닌 고정 상수), 풀이 비어 있는
  동안에도 isolate당 하루 최대 144회입니다.
