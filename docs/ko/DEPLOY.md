...
| `POOL_CACHE_TTL_MS` | 아니오 | `60000` | 각 isolate/프로세스가 key 풀 스냅숏을 메모리에 유지하는 시간. `0`이면 캐시를 끔. **하루 KV 읽기 횟수 = 활성 isolate 수 × (86400 ÷ 이 값의 초) × (1 + 풀의 key 수)이며, 요청 수와는 무관.** 대가: 다른 isolate가 판정한 쿨다운/제외가 이 isolate에 보이기까지 최대 이만큼 걸림. |
