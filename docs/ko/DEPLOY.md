...
| `POOL_TOUCH_INTERVAL_MS` | 아니오 | `21600000` | key의 "마지막 사용 시각"을 최대 얼마 만에 한 번 저장할지. `0`이면 성공한 요청마다 저장. 표시 전용 필드로 스케줄링은 전혀 읽지 않으며, 요청마다 쓰면 무료 등급의 하루 1,000회 쓰기 할당량을 다 써버려 쿨다운과 제외조차 기록하지 못함. 대가: "마지막 사용"의 정밀도가 이 간격까지 거칠어짐. 같은 간격은 패널의 사용량 카운터(요청 수 / 성공률)에도 적용됨. **스토리지의 `stats`를 손으로 줄여도(예: 0으로 초기화) 패널에 즉시 반영되지 않음**: 실행 중인 인스턴스는 자신이 저장한 기준값을 기억하고 그 기준값을 기준으로 계속 보고하기 때문. 늦어도 해당 인스턴스가 회수된 뒤에는 일치함. P3c에서 repo를 거치는 정식 초기화 경로를 제공할 예정. |
