...
| `TRUST_PROXY` | 아니오 | 미설정(`X-Forwarded-For`를 신뢰하지 **않음**) | 게이트웨이가 실제로 직접 운영하는 리버스 프록시 뒤에 있을 때**만** `1`로 설정하세요. 설정하면 `X-Forwarded-For`의 첫 구간이 클라이언트 IP로 로그인 실패 이벤트에 기록됩니다. |
...
`admin.login_failed` 이벤트에 기록되므로, `X-Forwarded-For`를 무조건 신뢰하면 누구나 무차별 대입
흔적을 임의의 IP에 뒤집어씌울 수 있습니다. 꺼진 상태에서는 대신 `CF-Connecting-IP`를 사용합니다
(Cloudflare에서는 플랫폼이 주입하므로 클라이언트가 위조할 수 없습니다). 둘 다 얻을 수 없으면 이
필드는 정직하게 `null`로 기록됩니다 — **`"unknown"` 같은 가짜 값은 절대 만들지 않습니다.**
실재하는 출처로 오해되기 때문입니다.
