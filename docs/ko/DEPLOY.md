...
| `MAX_STRIKES` | 아니오 | `3` | 연속된 일시적 실패(타임아웃, 네트워크 오류, 업스트림 `5xx`)가 이 임계값에 도달하면 해당 key를 영구 제거. |
...
들이므로 필요에 따라 설정할 수 있습니다.

위 설정과 무관하게, 업스트림 `401`/`403`은 즉시 해당 key를 영구 제거합니다
—— 이는 "이 key는 더 이상 유효하지 않음" 상태로 취급되며, 회복 가능한 일시적
상태로 취급되지 않습니다.
