...
/**
 * **后果层：这一段注释的开头，在原文里到底住不住在一个字符串里？——独立重算一遍。**
 *
 * 前三条后果层兜底（引擎验正则 / 块注释没闭合 / 模板栈失衡）意图是对的，但**落地仍然在盯符号**
 *（盯 flag 字母、盯 `/*`、盯反引号），第三轮复评实测它们对 `>` 那一族**一条都没接住**。
 * 这一条不看是哪个符号判反的，只看**后果**：致盲的定义是「一个注释开头其实住在别的字面量里」。
 *
 * **重算的方式与主判据完全无关**：只按引号收支扫本行（`"` / `'` / 反斜杠转义），
 * 不认正则、不认除法、不认注释——也就是说，**主扫描器判反的那个符号，这里根本没有对应的分支**。
 * ⇒ 将来出现一个谁都没想到的新形态时，只要它的后果是「注释开头落进了一个字符串」，这里就会红。
 *
 * **只在结论可靠时开口**：本行引号收支没闭合（奇数个引号 / 跨行）⇒ 重算本身没有结论 ⇒ 闭嘴。
 * 这一条不是「宁可错杀」，它是「有把握才开口」。
 *
 * ⚠️ **「这一条到底是盯后果还是又一个盯符号？」——判据是「将来出现一个谁都没想到的新形态时，
 * 它会不会红」，本轮拿两个实验回答了这个问题，答案是「在它覆盖的那一族里，是盯后果」：**
 * · **实验一（把已修的符号退回去）**：把上面那个 `>` 一档改回「一律正则位」（= 上一版的判据），
 *   §2 那一族的三种毒刺形态**仍然全部当场抛**，档名是本条。
 *   ⇒ 它接住那一族**不是因为 `>` 被修好了**，它自己就够。
 * · **实验二（一个没人在修的新形态）**：`of` 是一个完全合法的标识符，同时又在
 *   `REGEX_AFTER_WORD` 里（`for (x of y)` 的那个 `of`）⇒ `const of = 4; … of / 2; …`
 *   今天判反。**没有人在修这一族**，判据表里一个字都没动过，而这一条当场红
 *  （端到端：把那一行加进 `src/ports/logger.ts`，负责裸 `console` 的门禁从「绿」变「红」）。
 * · **但它不是万能的**：下面那条「接不住」写的是真话，别把这一段读成「关上门了」。
 *
 * ⚠️ **它的射程与盲区（实测，不是散文；测法与断言写在
 * `tests/unit/source-guards.test.ts`「已知误伤：两条各含奇数个引号的正则夹着一段注释 ⇒ 独立重算会假红」
 * 与同一节的「判不准要吵」那张探针表里）**：
 * · ✅ 接得住：注释开头落进字符串（`"https://…"` / `"/admin/api/*"` 这种毒刺遍地都是），
 *   与「是哪个符号判反的」无关——`>` 那一族、`of` 当标识符那一族都实测会红。
 * · ❌ 接不住：本行引号数为奇数时（重算没结论）；注释开头落进的是**正则字符组**而不是字符串
 *  （`!/[/*]/` 那一族，全行一个引号都没有 ⇒ 重算说「不在字符串里」，它说的是实话）。
 * · ⚠️ 会误伤：同一行上两条各含**奇数个引号**的正则夹着一段注释（`/["]/ … 注释 … /["]/`），
 *   重算会把两条正则里的引号配成一对，于是判定那段注释住在字符串里 ⇒ **假红**。
 *   射程 259 个文件实测 0 例；真撞上时它是一声响亮的抛，不是静默，改法是把那一行拆开写。
 * @param {string} src
 * @param {number} start 注释开头（那个 `/`）在 `src` 里的下标
 * @returns {boolean} 真 = 重算有结论、且这个开头落在一个字符串里
 */
function openerLivesInString(src, start) {
  ...
  for (let k = lineStart; k < lineEnd; k += 1) {
    if (k === start) insideAtStart = quote !== "";
    const ch = src[k];
    if (quote === "") {
      if (ch === '"' || ch === "'") quote = ch;
      continue;
  ...
  // 本行引号没收支平衡 ⇒ 这次重算没有结论，闭嘴。
  if (quote !== "") return false;
  return insideAtStart;
}

/**
 * 抠掉一段注释之前先过这一关：独立重算说这个开头住在字符串里 ⇒ 当场抛。
 * @param {string} src
 * @param {number} start
 * @returns {void}
 */
function guardOpener(src, start) {
  ...
  fail(src, start, "openerInString",
    "这里被判成一段注释的开头，但按本行的引号收支独立重算，这个开头其实住在一个字符串里 "
    + "⇒ 上游某个斜杠判反了（判成正则/除法之后错位一格，字符串里的双斜杠或斜杠星号就变成了注释开头），"
    + "**接下来会被当注释吞掉的是真代码，而长度/行数/引擎验那几条一条都不会红**。"
    + "这一档不看是哪个符号判反的，只看后果：注释开头不许落在一个字符串里");
}
