 ...
 * MAX_GZIP 取 **3 MiB 的 1/4 = 768 KiB（786432 字节）**，把 UI 这一项摁在 1/4 以内，
 * **给脚本其余 3/4（业务逻辑、依赖）留足空间**。
 *
 * ── 这个分母为什么从 1/8 改成 1/4（三件事，缺一条这段论证就不成立）────────────
 * ① **这个数是怎么来的。** 分母是「留给业务代码的份额」，**不是量出来的线**：
 *    1/8 当初的意思是「UI 最多吃一成多，其余留给路由 / dispatcher / 注册机 / 依赖」。
 *    改成 1/4 是同一句话换一个份额 —— UI 最多吃四分之一，业务代码仍占四分之三。
 *    ⚠️ **本仓从来没量过业务代码那一半今天有多大**，所以两个分母都不是算出来的、
 *    是划出来的。真要把它变成算出来的，得先量一次「整个 Worker 脚本 gzip 后多少、
 *    其中 UI 占多少」—— 那是另一件事，没做就别假装做过。
 * ② **抬它损失了什么。** 这条线的作用是「面板长到某个量级时先吵一次」，
 *    分母翻倍 = 同一类失控要多一倍的量才会被它拦下。具体地说：一条五语言 i18n key
 *    的典型开销在 1 KB 量级（gzip 后），1/8 那条线在余量不足两条 key 时就会红，
 *    1/4 这条要到几百条 key 之后。**中间那一段没有任何别的线接住它** ——
 *    平台那条 3 MiB 在更远的地方，raw 那条按下面的换算也接不住（压缩比不够）。
 * ③ **今天离平台硬限还有多远。** 跑一次本脚本，最后那行会现算给你。
 *    改这个分母的当天：UI 占平台硬限约一成、占本仓上限（1/8 那版）已逼近满格
 *    —— **卡住的一直是这个自设分法，不是 Cloudflare**。
 *    （这里刻意不写死百分比，理由与下面「别把某一天的实测值写死」同一条。）
 * ⚠️ **不许把「因为不够用了」当成理由写在这里。** 不够用是触发重新审视的**由头**，
 *    不是论证本身；论证是上面那三条，它们不依赖任何一次具体的需求。
 ...
 *
 * ③ **两条线的关系。** 最早 raw 那条（1 MiB）与 gzip 那条（384 KiB）几乎重合
 *    （按本仓 2.7x 的压缩比，384 KiB gzip 折回去差不多就是 1 MiB raw），等于两条线
 *    在测同一件事；两条各自抬高之后（raw 2 MiB / gzip 768 KiB）**这个比例没变**——
 *    它们仍然贴得很近，真正把它们分开的是下面那条压缩比换算，不是这两个数本身；
 *    抬完之后它们才真正分工：**gzip 那条管「部署得进去吗」，raw 这条管「这个目录还是
 *    人写的吗」**。⚠️ 因此**报警会先从 gzip 那条来**——要放宽体积，该被重新论证的是
 *    MAX_GZIP 与那个 1/4 的分法，不是这里。
 */
...
const MAX_GZIP = Number(process.env.UI_MAX_GZIP_BYTES ?? PLATFORM_GZIP / 4);
...
// 这一行回答的是「平台那条还剩多少」，本仓自设的那个分法由上面那行负责报。
