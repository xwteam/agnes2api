...
#   一键部署按钮（阶段 5A 的 `016d89c` 删的），而 `check-comment-refs` 够不着 markdown
...
#（它在 `21861c5` 引入、阶段 5 重写 README 时被删：根 `016d89c` + 五份语言版那五笔）。
...
#   不再手写。上面那笔 23 → 30 在下一个提交 `b71eef1` 就被作废了而没人回头重算 ——
