...
#   P3f 阶段 4 评审回填从 3810 改到 3826：W33 声称验收过的四项恒等式（① 标题译名
...
#   文件数不变。
#   P3f 阶段 5B-4 之一从 3840 改到 3851（+11）：W53 的验收是 **6 × 9 = 54 条**锚点，
...
#   理由写在该名册上方那段注释里。
#   P3f 阶段 5B 第 1 轮评审回填之一从 3855 改到 3866（+11）：W136 补的是
#  「跨文档指认的真实性」这条轴——五份 DEPLOY.md 曾逐字指着一颗**全仓已不存在**的
#   一键部署按钮（阶段 5A 的 `cde41db` 删的），而 `check-comment-refs` 够不着 markdown
...
#   文件数不变。
#   P3f 阶段 5B 第 1 轮评审回填之二从 3866 改到 3871（+5，ADJ §63）：五份 README 的
...
#   抽不出末段时报「认不出」）。文件数不变。
#   P3f 阶段 7a 之一从 3871 改到 3876（+5，W125）：`.env.example` 的「不回退」下限
#   此前**只写在规格里**（C20 / ADJ ⑬ 说了三遍「这份文件一个字都不改」），
#   而 W 清单、R 判官、W88 基线三处**都没有承载点**——一条没有判据兜着的「不许动」
...
#   删注释一个名字都不少）。新增 W125 = 2 格实测下限（`wc -l` ≥ 197、
#   `grep -c '^#'` ≥ 139，2026-08-30 实测）+ 2 格反向控制（删 20 行 / 只删注释）
#   + 1 格「认不出要吵」。文件数不变。
#   P3f 阶段 7a 之二从 3876 改到 3890（+14，W124）：阶段 7 有四条验收判据
#  （W99/W102/W104/W107）写的都是「五份 `##` 序列 toEqual 目标骨架」，而**规格全文
...
#   P3f 阶段 7a 之三从 3890 改到 3900（+10，W116）：40 份出货文档的表格分隔行此前有
...
#   示例表不进射程）。文件数不变。
#   P3f 阶段 7a 之四从 3900 改到 3907（+7，W117）：4 空格嵌套列表 **105 处**（全在五份
...
#   P3f 阶段 7a 之五从 3907 改到 3913（+6，W75）：25 份非 README 文档的第 3 行此前各有
...
#   `⚡⚙⚠☕⭐→` 全部漏网）。文件数不变。
#   P3f 阶段 7a 补漏之一从 3922 改到 3924（+2，W124 的验收补齐）：W124 那条验收
...
#      W96a 的 `鉴权 → 认证` 只欠中文两份），条数一掉就红并把剩下的差距逐条打印；
...
#   P3f 阶段 7b 之一从 3924 改到 3933（+9，W103）：五份 `DEPLOY.md` 的环境变量表里
...
#   P3f 阶段 7b 之二从 3933 改到 3943（+10，W98）：`### 配额账` 是全仓最大的一块散文
#  （改写前 283 行 / en 373 行），而 V27 已裁定**不拆 QUOTA.md** ⇒ 只能就地压缩观感。
#   处置两件事一起做：`####` 五层分节 + 一处 `<details>` 折叠最深的那段（13 次 put 的
#   ①–⑥ 推导）。⚠️ **`<details>` 是 C23「这 25 份不用折叠块」的具名例外**，具名例外
#   必须同时上名册，否则下一个人「顺手删掉 details」就把 ADJ ㉑ 推翻了还一格不红。
#   W94 那份名册本期未落地 ⇒ 先以测试内常量表 `DETAILS_ALLOWLIST` 的形态承担，双向钉死。
#   新增 10 格 = 1 格射程自守（五份都定位得到那一节、抽得到 run）+ 1 格名册（恰 5 处）
#   + 1 格「各恰 1 组标签且折叠块落在那一节里」+ 1 格 text-run ≤1500（W98 的验收）
...
#   P3f 阶段 7b 之三从 3943 改到 3956（+13，W99–W102）：五份 `DEPLOY.md` 的 `##` 骨架
#   从 **5 节**（环境变量 / Cloudflare Worker / Docker / 导入 key / 吊销 key）重建成
#   **15 节**，右操作数取 W124 的 `DOC_SECTIONS`，**实施者一个节名都没自编**。
...
#   + 2 格 W100（```env ≥5 个/份；每个围栏的注释结构 1–3 行、必填/可选起头、变量间空行）
#   + 3 格 W100 反向控制（删注释 / 注释没写必填可选 / 合规围栏不许乱红）
#   + 2 格 W101（三段式成对 + 五语言条数相等；不回退下限 ≥6）
#   + 2 格 W102（安全建议 ≥4 条 bullet；页脚节恰 4 条且末节走 R26e'）
...
#   P3f 阶段 7b 之五从 3959 改到 3976（+17，W96/W96a/W104–W107）：五份 `API.md` 从
...
#   一次都不进循环而整组照绿）
#   + 16 格 W104–W107 验收：1 格射程自守 + 1 格端点全部降 `###`（`## METHOD /path` 恒为 0）
#   + 1 格 `### 方式 N` 编号 1..4 + 1 格状态码总表**严格升序且无重复**
#   + 2 格 W105（`**请求**`/`**响应**` 各 ≥10 且五语言相等；五件套次序：`**请求**` 后紧跟
#     ```bash、`**响应**` 后紧跟 ```json/```text）
#   + 2 格 W106（参数表表头逐字 + 必填列只许「是/否」；正文 `✅`/`❌` 恒为 0 —— C26，
#     README 的配置表才用 ✅/❌，两处不是同一条规矩）
#   + 2 格 W107（请求示例节恰三个 `###` 逐字同序；cURL 块内两条注释各恰 1 次）
#   + 4 格该红时红 + 1 格不许乱红（围栏里的 `## GET /x` 不算标题）+ 1 格认不出要吵。
#   ⚠️ **刻意不写死 `###` 的条数**：那个数每往 `router.ts` 加一条端点就过期一次，
#   判据钉的是「五语言彼此相等 + 不回退下限（≥40）」，与 W101 同一形态。
...
# P3f 阶段 7C（W108–W110 + W123 的一半）从 3992 改到 4010（+18）：五份 `ADMIN.md`
#   补 `###`/`####` 两级（0 → 48 / 2）、补 4 段带语言标记的围栏（0 → 4）、排障节改成
#   `### {症状}` + `**原因**` + `**解决**`、页脚从第三种形态统一成形态 A（ADJ ㊷）。
#   新增 18 格：
#   + 4 格 W108（`##` 恰 14 个「骨架不动」/ `###`/`####` 五份相等且不回退 /
#     该红时红「把一个 `###` 降回正文」/ 不许乱红「`####` 不会被当成 `###` 数进去」）
#   + 7 格 W109（≥3 段围栏且**开围栏**语言标注率 100% / 该红时红「```json 写成裸 ```」/
...
#   + 7 格 W110（排障三段式两个标签各恰 1 次 / 条数五份相等且 ≥6 /
