...
#   ⚠️ **「今天前件为空」记的是回填当时的状态，已经过期**：阶段 P3g 把按钮加回六份
...
# P3f 阶段 7C（W120 + W121）从 4039 改到 4047（+8）：五份社区文件轻改排版。
...
#   新增 8 格，全在 `repo-front-door.test.ts` 的「W120 / W121 五份社区文件的排版」一组：
...
# P3f 阶段 7C 第 1 轮评审回填（其二）从 4051 改到 4053（+2）：
...
#   补完再放宽 ⇒ **1 格红**（改之前是 0 格）。
#   同批把抽取逻辑收成唯一一份 `docEnvVarsFrom(md)`，W112a 那格探针改成调它
...
# 4181 → 4260：P3f **整分支评审**的回填，六批 +79 格，逐批点名（文件数仍是 140，
# 补的全是已有文件里的格子；新增的 `tests/helpers/ship-docs.ts` 是 helper 不是测试文件）：
#   · +23  `docs-parity.test.ts` 的 W137「文档真值锚」（发现 1–6：六处与源码相反的行为陈述，
#          各配一格从真源现算的判据 + 禁用字面表 + 反向控制）
#   · + 8  `docs-typography.test.ts` 的 R14（发现 7/11/18：用户裁定 D3 的商用措辞判官，
#          正面表 + 否定守卫 + 负面表 + 6 条反向控制）
#   · + 8  同上文件的 R27 词表两侧（发现 8/12：fail-closed 语义的方向，此前只有 W131 管数字）
#   · + 5  同上文件的 R19'（发现 19：出货文档正文里不许有 HTML 注释 —— 载体过滤那一层）
#   · + 2  `docs-parity.test.ts` 的 W131 射程自守 + `SECURITY.md` 那条该红时红（发现 13）
#   · + 5  W138 页脚形态 A 按公式现算（发现 9/10/20/21）
#   · + 7  ⑥ 跨文档锚点（发现 16）
#   · + 5  W139 五链语言顺序（发现 22）
#   · + 2  偏离名册第 17 条的两格永久探针（发现 17）
#   · + 8  W140 徽章 / logo 引用 / 页脚 `<sub>` 的真源锚（发现 14/23）
#   · + 7  W141 六份 README 配置表的名单与默认值锚（发现 15）
#   合计 +80，其中 ADMIN 页脚那条反向控制由 2 格并回 1 格（条数不符那一支带 `continue`）⇒ 净 +79。
# 4260 → 4271：**阶段 P3g 用户裁定把 Cloudflare 一键部署按钮加回六份 README**
#（它在 `21861c5` 引入、阶段 5 重写 README 时被删：根 `016d89c` + 五份语言版 W40–W44 那五笔）。
# 净 +11 格，文件数仍是 140（补的全是已有文件里的格子）：
#   · + 8  `docs-parity.test.ts` 新增 W142「按钮形态锚」：真扫描 1（六份各恰 1 行、
...
#   · + 2  同文件 W136 的 (B)：**它的前提变了**。回填 W136 时按钮全仓零命中，(B) 的前件
...
# ⚠️ **4271 不变**（P3g 复评发现 1 的回填）：R23'B 那格反向控制的填充量改成**从磁盘现算**，
...
# 4271 → 4278：**P3g 复评发现 2 的回填**，净 +7 格，文件数仍是 140（补的全是已有文件里的格子）：
#   · + 4  `docs-parity.test.ts` 新增 W143「版本条目那三条 bullet 的真源锚」。复评实测：
...
#   · + 3  同文件新增 W144「发版日期七处同一天」。复评实测：把 `docs/ko/README.md`
