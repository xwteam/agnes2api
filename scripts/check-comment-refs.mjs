 ...
 *
 * ── 它为什么存在 ────────────────────────────────────────────────────────────
 * 本仓的台账已经记了二十余次「注释/文档里写下一句假断言」，最近一轮通读评审又新
 ...
 *（回填当场实测出来的射程边界，别再照着字面读成「每一处都被逼着给锚」）：
 ...
 * ⚠️ **「逐条」这两个字本身也得有东西守着**：复评实测，往这张表里加一个
 ...
 * 这句话的核对方式就是去读那三行）；这两族的活口子是**整轮一起量的**
...
/**
 * ⚠️⚠️ **这条漏法是本文件自己登记的，后来补上了，结论留在这里。**
 *
...
function anchorMatches(target, anchor, mustNameACase) {
  ...
  //
  // ⚠️ **反引号也要一起抠掉，而且这一条是补的（阶段 D）**：`flatten()` 已经把注释
 ...
 * ① 讲历史（"那个文件在上一轮评审里被删掉了"）；② 举例子（门禁脚本自己的说明里那些
 ...
 *
 * **它为什么在这里**：一次定向复评抓到过——往一段 JSDoc 中间插入几行
 ...
 *
 * **它为什么在这里**：一次定向复评抓到过，而那一次的形态足够说明问题
 ...
 * 这道门禁**一个都看不见**——`scan-secrets.sh` 那两处当时就是手改的，
 * 写回去机器一声不吭。
 *
 ...
 * 门禁自己那份元测试**里加一格源码级断言，与上面 `.yml` 那格同形——后来补上了，
 ...
 * 「上面那条判据与 check-comment-refs.mjs 的规则 E 是同一条」，把两侧各自钉一个定值：
 * 这一侧比的是**下面 `GATE_ORDINAL_RE` 整块 alternation 的源码字节**，那一侧比的是
 * 抄过去那一份编出来的 `RegExp.source`
 * ——**改下面那两个常量时，那一格会红，那是提醒不是事故。**
 * ⚠️ **上一版这里写的是「做逐字节孪生体比对」，那句话当时是假的（复评发现）**：
 * 那一格只挑了四条 alternation 里的三条断言，排在最前面那一条整条没人看。复评变异
 * 实测：把最前面那条 alternation 结尾那两个字换成一个近义词 ⇒ 那边一格都不红。
 * 现在改成整块比对，两条 alternation 的增删也会红。
 *
