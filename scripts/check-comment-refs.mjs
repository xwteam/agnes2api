...
/**
 * ⚠️⚠️ **登记一条已知漏法，连同实测数据一起留在这里（P3c Task 7 评审提出，本轮没做）。**
 *
 * @refs-ignore（**从这里到本段末尾**：里面的路径是一次扫描的**结果**、不是指向声明，
 * 而 `拦得住`/`抓得住` 是被引用的**词表成员**、不是断言——规则 B 对这两种形态都会误报，
 * 这正是它自己的边界；本段刻意不拆开写，就是为了让豁免区间盖住整段。）
 * 这张表的漏法有一个**最典型的形状**：**肯定式在表内、它的否定式全在表外。**
 * `拦得住` / `抓得住` 在表里，而 `拦不住` / `抓不住` **一个都不在**；
 * 同一族还有 `只拦` / `管着` / `负责`。
 ...
 *    每个锚都是一次写错的机会——而这一轮是四条 Critical 的修复轮，评审面应当最小；
 * ② 更要害的是**评审自己指出的那条**：规则 B 的判据是「有断言性措辞就必须给得出指向」，
 *    **而它检查的是指向存不存在，不是那句话真不真**。本期真正咬人的那句
 *    （配置 handler 里「直接 PUT 一把新的就行」——一句实测为假的承诺）
 *    **连断言性措辞都没有，门禁对它完全隐身**。
