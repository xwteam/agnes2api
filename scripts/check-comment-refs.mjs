 ...
 * 光有上面那两张表治不了这一条：它们钉的是**已经写下来的那几个词**，新说法压根不在
 * 表里，谁也不会红。所以这张表还有一条**结构不变量**盯着：**凡是收了 `X得住`，
 * 就必须同时收 `X不住`**——肯定式与它的否定式是同等强度的断言，而这张表历史上恰恰
 * 长期只收了肯定式那一半（下面那段登记的就是这件事）。判据从**本文件的源码**里逐字
 * 取出这张表再自己推导，不是抄一份期望值，由 `tests/unit/check-comment-refs.test.ts`
 * 的「词表对否定式封闭」钉着。
 *
 * 这张表被扩过几轮：原来只认「钉住/钉着/钉死/守着/会变红/变红」，后来补进
 * 「由 X 保证」「已核实」「已实测」「拦得住」「抓得住」，再后来补「守护」「会红」，
 * 本轮补否定式与归属式两族。**每一轮补的都是当时正在漏的活口子，不是预防性扩容**
 * ——逐词的理由写在下面各自那几行上。
 *
 ...
 * 而锚指不了一个不存在的东西 ⇒ 这种句子只能把路径去掉、用文字说清是哪一份。
...
const CLAIM_MARKERS = [
  ...
  "拦不住", "抓不住", "只拦",
  // ⚠️ **归属式一族**：`由 X 管着` / `由 X 负责` 是本仓声称覆盖时最常用的两种说法，
  // 与表里早就有的 `由 X 保证` 完全同型，而它们一个都不在。
 ...
 * **`.sh` 那一半今天没有补，理由写在这里而不是"以后再说"**：`scan-secrets.sh`
 ...
 * 门禁自己那份元测试**里加一格源码级断言，与上面 `.yml` 那格同形——今天还没有那一格。
 *
