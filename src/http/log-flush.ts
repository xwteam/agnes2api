 ...
 *
 * **必须 `await`**：⚠️ 原来的理由是「fire-and-forget 在 Worker 上会被响应返回后的
 * isolate 停摆截断」，那个形态在 v0.4.0 没了。**结论没变，理由换成今天这两条**：
 * ① 不 `await` 的话落盘失败发生在响应之后，`onError` 那条诊断与引发它的那次请求
 *    再也对不上号；② 进程收到 `SIGTERM`（`docker compose down` / 滚动重启）时，
 *    还挂在半路的那次写没有任何人会等它，最后一批事件直接丢——
 ...
 * 才落，且 进程被结束时那批缓冲本来就会丢（响应里的 `buffered` 字段就是为这件
