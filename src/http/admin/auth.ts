...
/**
 * 口令的字符集：**可打印 ASCII（0x20–0x7E）**。
 *
 * 这条规则原本由**两半**组成，第二半在评审裁定后被**放掉了**。两半的性质不同，
 * 拆分保留，别再把它们说成一件事——**判据是「留物理，去口味」**：
 *
 * ① **留下的那半是物理：0x20–0x7E 之外的字符浏览器真的送不出去。**
 *    `fetch` 在设置含非 Latin-1 或控制字符的请求头值时**直接抛 TypeError**，
 *    于是一个含汉字 / emoji / 零宽空格的 ADMIN_TOKEN 会装出一棵 200 但永远进不去
 *    的面板：用户看到「网络错误」，而服务端**连一条 `login_failed` 都没有**
 *    （请求压根没发出来）。在装配期就说清楚，比让人对着一串看不见的字符排查便宜得多。
 *
 * ② **放掉的那半是口味：空格（0x20）本身送得出去，所以放行。**
 *    本文件的上一版把 0x20 排除在外，理由是「中间带空格的口令只会让人在复制粘贴时
 *    出错」——那是**替运维做主**，不是物理。而 passphrase（`correct horse battery
 *    staple` 那种）是公认更强的密钥形态，在 24 位下限之下运维很自然会用它；
 *    把一个**更强**的形态拒掉，去换一个我们替他担心的复制粘贴问题，不划算。
 *    这与本项目「两条邮箱通道完全平级、不预设主备、不替用户选」是同一条取向。
 *
 *    ⚠️ **首尾空白仍然拦，但那条是物理不是口味**：HTTP 头值的首尾空白在传输层会被
 *    去掉 ⇒ 客户端**送不出原值**，口令明明对却一直 401。它归 `whitespace_padded`，
 *    语义与本条完全无关，别把两条并成一句。
 *
 ...
 *    一串看不见的空格排查便宜得多。
 *
 * ① 字符集：只允许可打印 ASCII（0x20–0x7E）。留物理去口味的理由见 SENDABLE。
 *
