 ...
 *
 *   ⚠️ **这里原来标着「workerd 侧没验，这是个双运行时项目，别把 Node 的实测
 *   当成两边的答案」。v0.4.0 之后只剩一个运行时，那条限定不再有对象**——
 *   全 256 字节穷举就是在今天唯一会跑它的那个运行时上量的。
 *
 ...
 *
 *
 *   ⚠️ **TAB `0x09` 与 `0x80–0xFF` 虽然同属 (c)，拦它们的理由并不相同，别混着说**
 *   （刚在 (b) 上栽过一次范围断言，这里不再重蹈）：
 *     · **TAB**：它是**不可见字符**。粘进 `.env` 或 secret 里没人看得见，与
 *       whitespace_padded 挡的是同一类可诊断性问题——只不过首尾空白那条是物理
 *       （传输层会去掉），TAB 这条纯粹是取舍。
 *     · **`0x80–0xFF`**：理由是编码口径，见下面两条。
 *
 *   `0x80–0xFF` 那半为什么还是拦？两条，都写清楚：
 *     · **跨运行时编码口径不一致是真的**：环境变量按 UTF-8 解、HTTP 头值按 Latin-1 解，
 *       两边碰巧在 U+0080–U+00FF 这段对上是**实现细节**，不是规范承诺
 *      （⚠️ 这条原来还有半句「而 workerd 侧我们没验过」，那一侧没了，删掉）；
 *       **换一个 Node 大版本、换一层反代，这个巧合就可能不成立。**
 ...
 * 运行中不会变的输入上。** `ADMIN_TOKEN` 只从环境变量读，整个进程的生命周期
 ...
 *
 * ② 长度下限 24：**本网关没有任何登录限速**，因此口令熵就是唯一的防线，下限不是建议值。
 *    ⚠️ **这条的理由在 v0.4.0 换过，结论一个字没变**：原话是「Worker 形态没有分布式
 *    限速（做它要拿 KV 当窗口，等于给攻击者一根消耗写配额的杠杆，能把 DoS 面从
 *    『猜口令』扩大到『打死 key 池的状态回写』）」。摘掉 Worker 形态之后，
 *    **单进程本来就做得起内存里的限速**，那条「代价太大所以不做」的论证不成立了
 *    ⇒ **今天的实话是：限速这件事没有做，不是做不了。** 它是一件独立的功能决定
 *    （要定策略、要定五语言面板文案、要考虑反代已经在限速的部署），
 *    不是这次摘形态的顺带产物。在它落地之前，下限 24 仍然是唯一的防线，一格都不许放宽。
