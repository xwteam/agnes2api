...
const REJECT_MESSAGE: Readonly<Record<NonNullable<AdminTokenCheck["reason"]>, string>> = {
  ...
  // 文案只讲**物理**那一条：这些字符浏览器真的发不出去。空格不在其中（送得出去，
  // 因此放行），别再把它写进这句话里——见 auth.ts 的 SENDABLE「留物理、去口味」。
  not_sendable:
    "ADMIN_TOKEN 含有浏览器发不出去的字符，管理面板未启用（网关转发不受影响）。"
    + "只允许可打印 ASCII（0x20–0x7E）：含汉字 / emoji / 零宽空格 / 控制字符的口令，"
    + "浏览器在设置请求头时会直接抛 TypeError，结果是一棵 200 但永远进不去的面板，"
    + "而且服务端连一条 login_failed 都不会有。"
