...
const REJECT_MESSAGE: Readonly<Record<NonNullable<AdminTokenCheck["reason"]>, string>> = {
  ...
  // 文案把**两半**分开讲：0x21–0x7E 之外的字符浏览器真的发不出去，而空格是送得出去的、
  // 排除它是刻意的取舍。混成一句「都发不出去」对空格那半是假话（见 auth.ts 的 SENDABLE）。
  not_sendable:
    "ADMIN_TOKEN 含有不被接受的字符，管理面板未启用（网关转发不受影响）。"
    + "只允许可打印 ASCII（0x21–0x7E）且不含空格：含汉字 / emoji / 零宽空格 / "
    + "控制字符的口令浏览器发不出去（fetch 在设置请求头时直接抛 TypeError），"
    + "结果是一棵 200 但永远进不去的面板，而且服务端连一条 login_failed 都不会有；"
    + "空格本身是送得出去的，排除它是刻意的——首尾空格会在传输层被去掉，"
    + "中间的空格只会让人在复制粘贴时出错",
  too_short:
