 ...
 *    邮箱服务发起过量请求；
 * ② 给它一把存储护栏就要新增写（护栏键是读改写），而本任务把这两条端点**做成了零写
 *    并用一格用例钉住**；上了护栏就得推翻那条性质，并回去把这一笔加进五语言 DEPLOY.md
 *    的写侧账（那笔账今天没有这两条端点，正因为它们不写）。
 *    ⚠️ 这里原来写的是「配额账把这两条新端点算成零写」——**那是我自己的口径，不是
 *    DEPLOY.md 里写着的东西**（那两条端点是本任务才有的），措辞已改准。
