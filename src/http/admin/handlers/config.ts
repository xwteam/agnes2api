    ...
    deps.logger.log({
      ...
      msg: fatalAfter
        ...
        : (blockedAfter.length > 0
          // ⚠️ 上一版的后半句是「或把依赖它的那条通道从主/备里去掉」——**今天没有主/备**
          // 这回事了（两条通道二选一，见 `src/core/registrar/config.ts` 那个字段的说明），
          // 而这条 msg 会渲染进事件板块与容器 stdout，等于教运维去找一个不存在的开关。
          ? "面板清空了一把凭据，注册机因此装不起来了——网关转发不受影响，但补池会停摆、"
            + "池子会慢慢耗干。请在设置页里补回来，或改用另一条邮箱通道。"
          : stillConfigured
