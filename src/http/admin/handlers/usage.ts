 ...
 * **不是另外注入一个 `Storage`**。
 *
 * ⚠️⚠️ **这个决定的理由订正过一次，因为上一版给的危害场景是假的（评审发现）。**
 ...
 *
 * ⚠️ **整块可空，而不是「存储恒在 + 一个 enabled 布尔」**（那条「关必须是零成本」）：
 * 关闭时读路径**在结构上不存在**，不是一个 `if` 挡着。一个「反正 enabled 是 false」
 * 的读路径迟早会被某次改动接上。
 *
 * ⚠️ **代价明写，并且是实测过的**（本任务变异，**装置一并记下否则复现不出来**）：
 * 「关着时不读存储」这条性质**不是一行可以被变异掉的代码**——要证伪它得先把接线
 * 本身改回「存储恒在」。实测那一次动了 **7 处，逐条列全**（上一版只列了 5 条却写着 7，
 * 评审点过）：① `StoreLogger` 加一个 `storage` getter；② `AdminRouterDeps` 多一格
...
export const USAGE_NOTES = [
   ...
   *
   * ⚠️⚠️ **这一条是评审抓出来的第七种状态。缺陷只有一条：`note` 那句话是假的。**
   ...
   *
   * ⚠️⚠️ **立案理由订正过一次，别照上一版那句去理解（定向复评）**：
   * 上一版写「它与 `no_shards` 的**响应体逐字相同**」——**那句是假的**，
   * 而且被同一个提交自己推翻：`malformed` 在那条发现之前就已经在返回语句里，
   ...
   *
   * ⚠️⚠️ **这是定向复评抓出来的第八种状态，而它上一版走的是 `note: null`
...
export function keyUsageHandler(repo: KeyPoolRepo, now: () => number) {
  return async (c: Context) => {
    ...
    // ⚠️ **走 `httpError`，不自己拼 `c.json({error}, 404)`**（评审发现）：
    // `src/http/admin/handlers/keys-write.ts` 那两条同语义的 404 用的就是它
    //（逐字同一句「没有这把 key」）。
    // ⚠️⚠️ **这个改动今天一格用例都不会红，如实登记**（变异实测：
 ...
 * 上一版把两个例子写反了（评审发现）**。逐项实测（node v24 / V8）：
 *
 ...
 * **绝不把读到的那部分当成全份交出去**。
 * ⚠️ **出处订正（评审发现）**：「不许把一段不完整的 30 天显示成完整的」这句话
 ...
 * ⚠️ **立案理由订正（定向复评）**：上一版写「⑦ 与 ③ 的**响应体逐字相同**」
 * ——**那句是假的**，`malformed` 在那条发现之前就已经在返回语句里，两者一直分得开。
 * 完整的订正与那条形状（**给一个真缺陷编一个更吓人的理由，会把它变成假的**）
 * 记在 `USAGE_NOTES` 的 `all_malformed` 上方，**这里不复述**。
 *
 * ⚠️⚠️ **⑧ 是定向复评抓出来的，而它上一版走的是 `note: null`（「一切正常」那一支）。**
 ...
 *
 * · **畸形那两条压过 `range_clamped`，这个顺序订正过一次（定向复评）。**
 *   上一版是反的（`range_clamped` 在前），判据写的是「你要的窗口被缩小了更该先说」。
 *   ⚠️ **那个顺序与档位映射那个场景叠起来会变成永久静音**：面板的 `30d` 按钮只要把
 ...
 *
 * ⚠️ **`tokensCoverage` 不在这里**（评审发现）：它只从 `GET /admin/api/capabilities`
...
export function usageHandler(deps: { usage: UsageWiring | null; now: () => number }) {
  return async (c: Context) => {
    ...
    // （本任务先照 `usageCandidateKeys` 的形状加了它，再变异把它删掉 ⇒
