 ...
 * 共用一个 kind ⇒ 整轮测试**受同一个最小间隔的约束**，那就是**扛上游 CF 1015 的那道闸**
 ...
 *
 * ⚠️⚠️ **上一版这里写的是「共用一个 kind ⇒ 整轮测试自动被摊到最小间隔上」，那句是假的，
 * 而 v0.3.0 的面板正是照着那句话写的**（本轮线上实测推翻）：共用一个 kind 只会
 * **截断**一轮，**摊开一轮必须由发起方限速**。零间隔的串行循环撞上这道闸的结果是
 * 整轮 824ms 跑完、6 行里 5 行 `probe_cooldown` —— 也就是把**我们自己的节流**
 * 当成一张连通性矩阵交给运维，而那 5 行盖住的可能是真的不通的模型。
 * ⚠️ **所以面板那一侧的「串行 + 隔满最小间隔」不是这条的替代品，也不是可选项**，
 * 两者是同一件事的两半，缺哪一半都不成立：
 * · 缺前端那一半 ⇒ 整轮只测得到第一个模型（就是上面那次实测）；
 * · 缺后端这一半 ⇒ 闸只管住「本面板这一个标签页」，两个标签页、一条 curl 循环都绕得过。
 * 前端那一半在 `admin-ui/js/pure/model-test.mjs` 的 `nextTestDelayMs()`，
 * 它镜像的就是 `../probe-guard.ts` 的 `PROBE_MIN_INTERVAL_MS`。
 *
 * ── 响应体：与验活同族 ──────────────────────────────────────────────────────
