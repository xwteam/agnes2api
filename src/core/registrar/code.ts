 ...
 * | 限长到 200 后 | <5 ms | 12~460 ms |
 *
 * 中间那条是第一版限长时**漏掉的**：只限了两段 `[^"']` 而放过了 `[^>]*`，而它比
 * 已经修掉的那两条还差 6 倍。漏掉的直接原因是当时的对抗用例只有**一个** `class=`
 * 起始位置（约 11 ms 轻松通过）——`[^>]*` 的二次律需要大量起始位置才显形。
 * 新增/改用例时请照着「这条正则的外层循环是什么」去构造载荷，别只盯着长度。
 *
...
const TAGGED_CODE = new RegExp(
  `class=["'][^"']{0,${MAX_GAP}}verification[^"']{0,${MAX_GAP}}["'][^>]{0,${MAX_GAP}}>\\s*(\\d{6})\\s*<`,
);
