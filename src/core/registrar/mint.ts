  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    try {
      ...
      if (verdict === "domain_blocked") {
        ...
        //    ⇒ 处置接在 `./tender.ts` 的 `finishRound`：**这一轮成片判出「域名被屏蔽」
        //    且零产出**时按 `cluster` 那一档记一个跨轮退避，把请求量按回去。
        //    「成片」有两条判据，缺一条就漏掉一整类部署：同一轮里好几个域名被判屏蔽
        //    （钳位的 `discarded`），**或者上游列出来的域名一个不落全被判屏蔽**
        //    —— 后一条是给**只配了一个邮箱域名**的部署的，它永远凑不满前一条要的那 2 个。
        //    判据是 `tests/unit/registrar/domain-ledger-io.test.ts` 的
        //   「上游改了限流文案时：一轮打满 mintBatch 次，但记下 cluster 退避把后面几轮按住」
        //    与「只配了一个邮箱域名 + 上游换了限流措辞：退避照样记得下来，请求量被按住」。
        // ③ 🔴 **接上处置之后仍然比那道保险还在时贵，数写出来**（量出来的，不是估的）：
        //    走到指数封顶之后是每 8 轮打一次、每次 `mintBatch` 次 ⇒ 内置值下**约 30 次/天**；
        //    而保险还在时那一档是「撞上就整轮停手」⇒ 约 6 次/天。接处置之前是约 240 次/天。
        //    **30 不是 6** —— 换来的是那把 `at` 冻住的死锁不会回来、以及台账照常学得到结论。
        // 三条加起来仍比 7 天零产出便宜得多，但它**不是零成本**。
