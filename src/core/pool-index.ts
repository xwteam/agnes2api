 ...
 *
 * ⚠️ **它的起因在 v0.4.0 之后不再是最要紧的那条，如实登记**：原话是
 *「改造前每个转发请求消耗 1 次 `list`，而 Cloudflare KV 免费档的 list 配额是
 * 1,000 次/天、与转发请求数 1:1 挂钩 ⇒ Worker + 免费 KV 被卡在约 1,000 次转发/天」。
 * 那个配额随 Worker/KV 形态一起没了。
 * **索引没有跟着删，理由换成今天成立的那一条**：`FileStorage.list()` 是一次
 * `readFile` + `JSON.parse` **整份 `store.json`**，把它挂在每个转发请求上，
 * 池子一大就是逐请求的全量反序列化。索引把它降到 0 次 list。
 *
 * **不退回「整池存成单条 JSON」**：那样确实只要 1 次 get，但一 key 一记录正是为了
