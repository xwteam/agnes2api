 ...
 *
 * `TendResult` 的严格超集，只多一个 `trigger`：**刻意用 `extends` 而不是把九个
 * 字段抄一遍**——抄一遍就会漂，而"两份字段清单悄悄分叉"正是本仓被咬过的形态。
 */
...
function isFailureList(v: unknown): boolean {
  return Array.isArray(v) && v.every((f) =>
    ...
    // `default`；而 `reg.fail.<reason>` 的 i18n 键也只对这十个成员齐全。
 ...
 * `TendRecord` 不一样——它是一张**定长表的一行**，九个字段全部由本仓自己的代码
