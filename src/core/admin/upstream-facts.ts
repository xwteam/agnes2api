 ...
 *
 * ⚠️⚠️ **这张表里只放「这条事实的依据强度」，一个路径 / 字段值都不许复制。**
 * 媒体端点的两半刚被合并进 `src/core/admin/protocol-catalog.ts`，再开一张表有制造
 * **第三份端点知识**的风险。要指向别处时写**符号名**（`anchor: "VIDEO_TASK_ID_RE"`），
 * 由锚断言绑住；表里因此看不到任何一条路径、任何一个字段名的副本。
 *
...
export interface UpstreamFact {
   ...
   * 于是「把限定句挪到文件末尾」「贴在一个毫不相干的端点下面」都照样绿，
   * 而这句话的全部意义就是**贴在读者正要照抄的那段示例旁边**。
   */
...
/**
 * 表体。**它先被写成一个带类型标注的常量，再冻起来对外**：直接给
 * `Object.freeze([...])` 标注返回类型的话，`status` 这类字面量联合会在推断里被拓宽成
 * `string`，于是「只许写这两个值」这条约束在编译期就没了。
 */
const FACTS: readonly UpstreamFact[] = [
  {
    id: "video.taskIdSlot",
    subject: "建任务响应把任务标识放在哪一格",
    assumed: "顶层 id（P3e Task 21 之后：一张具名候选槽表，顶层 id 恒为第一条）",
    status: "assumed",
