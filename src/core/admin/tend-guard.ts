/**
 * 手动补池的**第二、第四条护栏**（设计 §10.2 第 2 条 + 计划登记在案）：
 ...
 * 代价明写：这把键会在存储里长期驻留（1 把）。永久关掉注册机、或删掉容器但保留
 * 数据卷时它不会自己消失，清理靠手工改 `store.json`（五语言 REGISTRAR.md 写了）。
 */
...
/**
 * 每天最多几次手动补池 —— **第四条护栏**（计划登记在案）。
 *
 * 三重护栏挡住了并发与邮箱名额，**挡不住写量本身**：10 分钟冷却只把上界压到
 * `24 × 6 = 144` 次/天 = `144 × 5 = 720` puts/天，而每一次 put 都是一遍
 * `store.json` 全量重写，还与 key 池状态回写共用同一条写队列。
 *
 * ⚠️⚠️ **这条护栏当初的算式是照着 Cloudflare KV 免费档「1,000 次写/天」算的**
 *（720 + 416 = 1,136 > 1,000 ⇒ 直接打穿），**那个分母在 v0.4.0 没了。**
 * **护栏本身留着**：上面那条「一次点击就是几遍全量重写」与它挡的连点是同一件事，
 * 而下面那条「按钮点不动了要说得清楚」的诚实性要求一个字没变。
 * 🟡 **但 24 这个数今天没有推导它的分母**，如实登记：要调它得先定一个新的分母
 *（磁盘写放大能接受到什么程度 / 邮箱名额），那是一次独立的取舍。
 *
 * ── 取 24 的算式（全局约束 13 要求写进五语言 DEPLOY.md，那里有同一份）──────
 *
 * 一次手动补池的**非铸造**写侧成本是**固定 3 次 put**（护栏键 1 + 抢锁 1 +
 * `tend:history` 1）**外加两笔条件写：域名台账与退避键各最多 1 次 ⇒ 上界 5 次 put**，
 * 再加 1 次 delete（释放锁，走另一个桶）；每铸出一把 key
 * 再加 2 次 put（记录 + 索引），而铸造那部分**自限**——`rounds = Math.min(need, mintBatch)`，
 * 缺口填平之后 `need <= 0` 就一把都不铸了。
 *
 * ⚠️ **「固定 4 次」是这一版之前的说法，它两头都不成立，已订正**（评审发现）：
 * 域名台账（`registrar:domains`）只在这一轮真的学到新结论时才写，稳态下是 0
 *（`src/core/registrar/domain-ledger.ts` 的 `commitJournal` 那一段）；而撞上限流的那一次
 * 点击还要多写一把退避键（`src/core/registrar/backoff.ts` 的 `REGISTRAR_BACKOFF_KEY`）。
 * **实测的下界是 3**：`tests/contract/manual-tend.test.ts` 的
 * 「成功一轮的确切代价：3 次 put（抢锁 + 护栏键 + tend:history）+ 1 次 delete，手写字面量」
 * 逐格数过。下面两栏按**上界 5** 算，实际会更低。
 *
 * · 可持续：`24 × 5 = 120` puts/天，叠上「注册机开着且每轮都有失败事件」的稳态 416
 *   ⇒ **536/天 = 53.6%**。
 * · 突发上界（每次都铸满默认的 `MINT_BATCH = 5`）：`24 × 15 = 360`
 *   ⇒ **776/天 = 77.6%**，且这一栏靠邮箱名额与 `targetKeys` 先撞死，不可持续。
 *
 * **K 的基线只取「真正有硬上界的那几项」**（key 池 80 + 补池锁 48 + `tend:history` 48
 * + 域名台账 48 + 退避键 48），不拿会随部署形态浮动的 `12 × M` 去当尺子——那会是拿一个
 * 非上界当基线。
 */
