 ...
 * `24 × 6 = 144` 次/天 = `144 × 4 = 576` puts/天，叠上稳态之后占日写配额约 94%，
 * 而稳态那部分（事件 sink 的 `12 × M`）**本身就不是上界**——`M`（并发 isolate 数）
 * 连用户都不能调。**一个占 75% 且两端都在浮动的账，没有余量可言。**
 *
 * ── 取 24 的算式（全局约束 13 要求写进五语言 DEPLOY.md，那里有同一份）──────
 *
 * 一次手动补池的**非铸造**写侧成本固定是 **4 次 put**（护栏键 1 + 抢锁 1 +
 * `tend:history` 1 + 域名台账 1）外加 1 次 delete（释放锁，走另一个桶）；每铸出一把 key
 * 再加 2 次 put（记录 + 索引），而铸造那部分**自限**——`rounds = Math.min(need, mintBatch)`，
 * 缺口填平之后 `need <= 0` 就一把都不铸了。
 *
 * ⚠️ **域名台账那一笔是本轮新加的，而它只是上界**：`registrar:domains` 只在这一轮
 * 真的学到新结论时才写，稳态下是 0（`src/core/registrar/domain-ledger.ts` 的
 * `commitJournal` 那一段）。下面两栏按上界算，实际会更低。
 *
 * · 可持续：`24 × 4 = 96` puts/天，叠上「注册机开着且每轮都有失败事件」的稳态 368
 *   ⇒ **464/天 = 46.4%**。
 * · 突发上界（每次都铸满默认的 `MINT_BATCH = 5`）：`24 × 14 = 336`
 *   ⇒ **704/天 = 70.4%**，且这一栏靠邮箱名额与 `targetKeys` 先撞死，不可持续。
 *
 * **K 的基线只取「真正有硬上界的那几项」**（key 池 80 + 补池锁 48 + `tend:history` 48
 * + 域名台账 48），不拿会随部署形态浮动的 `12 × M` 去当尺子——那会是拿一个非上界当基线。
 */
