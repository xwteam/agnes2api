 ...
 * ⚠️ **指向订正**：上一版写的是 `:159` 与 `:172` 两个行号，**在 `b004e03` 上就已经
 ...
 * ⚠️ **指向订正**：上一版写的是 `src/core/admin/usage-stats.ts:228`，
 * **在 `b004e03` 上就已经指错了**（那个常量今天在 394 行）——**不是本任务引入的，
 * 是既有欠账**；改成「本文件的 `USAGE_TTL_MARGIN_MS`」之后不会再随增删行漂走。
