 ...
 *
 * ── 零 IO ──────────────────────────────────────────────────────────────
 * 纯数据 + 纯函数，时间一律从参数进。落盘由 `src/http/usage-sink.ts` 的 `UsageSink` 负责。
 *
 * ⚠️ **这一句原先写的是「那个文件今天还不存在，所以这里刻意不写它的路径」。**
 * 落笔那天是实情：`scripts/check-comment-refs.mjs` 校验「注释里的仓内路径必须解析得开」，
 * 一条指向尚未落地的文件的引用会让它红（当时实测过一次），所以只写了类名、没写路径。
 * 后来那个文件真的落地了，而这句话没人回头改，于是它从实情变成了假话——
 * **这正是那道门禁存在的理由所指的那类漂移，只是它抓不到这一种**：
 * 门禁查的是**已经写下的路径**，而一句「刻意不写路径」的散文，它眼里根本没有路径可查。
 * ⇒ 补上真路径的同时把这条边界一起记下：**回避写路径 = 回避这道门禁**，
 * 下次再想用「那个文件还不存在」当理由省掉一条指向时，先想清楚谁来负责把它改回来。
 *
...
export interface UsageBucket {
   ...
   * 而 `scripts/check-comment-refs.mjs` 这道门禁看不见这类矛盾（规则 B 查的是指向存不存在，不是那句话真不真）。
 ...
 * **两页对不上 ⇒ 按本仓对双运行时那条待验证项 / 设计 §17 同一套纪律处置：当作「没有平台承诺」，不臆测。**
 *
 * ⚠️⚠️ **控制端已就此裁定（计划 §配额账里为它专写的那一节）：这个 30 保留，
 ...
 * ⇒ **那一条只被了结了一半**：「代码路径跑不跑得完」有实测了，「线上免费档会不会超」
 ...
 * ⚠️ **指向订正**：上一版写的是 `:159` 与 `:172` 两个行号，**在 `36406888c` 上就已经
...
/**
 * Tier-2 用量键的前缀。**提成常量是为了让它可被扫到**：本仓对全部存储键做了一份
 * 「重置到底重置了什么」的封闭登记（九把键逐把表态），而钉住那份登记的几格里有一格是
 * **按名字扫源码**（`export const …_KEY / …_KEY_PREFIX = "字面量"`）。
 * 键名藏在模板串里的时候那一格扫不到它，于是「新增一个存储键而不进登记」就没人拦。
 */
 ...
 * 那两道护栏是它的一部分，不是可有可无的防御性代码**（成对的那两条评审发现，
 * 说的就是这件事；另一半在 `tests/unit/admin/usage-stats.test.ts` 那一格上）：
 *
 ...
 *
 * ⚠️ **夹逼的理由就是上面那条上界，不是「反正已经过期」**（评审点出来的，这里原来
 ...
 * 写端点那一期把畸形事件条目穷举过一遍（不是抽样，一共五行形态），
 * 其中三行让端点 **500**、两行**原样进了响应体并让游标永远推不动**。
 * ⚠️ 那份穷举表在一份内部计划文档里，**未随公开仓发布**；结论已逐字写在这里，
 * 数字与后果不依赖那份文档。
 ...
 *
 * ⚠️ **`hours` 是跨天求和的，调用方必须传单日区间**（评审点名，这是一条契约不是实现细节）：
...
/**
 * ⚠️ **数组要单独拒掉**（评审点过名）：`typeof [] === "object"` 且 `[]` 是 truthy，
 ...
 *
 * ⚠️ **它有一处连带变化，明写出来免得下一个人以为只影响 `total`**（评审另点的一条）：
