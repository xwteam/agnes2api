 ...
 *
 * ⚠️ **下面这笔配额账的分母（Cloudflare KV 的每日配额桶）在 v0.4.0 没了，数字没有
 * 重算**——全文见 `src/ports/storage.ts` 的文件级登记；这里的算式保留原样，
 * 它今天管的是「每 10 分钟至多一次整份 `store.json` 的 `readFile` + `JSON.parse`」。
 *
 * `list` 是独立于读配额的第四个桶，免费档 1,000 次/天（设计文档 §17），
 * 而快照缓存只把它压到「每个 TTL 一次」= 60 秒 TTL 下 1,440 次/天/副本，**本身就超配额**。
 * 两扇门都通向同一个桶，所以退避也必须是同一个：
 *   · 空池兜底（索引合法但一条活记录都读不到）；
 *   · 索引缺失回落（`readIndex()` 返回 null，多半是写桶打穿导致索引一直建不起来）。
 * 只堵前者时，后者实测仍是 1,440 次/天/副本。
 *
 * 取 10 分钟 ⇒ 86400/600 = **≤144 次/天/副本**，与文档「配额账」用的同一套
 * 副本计数口径（3 个活跃副本 ⇒ 432 次）叠上对账的 48~96 次仍有余量。
 *
 ...
 * ——那句是假的**（通读评审查实）：那条用例只断言第 4 次尝试之后 `st.lists`
 ...
 *
 * 全局 `crypto.subtle` 是**既定豁免**（硬约束 2）：WebCrypto 是标准全局 API，
 * 不属于「环境能力注入」的范畴——注入它只会多一个端口、多一份假实现，
 * 换不到任何可测性。
 */
...
export interface KeyPoolRepoOptions {
  ...
  logger: Logger;
  /**
   * 进程级快照缓存的存活时长。**0 = 关闭缓存**（每次 all() 都真读存储）。
   *
   * ⚠️ **下面这笔配额账的分母（Cloudflare KV 的每日配额桶）在 v0.4.0 没了，数字没有
   * 重算**——全文见 `src/ports/storage.ts` 的文件级登记。算式保留原样：它今天数的是
   * **整份 `store.json` 的读次数**，那仍然是一件要紧的事，只是没有百分比可算了。
   *
   * 它决定每天的存储读取次数，而读取次数与请求数**无关**：
   *
   *   读取次数/天 = 活跃副本 数 × [ (86400 / 本值秒数) × (1 + 池中 key 数)
   *                                  + 86400 / CONFIG_TTL_MS 秒数 ]
   *                + 对账（每天 48~96）
   *
   * **中括号里第二项是 `ConfigHolder` 的，别漏**：它每 30 秒也刷新一次配置键，
   * 吃的是同一个 100,000/天的桶，每个实例 每天 2,880 次。漏算它会把余量算多
   * 将近一成——本项目已经栽过一次「把有条件的保证写成无条件」。
   *
   * 默认值（本值 60 秒、20 把 key、配置 TTL 30 秒）下每个实例 每天
   *   1440 × 21 + 2880 = 33,120 次，**3 个活跃副本 就用掉 99.36%**
   *（3 × 33,120 = 99,360；再加上每天 48~96 次索引对账约 99.4%，五语言 DEPLOY.md
   *  写的是含对账的那个数。原来这里写的是「99.5%」，与它自己上一行的算式对不上）。
   * 也就是说默认值在推荐配置处已经临界；预期副本更多就要把本值调大
   * （20 把 key、5 个副本 需要约 120 秒）。
   *
   * 代价：别的副本 判定的冷却/剔除，本实例 最多晚**本值 + 约 60 秒**才看到
   ...
   * **抹掉**别的副本 在这个窗口里刚写下的 `evicted` / `cooldownUntil`——那次判定
   ...
   *
   * 改造前每个成功请求写一次存储 —— 而每一次写都是**整份 `store.json` 重写**。
   *（⚠️ 这里原来的理由是「免费档写配额 1,000 次/天 ⇒ 与 list 一起把转发数卡死在
   * 约 1,000/天」，那个分母在 v0.4.0 没了，见 `src/ports/storage.ts` 的文件级登记；
   * **结论没变**：`lastUsedAt` 是纯遥测字段，为它付一次全量重写不划算。）
...
export class KeyPoolRepo {
  ...
  /**
   * 进程级池快照。**与 `ConfigHolder` 建在同一个 `Refreshable` 上**，不是为了省
 ...
 * 分叉立刻变红。
   ...
   * 「最多晚一个 `touchIntervalMs` 落盘 + 进程在落盘前退出时丢这一段」。
   *
   ...
   * `maxStats` **只增不减**地吸收「别的副本 写得更高」的情形。
   *
  ...
  async all(): Promise<KeyRecord[]> {
    ...
    // 直接把缓存数组交出去等于让一次请求的中间状态污染 进程级缓存。
   ...
   *
   * **不加这一步会出人命的场景**（评审实测复现；当时的复现环境是 Worker + 真 KV，
   * **同一条链在今天的 Docker 上逐字成立**）：全新部署上定时轮先在空存储上跑了一次
   * 对账，写下**权威的空索引** `{"v":1,"ids":[]}`；用户随后按 DEPLOY.md 手工往
   * `store.json` 里塞 key——注册机默认关闭时手工导入就是装 key 的路径之一。
   * 此时索引「合法」，永远不会走缺失回落，`all()` 恒返回 0 条，网关一直 503 pool_empty。
   * 改造前 `all()` 直接 `list("key:")`，导入即刻生效——这是本次改造引入的回退。
   *
   * 代价**不**可控，这一点原来这里写错了（原文是「池子真空时本来每个请求都在返
   * 503，这次 `list` 不吃有效配额」）：`list` 是独立于读配额的第四个桶，免费档
   * 1,000 次/天，而快照缓存只把它压到「每个 TTL 一次」= 1,440 次/天/副本，
   ...
   *
   * 校准要处理的是「别的副本 把计数写得更高」：快照过 TTL 之后带回对方的值，
   * 取大就把它吸收进来，本实例不会再把它压回去。
   *
   * ⚠️ **但只能在 `delta` 为零时吸收。** `delta` 非零时 `seen` 里含着的正是
   ...
   * 的那种**——这句话第一版写反了，是实测订正的：只有 `lastUsedAt` 在动时，
   ...
   *
   * 缺它时的失效链（评审实测复现，单副本 Docker 也中招）：
   * 加了池快照缓存之后 `all()` 交出的是一份最长 `poolCacheTtlMs` 的进程级快照，
   * 而运维吊销一把泄漏的 key 时若绕开面板、直接编辑 `store.json`，
   * 记录没了，可**本实例的快照里它还在**：下一个
   ...
   *
   * **残余风险（同样老实算）**：KV 的 `get` 走边缘缓存（默认 60 秒，§17），
  ...
  private async stillExists(id: string): Promise<boolean> {
    ...
    // 记录确实没了，而本实例 的快照里还留着它——不失效的话它会继续被选中整整
  ...
  /** 写穿透：同一进程内下一次 all() 立刻看到刚写下去的状态，不必等 TTL。 */
  ...
  async add(key: string): Promise<KeyRecord> {
    ...
    try {
      ...
      // **真实可见上界是转发进程自己的一个 `POOL_CACHE_TTL_MS`**
      //（多容器共卷部署下还要 × 每个副本各自的 TTL）。为什么不去改接线：见 wire.ts
   ...
   *     **整池每一把都被当成新 key 覆盖一遍**，那个后门从另一扇门原样回来；
   ...
   *   · `addedAt`——设计文档把「`addedAt` 被刷新」列为那条缺陷的一部分，重置状态
   *     不等于抹掉这把 key 的来历；
   *   · `stats`——用量是历史，不是失败态；
   *   · `note`——运维写给自己看的话。
   *
   ...
   *
   * **今天调用它的一共两处**（v0.4.0 从三处变成两处：Worker Cron 那一处随入口一起没了）：
   * · `src/entry/node.ts` —— 定时轮每一轮开头（默认每 30 分钟一次）；
   ...
   * list，60 秒 TTL 下 1,440 次/天/副本（实测）。
   *
