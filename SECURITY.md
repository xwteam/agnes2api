...
your deployment.** Two CI steps enforce it on every push — `scripts/check-no-binary.mjs`
(no tracked file may be binary, because a binary file is a blind spot for the next step) and
`scripts/scan-secrets.sh` (tracked and untracked files are scanned for key-shaped strings and
for `IP:PORT` pairs). A build fails if either finds something. That is the entire claim.
...
Three things are worth knowing before you deploy, and all three are backed by tests rather
