...
> **这两串枚举有完备性判据**（P3e Task 29 复评 RX7 逼出来的：当时往 `js/pure/` 里新加
> 一份文件而不改这里，`docs-parity` 227 格全绿——手抄的清单会静静过期，正是本节存在的理由）。
...
> 没有任何一格看得见它（复评 RX1）；同时主格的下限是硬编码的 `8`，于是把一个板块从
> 三处一致地删掉之后仍然被拦下，报文还说「一个 data-section 都没扫到」——当时扫到了
> 7 个，那句报文是假的（RX8）。现在这张表与 `data-section` 是**双向**比集合：表里多一行、
