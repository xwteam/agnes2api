 ...
 *
 * ⚠️⚠️ **这条不变量是有代价的，写清楚（复评 F1，这是本文件自己立的规矩）**：
 ...
 * ⚠️ 先把那一套说全（复评 F7：上一版把它概括成「最新的一条赢的世代号」，**漏了一半**）——
 ...
 * 那句在本轮已经变成假话，本轮一并改真**：复评 F1 之后那颗「再读一次」**有权抢占**
 ...
 *
 * ── 触屏上「可用 / 不可用」只剩颜色一条线索（**已知不达标，登记 P3e**）────────
 ...
 * **登记给 P3e 统一处理**（做法之一：给徽章加一个不依赖 hover 的可见记号）。
 * ⚠️ 这条写在这里而不是只写在报告里，理由是本任务自己立过的那条：
 * **报告在 `.superpowers/` 不进仓，只写在报告里等于没写。**
 *
 ...
 *（P3d Task 6 定向复评 F2，实测订正）：另一条是 `onShow()` 里的
 ...
 * 「再读一次」可以**抢占**一条挂住的读（复评 F1），于是被抢占的那条必须作废。
 ...
 * 作废判据是上面那个世代号。
 *
 * ⚠️⚠️ **abort 这一半在测试里天然不可观测，写清楚**（复评 F1）：
...
function buildUnavailable() {
  ...
  // ⚠️ **`true` = 抢占**（复评 F1）：这是用户明确表达的意图，它应该赢。
...
async function load(preempt) {
  ...
  try {
    ...
    // ⚠️⚠️ **这一行的安全性架在**三**条守卫上，不是一条**（复评 F2 实测订正：
