 ...
 * ⇒ **本文件里没有任何一条端点路径、没有任何一个协议 id、没有任何一份请求体形状。**
 ...
 *
 * ── 本板块**没有**在飞请求的作废条件，这是「今天不需要」不是「已经守着了」 ────
 * 它只有一条异步链、一次零存储的静态读，而目录本身是静态的 ⇒ 回来晚了的那一份
 * 写进去也不会是过期数据。**哪天这个板块开始读会变的东西，第一件事就是给它加
 * 世代号**（形态照 `admin-ui/js/sec-usage.js` 的 `seq` / `detailSeq`）。
 */
...
async function load() {
  try {
    ...
    // **不留着上一次的数据**：这个板块成功读过一次就不再发请求，所以走到这里
    // 只可能是「从来没成功过」。把它清成 `null` 是为了让下一次 onShow 会重试。
    catalog = null;
  }
  render();
}
