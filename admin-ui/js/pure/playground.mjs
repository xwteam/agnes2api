 ...
 * ③ ~~**样例请求体里那句占位文本**~~ **——这一件已经不在了（后来消掉的）。**
 *    这里曾经留过一个 `PROMPT_SLOT_SAMPLE = "ping"`，登记成「必然的副本」，
 *    理由是「这个目录下禁止 `import`，而它又不在 `GET /admin/api/models` 的响应里」。
 *    **后半句是可以被改掉的，后来就把它改掉了**：真源的 `catalogPayload()` 多了一格
 ...
 *
 * ⚠️ **`samplePrompt` 是整份响应级别的一格，却挂在每一条协议上**。
 ...
 * ⚠️ **「漂了没人会发现」这句一度对**函数体**也成立**（实测：单给网关那份
 ...
 * ——**行尾不归本函数管**。
 *
 * ⚠️⚠️ **上一版这里写的失败形态逐字为假，实测推翻**：原文说「一帧都切不出来
...
export function sseFrames(buffer) {
  ...
  while ((end = frameEnd(rest)) !== null) {
    ...
    for (const line of frame.split("\n")) {
      ...
      // **就地收尾，与网关那份逐字同语义**（同一条评审发现，理由全文在上方那段 ⚠️⚠️）：
...
/**
 * 这个媒体结果能不能**内嵌**进面板。**判据是纯函数，不许目测**（订正）。
 *
...
/**
 * 这个媒体结果能不能**做成一个可以点开的链接**。**裁定的另一半**（评审提出）。
 *
 ...
 *
 * ⚠️ **前端也要判，理由不是重复保险。但那条理由已经换过一次了，别照抄旧的**：
 * 上一版这里写的是「那条 400 的文案说的是『格式非法』，运维看不出为什么」——
 * **这句话今天不成立了**（网关那条 400 现在逐字说出接受什么形状、
 * 并说明这个标识是上游签发的、还把读者指去 API.md 的对应一节；真源侧是
 * `src/core/admin/protocol-catalog.ts` 的 `videoTaskIdShape()`）。
 *
 * **今天成立的理由是这一侧自己的结构，与那条 400 说得清不清无关**，有两处：
 * ① `buildPollRequest()` 是**字符串替换**拼出轮询 URL 的（`split(slot).join(taskId)`，
 *    这一侧不做编码）——一个带 `/` `?` `#` 的标识会改掉这次请求到底打向哪条 URL，
 *    而**那个函数里约束 `taskId` 的就只有这一条判据**（自己数一遍：它一共三条前置判断）。
 * ② `videoTaskIdOf()` 拿它当**槽位过滤器**：某一格的值不是这个形状时接着试表上的下一格
 *    （`{name:"operations/abc123"}` 就是这一档，见下面那段的 ⚠️⚠️）。
 * 拦在这一侧还有个附带好处：不必先打一次网关才知道这条路走不通。
 * **那是好处，不是它今天留在这里的理由**——别再把好处写成理由，上一版就是那么错的。
 */
...
/**
 * 建任务响应里，**本面板认得的**任务标识候选槽。有序，先到先得。
 *
 * ⚠️ **这不是「往下猜」。** 被反对的那件事是「随便找一个过得了形状判据的字符串」——
 * 那样一个 `"queued"` 状态值就会被当成任务标识（它完全过得了那条正则），
 * 于是面板拿着它去轮一个不存在的任务，直到轮满上限。
 * 具名槽表**仍然不猜**：它只是把「我们认得的槽」从 1 个变成一张明写的表，
 * **表上有的才认，表外的一格都不看。**
 *
 * ⚠️ **顺序是语义不是巧合**：`["id"]` 恒为第一条——两格都在的响应给出哪一个由它决定。
 * 这条由 `tests/ui/playground-media.test.ts` 的「非空锚 + 顺序即语义」那一格钉着。
 *
 * ⚠️⚠️ **`name` 不在这张表上，是量过之后拿掉的**（回填时量的；上一版它在表上）。
 * 把它加进来的理由是「长跑作业的资源名」，而那个形态**过不了形状判据**：
 * `videoTaskIdOk()` 不收 `/`（与网关那条路由同判 ⇒ 放行了也只会换来一个 400），
 * 实测 `videoTaskIdOf({name:"operations/abc123"})` 与 `{name:"models/veo-3.0/operations/abc"}`
 * **都是 `null`** ⇒ **它对自己被加进来的那个理由是死的**。
 * 而**能**过判据的 `name` 剩下的是另一族：**运维自己在建任务时传的显示名被上游原样回显**
 *（`{"name":"my_video","status":"queued"}`）。把它当任务标识的后果实测是屏幕**一边贴着 404
 * 与 `task not found`、一边说「正在轮询这个任务的结果，已经查过 N 次」**，
 * 并一路轮到上限（60 拍 / 5 分钟，**每一拍都烧一次配额**，全局约束 14）——
 * 而在它进表之前，这一档走的是诚实的「本面板认得的那几格里没有可用的标识」。
 * ⇒ **对理由是死的、对显示名却是活的，那就不该在表上。** 加回去当场红，
 * **直接冲着这件事来的是这两格**：`tests/ui/playground-media.test.ts` 的
 * 「建任务响应里的任务标识：只认明写的那几格，其余一律不猜」反向那一组，
 * 与 `tests/ui/dom/playground-section.test.ts` 的
 * 「上游把运维传的显示名回显在 name 里时一次都不轮 —— 拿显示名去轮只会一边 404 一边说「正在轮询」」。
 *（实测同一条变异还会连带打红屏幕那串与「没有标识」那一档几格——那是台账跟着动，
 *  不是这条的执行机构，所以这里不记它们的条数。）
 *
 * ⚠️⚠️ **依据强度：假设，不是核实过。** 关于上游究竟把标识放哪一格，本仓**没有任何
 * 真上游依据**——唯一出处是契约测试里我们自己写的那份假上游夹具，拿它当依据是循环取证
 *（上一版这段注释逐字写着「上游给的正是这一格」，那句话就是这么来的）。
 * ⇒ 这张表是「**我们认得几格**」的登记，不是「**上游有几格**」的断言；
 * 屏幕上那句话（`pg.media.noTaskId`）也照同一个口径写成了限定句。
 * ⚠️ **这条依据强度今天有机器管着了**（上一版这里写的是「没有任何机器在管……登记为
 * 已知盲点」，那张表已经落地，这段跟着改）：它登记在 `src/core/admin/upstream-facts.ts`
 * 的 `UPSTREAM_FACTS` 里，id 是 `video.taskIdSlot`、状态 `assumed`、依据一栏逐字写着
 * 那份假上游。`tests/unit/admin/upstream-facts.test.ts` 的「真表逐条过判据」
 * 与「真表逐条的锚都在它说的那个文件里」两格钉着它：前者不许有人在没有真上游依据时
 * 把它翻成 `verified`，后者把下面那个名字与那条登记绑在一起——**名字被删掉或改名，那一格当场红**。
 * ⚠️ 屏幕上那句限定之外，**五份 API.md 的建任务那一小节里也各写着一句「未经真实上游核实」**，
 * 由 `tests/unit/docs-parity.test.ts` 的「该红时红：某一种语言的限定句被删掉（其余四份不动）」
 * 逐语言、逐小节钉着。
 * ⚠️ **那张表管的仍然只是「这条事实的依据强度没被人偷偷改掉」，不是「上游到底放哪一格」**：
 * 后者只有一次真上游能定案，本仓至今**零非自造依据**。
 */
export const VIDEO_TASK_ID_SLOTS = [
 ...
 * 槽表渲染成给运维看的那一串。
 *
 * ⚠️ **它是屏幕上那句限定句唯一的数据源**：`pg.media.noTaskId` 说的是「本面板只认这几格」，
 * 而「这几格」必须**从表里现渲染**，不许在字典里手抄一份——手抄的那份会在下一次有人
 * 往表里加一格时静静变假，而那恰好是这句话唯一要说清的东西。
 * 由 `tests/ui/playground-media.test.ts` 的「槽表渲染成给运维看的那一串」那一格钉着
 *（那边的期望值是手写整条字面量，不从这张表推）。
 *
 * ⚠️ **手写字面量钉的是内容，钉不住「派生」这层关系**（回填时实测）：
 * 把这个函数体换成手抄的字面量、**同时**往表里加一格，那条比对照样全绿
 *（手抄的那份逐字等于旧期望值），而此刻面板读 N+1 格、屏幕只列 N 格——
 * 正是上面那段说要防的「静静变假」。⇒ 同一格里另配了一条
 * `videoTaskIdSlotsText().split(" / ").length === VIDEO_TASK_ID_SLOTS.length`，
 * 它钉的就是这层派生关系，与那条字面量比对**不重叠**。
 */
export function videoTaskIdSlotsText() {
  return VIDEO_TASK_ID_SLOTS.map((path) => path.join(".")).join(" / ");
}

/**
 * 建任务那次的响应里，可以拿去轮询的那个任务标识。**取不到就是 `null`。**
 *
 * ⚠️ **只认 `VIDEO_TASK_ID_SLOTS` 上那几格，取到的值还要过形状判据**
 *（`videoTaskIdOk()`，与网关那条正则同判）。**表外的一格都不看**：
 * 往下猜的话（比如「随便找一个过得了形状判据的字符串」）一个 `"queued"` 状态值
 * 就会被当成任务标识——它完全过得了那条正则——于是面板会拿着它去轮一个不存在的任务，
 * 直到轮满上限。
 *
 * ⚠️ **某一格在、但它过不了形状判据时，接着试表上的下一格**，不是就此收手：
 * 那不叫「退而求其次去找别的字段」，下一格同样是明写在表上的。
 * ⇒ 整张表都取不到时才明说「本面板认得的那几格这次都没有可用的标识」，并把原文摆出来。
 */
export function videoTaskIdOf(body) {
  const o = obj(body);
  if (o === null) return null;
  for (const path of VIDEO_TASK_ID_SLOTS) {
    let cur = o;
    for (const seg of path) {
      cur = obj(cur) === null ? null : obj(cur)[seg];
      if (cur === undefined) { cur = null; break; }
    }
    if (videoTaskIdOk(cur)) return cur;
  }
  return null;
}
 ...
 * 两条同时判，**先到哪条算哪条**。
 *
 * ⚠️ **今天这三个值让两条上限在稳态下数学等价，如实登记（评审发现）**：
 ...
 * 屏幕上最多保留几轮对话。
 *
 * ⚠️⚠️ **为什么必须有上限**：`turns` 早先只进不出（板块文件里只有 push、
 ...
 * 早先那一轮只拿掉了放大它的那个 60 倍乘数，**没有动无上界这件事本身**。
 *
 ...
 *   ⚠️ **说的是把 `live` 与 `done` 成对改成等价形态**，只改 `live` 一半的话
 *   ⑤ 当场红——完整措辞见 `trimTurns()` 上方那条），
 ...
 * ⚠️ **「删掉它」这句话要说准，上一版的简写会把人读偏**（复评时两条都是实测）：
 * · 把 `live` 与 `done` **一起**改成等价形态（`live = []` **并且** `done = turns`，
 *   也就是「留最后 max 个」那种写法）⇒ DOM 那一族**一格都不红**，红的只有下面点名的那一格；
 * · **只**把 `live` 改成 `[]`、不动 `done` ⇒ 那是**不等价改写**（pending 那一轮会被整条丢掉），
 *   DOM 那一族的 ⑤ **当场红**。
 ...
 *
 * ⚠️ **非流式那一档仍然展示响应体原文，这不是遗留、是选择。** 这里一度把它记成
 * 「等真源加了『回答文本在哪』那一格再改成对话气泡」，而后来加的那一格
