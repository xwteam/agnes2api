...
/**
 * 危险区（屏幕上的第 4 张卡）上的那几颗按钮。**顺序即渲染顺序**，也是五份 ADMIN.md
 * 危险区那张表的行序。
 *
...
export function buildPatch(raw, body, touched) {
  ...
  for (const path of Object.keys(raw)) {
    ...
    // 而五语言 `set.loadBlocked.fatal` 正写着「改完保存即可恢复」。
    //
 ...
 * | 通道 key，env 里没有，且注册机开着 + 这条通道就是本次选中的那条 | **注册机装不起来、补池停摆**（网关转发不受影响） | danger |
 * | 通道 key，env 里没有，且注册机关着或这条通道不是选中的那条 | 现在不影响任何东西；**改用这条通道之前必须重新填** | info |
 *
 * ⚠️⚠️ **第三行的后果被本轮订正过，别照旧读成「下一次冷启动会失败」。**
 * `src/core/registrar/config.ts` 的 `creds()` 今天**只产 blocker、不抛**
 *（那一轮的整条发现就是「可选子系统装不起来不该让整个网关死掉」），
 * 于是清掉一把通道凭据之后网关照常起得来、转发照常——**停的只有补池**，
 * 池子慢慢耗干，几小时到几天后才以 `pool_empty` 炸出来。
 * 拿砖机的措辞去说一件不砖机的事，会让运维在一次「补池停了」的小事故上按大事故处置；
 * 反过来说成 info 又会让一次安静的功能缺失没人去看 —— 所以它仍然是 danger，
 * 但**说的是真话**。同一份口径在 `src/http/admin/handlers/config.ts` 的
 * `config.secret_cleared`（那条 msg 分两档说）与五语言 REGISTRAR.md 的「三态」一节。
 * 第四行也不是轻描淡写——那时它真的什么都不影响。
 *
