 ...
 * 而那一格**当场变红**（`板块引用了字典里没有的 key: [ 'usage.titel' ]`）。
 * ⭐ 记一条形状：**「某道门禁看不见它」是一句关于那道门禁的话，
 * 不是一句关于「有没有护栏」的话**；把前者写成后者，下一个人会以为这里可以随便写。
 *
 * ⚠️⚠️ **上一版这里写过「新板块想跳过它是做不到的」——那句全称命题实测为假，
 * 订正如下（P3d Task 5 定向复评 I2）。** 只要把引号从双引号换成**单引号**
 *（`elI18n("h2", 'usage.titel')`），**三道 i18n 护栏一起哑**：
 ...
 * 这里只负责如实写出来：**本板块的 key 一律双引号**，换成单引号就没有任何东西在守。
 *
 * 它仍然守不住的：动态拼出来的 key、单引号写法（见上）、
 * 以及命名空间本身写成字典里没有的前缀。那几类靠评审 + 人工冒烟。
 * ⚠️ **「只写在 `.html` 里而拼错的 `data-i18n`」不在这张清单里——上一版把它列进来了，
 * 而那是假的**：实测把 `admin-ui/index.html` 的 `data-i18n="nav.usage"` 改成
 * `nav.usagee` ⇒ `[check-i18n] ❌ 引用了字典里没有的 key: nav.usagee`，**exit=1**。
