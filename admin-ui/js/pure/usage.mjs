 ...
 *
 * ⚠️ **`30d` 刻意不是默认档**（待验证清单里至今没了结的一条）：它一次要发
 * `30 × USAGE_SLOTS` 次子请求，而 Cloudflare 的两页官方文档在「一次调用能发
 * 多少次子请求」上互相对不上（Workers limits 页免费档 50，KV limits 页 1,000，
 * 而前者没有定义 KV 算哪一行）。**这里、板块文案、以及任何一处注释都不许写成
 * 「那些子请求是安全的」。**
 * ⚠️ **真机冒烟已经量过它的一半**：`scripts/smoke-dual-runtime.sh` 的 ④ 那一格
 * 在真 workerd 上打了一次这一档，**本次实测跑得完**（回的是 30 段 days，
 * 不是「读不出来」那条路）。**但那是 `wrangler dev` 的本地 workerd，不是线上边缘**，
 * 而官方那两页的口径今天仍然对不上 ⇒ 「线上会不会超」仍然没有答案。
 * 能写下来的仍然只有一句：把它做成默认档等于让每一个打开面板的人都先付一次那个赌注。
 */
 ...
 * `scripts/check-i18n.mjs` 这道门禁当场 exit 1 并点名一批**正在用**的 key，而顺着那条报文去
 * 「清理未被引用的 key」删掉的就是活文案（门禁为此在报文里带了处置指引）。
 *
 * ── 这几个 key 今天有几层网：重量过一次（复评发现）─────────────────────────────
 * **第一层：`scripts/check-i18n.mjs` 这道门禁自己就认得它们。** 上一版写着「那道门禁只认双引号的
 * `t(…)` 与 `data-i18n=`，本仓主流的 `elI18n(tag, key, attrs)` 它一个都看不见」
 * ——那句话已经作废：第 ① 条换成了「命名空间前缀锚定的引号对」广扫
 ...
 * 那是当时的实测，今天两句都是假的**，照旧读会以为这里可以随便写。
 *
 ...
 * ⚠️ **`NS_KEY_RE` 今天已经引号无关**（单双引号都认，反引号仍不认）
 ...
 *
 * ⚠️⚠️ **简报里给的那段示例代码写的正是 `nowMs - days * 86400000`，
 ...
 * 评审实测：上面那句 `all_malformed` 早退**只作用于六张卡**，
 ...
 * 而且是同一次评审点名的「第三屏」**：`all_malformed` 时
 ...
 * 表格里一行该按哪一种状态渲染。
 *
 * ⚠️⚠️ **整块读不出来时每一行都读不出来 —— 这就是那条评审的根因那一句话的落点。**
 ...
 * 是同一句假话，只是挪到了下一屏**（`sec-usage.js` 的 `fillCell` 才是六张卡与日表
 * 唯一的共同下游：六张卡直接调它，日表经 `numberCells` 调它。⚠️ 上一版这里写的是
 * `numberCells`，**那句指错了函数**——`numberCells` 只服务日表与三张分解表，
 * 六张卡压根不经过它，照着去 `numberCells` 里找六张卡是找不到的）。
 * 压成 `"data"` 不改变**有数字那几格**的结局（`cellKind` 在
 ...
 *
 * ⚠️ **登记一条边界：`pending > 0` 那一半，这句 tooltip 比屏幕自己知道的更少。**
 * 这一档下同屏的尾巴会说「还有 N 条计数没有落盘」，也就是**确知有人用过**，
 * 于是新 tooltip 里「是真的没人用」那一支此刻已经不是活选项。**本轮刻意不收**，
 * 三条理由（不是嫌麻烦）：
 * ① 方向是保守的 —— 它不再断言「没有样本」，只说分不出来，比上一版那句严格地更弱，
 *   同一句认知措辞早就在已发货的横幅 `usage.note.noShards` 里，这一轮没有新造；
 * ② `pending.count === 0` 的另一半仍然需要这句话（`pending` 只反映服务那一次请求的
 *   isolate，理由写在 `usageState` 上方第五档那一段），把 tooltip 改成断言「有人用过」
 *   会在那一半上造出一句**新的**假话；
 * ③ 要分 pending 两版就得把 `pending` 穿进分档函数，与第五档那条
 *   「`pending` 只做加法，不参与分档」的裁定正面冲突。
 * ⇒ 真要收掉这一档，落点是**横幅那一句**（`usage.note.noShards`）与这句 tooltip 一起改，
 * 且前提是后端先把「这段区间有没有 pending」变成一个**不依赖单个 isolate** 的字段。
 * ⚠️ 另一半「一条分片都还没落盘」在这一档下是**恒真**的，没有说死的成分：
 * `src/http/admin/handlers/usage.ts` 只在 `shards === 0 && malformed === 0`
 * 且区间没被夹过时才发 `no_shards`。
 *
