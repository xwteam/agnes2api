 ...
 * 这是 P3d 核心设计决定（全局约束 15）的落点：四个消费者共用一份真源
 ...
 * 那条对外契约的不实登记给 P4，协议目录按**真实可用性**填。理由全文在
 ...
 * P3d 全局约束 12）：`scripts/check-i18n.mjs` 的第 ④ 条把动态拼出来的 key 一律
 * 看成「未被引用」，而那一条只报警告、从不 exit 1。
 * ⚠️⚠️ **别把上一句读成「所以第 6 道门禁在守这三个 key 的拼写」**：那道门禁只认
 * 两种形态——把 key 当双引号字面量直接写进 `t(…)` 调用，或写进 HTML 的
 * `data-i18n` 系列属性；本仓主流的 `elI18n(tag, key, attrs)` 它一个都看不见。
 * 真正守着这三个 key 的是 `tests/unit/i18n-dict.test.ts` 的
 * 「板块里当参数传的 i18n key（elI18n / labelKey 这类）同样必须在字典里」那一格，
 * **且前提是 `models` 这个命名空间已经进了它的 `NAMESPACES` 表**（本任务加的）。
 *
