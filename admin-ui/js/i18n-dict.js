...
export const I18N = {
  ...
  "reg.backoff.cluster": { "zh-CN": "这一轮补池里「域名被屏蔽」的判定成了片（要么同一轮里好几个域名一起被判成「被屏蔽」，要么上游列出来的域名这一轮一个不落全被判成「被屏蔽」），而这一轮一把 key 都没铸出来；按这个形状记了一个退避窗口，还要等 {left}（约 {at} 恢复）。⚠️ 上游的回话里没有一句我们认得出的限流的话——这是我们按这个形状下的判定，不是上游给的结论。两种可能都还开着：上游换了限流的措辞、我们的词表没认出来；或者上游真的成批换了黑名单。先去事件里翻那几条 registrar.domain_blocked 带的上游原话，看清是哪种，再决定调大 MINT_DELAY_MIN_MS 还是换出口。换一条邮箱通道逃不掉这一档——限流是按出口地址算的，与用哪条通道无关。", "zh-TW": "這一輪補池裡「網域被屏蔽」的判定成了片（要麼同一輪裡好幾個網域一起被判成「被屏蔽」，要麼上游列出來的網域這一輪一個不落全被判成「被屏蔽」），而這一輪一把 key 都沒鑄出來；按這個形狀記了一個退避視窗，還要等 {left}（約 {at} 恢復）。⚠️ 上游的回話裡沒有一句我們認得出的限流的話——這是我們按這個形狀下的判定，不是上游給的結論。兩種可能都還開著：上游換了限流的措辭、我們的詞表沒認出來；或者上游真的成批換了黑名單。先去事件裡翻那幾條 registrar.domain_blocked 帶的上游原話，看清是哪種，再決定調大 MINT_DELAY_MIN_MS 還是換出口。換一條郵箱通道逃不掉這一檔——限流是按出口位址算的，與用哪條通道無關。", en: "«Domain blocked» verdicts arrived in a cluster during this refill round — either several domains were ruled «blocked» within the same round, or every domain the upstream lists was ruled «blocked» in that round — and the round minted no keys at all; a backoff window was recorded on the strength of that shape; {left} left (recovers around {at}). ⚠️ Nothing in the upstream's replies matched our rate-limit word list — this is a verdict we drew from that shape, not a verdict the upstream handed us. Both readings are still open: the upstream reworded its rate-limit message and our word list missed it, or the upstream really has swapped its blocklist. Read the upstream wording carried by the registrar.domain_blocked events first to tell the two apart, then decide between raising MINT_DELAY_MIN_MS and changing egress. Switching mailbox channel does not help: the limit is counted per egress address, not per channel.", ja: "この補充ラウンドでは「ドメインがブロックされた」という判定がまとまって出ました（同じラウンドで複数のドメインが「ブロック」と判定されたか、上流が列挙するドメインが 1 つ残らず「ブロック」と判定されたかのどちらかです）。しかもそのラウンドでは key が 1 本も発行できませんでした。この形を根拠にバックオフ期間を記録しました。残り {left}（{at} 頃に回復）。⚠️ 上流の返答には当方の語句リストに載っているレート制限の文言がありませんでした——これはその形から当方が下した判定であり、上流が下した結論ではありません。可能性は 2 つとも残っています。上流がレート制限の文言を変えて当方の語句リストが取りこぼした場合と、上流が本当にブロックリストを入れ替えた場合です。まず registrar.domain_blocked イベントに載っている上流の原文を確認してどちらかを見分け、そのうえで MINT_DELAY_MIN_MS を大きくするか送信元を変更するかを決めてください。メールボックスチャネルを切り替えても回避できません——制限は送信元アドレス単位で、チャネルとは無関係です。", ko: "이번 보충 라운드에서 «도메인 차단» 판정이 무더기로 나왔고(같은 라운드에서 여러 도메인이 «차단»으로 판정되었거나, 업스트림이 제공하는 도메인이 하나도 빠짐없이 «차단»으로 판정되었습니다), 그 라운드에서는 key가 하나도 발급되지 않았습니다. 이 형태를 근거로 백오프 구간을 기록했습니다. {left} 남음({at}쯤 회복). ⚠️ 업스트림 응답에서 우리 단어 목록에 있는 속도 제한 문구를 찾지 못했습니다 — 이것은 그 형태를 보고 우리가 내린 판정이며, 업스트림이 내려준 결론이 아닙니다. 두 가지 가능성이 모두 남아 있습니다. 업스트림이 속도 제한 문구를 바꿔 우리 단어 목록이 놓쳤을 수도 있고, 업스트림이 실제로 차단 목록을 갈아치웠을 수도 있습니다. 먼저 registrar.domain_blocked 이벤트에 실린 업스트림 원문을 확인해 둘을 구분한 뒤, MINT_DELAY_MIN_MS를 늘릴지 발신 주소를 바꿀지 결정하세요. 메일박스 채널을 바꿔도 피할 수 없습니다 — 제한은 발신 주소 기준이며 채널과 무관합니다." },
