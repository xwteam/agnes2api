...
export const I18N = {
  ...
  // 而 cluster 那一档**上游一个限流字眼都没说**，先要去看的是事件里那几条上游原话。
  // 三条都要说「换邮箱通道逃不掉」——限流在「出口 IP → 上游」这条边上，与通道无关。
  //
  // 🔴 **`reg.backoff.cluster` 是新的第三档，它的证据类型与前两档不同**：前两档来自
  // 上游回话里的字眼，它来自**这一轮的形状**（同一轮里 ≥2 个域名被判屏蔽 + 这一轮
  // 一把 key 都没铸出来，见 `src/core/registrar/tender.ts` 的 `finishRound`）。
  // ⇒ 它的文案里那句归属**不能照抄 app 那条**（那句说的是「按词表认出来的」，
  // 而这一档一个词表都没用上），必须说清「上游一句限流的话都没说」。
  // 由 `tests/unit/i18n-dict.test.ts` 的
  // 「cluster 那条退避文案先说清上游什么都没说，再不许把换出口说成唯一出路（五语言各一格）」钉着。
  //
  ...
  "reg.backoff.cluster": { "zh-CN": "同一轮补池里有好几个邮箱域名同时被判成「被屏蔽」，而这一轮一把 key 都没铸出来；按这个形状记了一个退避窗口，还要等 {left}（约 {at} 恢复）。⚠️ 上游一句限流的话都没说——这是我们按「一轮里好几个域名一起挂」这个形状下的判定，不是上游给的结论；上游真的成批换了黑名单时它也会这么说。先去事件里翻那几条 registrar.domain_blocked 带的上游原话，看清是哪种，再决定调大 MINT_DELAY_MIN_MS 还是换出口。换一条邮箱通道逃不掉这一档——限流是按出口地址算的，与用哪条通道无关。", "zh-TW": "同一輪補池裡有好幾個郵箱網域同時被判成「被屏蔽」，而這一輪一把 key 都沒鑄出來；按這個形狀記了一個退避視窗，還要等 {left}（約 {at} 恢復）。⚠️ 上游一句限流的話都沒說——這是我們按「一輪裡好幾個網域一起掛」這個形狀下的判定，不是上游給的結論；上游真的成批換了黑名單時它也會這麼說。先去事件裡翻那幾條 registrar.domain_blocked 帶的上游原話，看清是哪種，再決定調大 MINT_DELAY_MIN_MS 還是換出口。換一條郵箱通道逃不掉這一檔——限流是按出口位址算的，與用哪條通道無關。", en: "Several mailbox domains were ruled «blocked» within the same refill round, and that round minted no keys at all; a backoff window was recorded on the strength of that shape; {left} left (recovers around {at}). ⚠️ The upstream said nothing about rate limiting — this is a verdict we drew from the shape «several domains failing together in one round», not a verdict the upstream handed us; it reads the same when the upstream really has swapped its blocklist. Read the upstream wording carried by the registrar.domain_blocked events first to tell the two apart, then decide between raising MINT_DELAY_MIN_MS and changing egress. Switching mailbox channel does not help: the limit is counted per egress address, not per channel.", ja: "同じ補充ラウンドで複数のメールボックスドメインが同時に「ブロック」と判定され、そのラウンドでは key が 1 本も発行できませんでした。この形を根拠にバックオフ期間を記録しました。残り {left}（{at} 頃に回復）。⚠️ 上流はレート制限について一言も述べていません——これは「1 ラウンドで複数のドメインがまとめて落ちる」という形から当方が下した判定であり、上流が下した結論ではありません。上流が本当にブロックリストを入れ替えたときも同じ表示になります。まず registrar.domain_blocked イベントに載っている上流の原文を確認してどちらかを見分け、そのうえで MINT_DELAY_MIN_MS を大きくするか送信元を変更するかを決めてください。メールボックスチャネルを切り替えても回避できません——制限は送信元アドレス単位で、チャネルとは無関係です。", ko: "같은 보충 라운드에서 여러 메일박스 도메인이 동시에 «차단»으로 판정되었고, 그 라운드에서는 key가 하나도 발급되지 않았습니다. 이 형태를 근거로 백오프 구간을 기록했습니다. {left} 남음({at}쯤 회복). ⚠️ 업스트림은 속도 제한에 대해 한마디도 하지 않았습니다 — 이것은 «한 라운드에서 여러 도메인이 한꺼번에 실패»라는 형태를 보고 우리가 내린 판정이며, 업스트림이 내려준 결론이 아닙니다. 업스트림이 실제로 차단 목록을 갈아치웠을 때도 똑같이 표시됩니다. 먼저 registrar.domain_blocked 이벤트에 실린 업스트림 원문을 확인해 둘을 구분한 뒤, MINT_DELAY_MIN_MS를 늘릴지 발신 주소를 바꿀지 결정하세요. 메일박스 채널을 바꿔도 피할 수 없습니다 — 제한은 발신 주소 기준이며 채널과 무관합니다." },
