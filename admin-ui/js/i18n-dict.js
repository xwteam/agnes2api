...
export const I18N = {
  ...
  "pg.turn.noTokens":        { "zh-CN": "流式响应不带 token 用量，所以这里不显示数字——显示 0 会是假的。", "zh-TW": "串流回應不帶 token 用量，所以這裡不顯示數字——顯示 0 會是假的。", en: "Streaming responses carry no token usage, so no number is shown here — showing 0 would be false.", ja: "ストリーミング応答には token 使用量が含まれないため、ここには数値を表示しません。0 と表示するのは誤りになります。", ko: "스트리밍 응답에는 token 사용량이 없으므로 여기에 숫자를 표시하지 않습니다. 0으로 표시하면 거짓이 됩니다." },
  ...
  "pg.media.noTaskId":       { "zh-CN": "建任务的响应里没有一格能当任务标识用的 id，面板不替你猜，所以这次没有自动轮询。", "zh-TW": "建任務的回應裡沒有一格能當任務識別碼用的 id，面板不替你猜，所以這次沒有自動輪詢。", en: "The create-task response carried no field usable as a task ID; the panel will not guess one, so nothing was polled.", ja: "タスク作成の応答にタスク ID として使える項目がありませんでした。画面側で推測はしないため、今回はポーリングを行っていません。", ko: "작업 생성 응답에 작업 ID로 쓸 수 있는 항목이 없었습니다. 콘솔은 추측하지 않으므로 이번에는 폴링하지 않았습니다." },
