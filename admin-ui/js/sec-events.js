...
function renderWarnings() {
  ...
  // 不是"持续亮着的告警"。**这一行的"粘不粘"本身仍然没有自动化守护**
  //（实测把它改成粘性全套用例照样全绿）。
  //
  // ⚠️ **这里原来写的理由是「本仓无 DOM 测试通道」——那句话早就不成立了**：
  // `tests/ui/dom/` 有 harness + 五组用例，`events-poll.test.ts` 就在真点导航
  // 按钮、真驱动本板块，本任务还用它钉住了 `warnCursorBroken` 的接线
  //（「响应体缺 cursor 字段时，契约破坏那条黄条真的显示出来」）。
  // 真实的缺口只剩**"逐轮覆盖 vs 粘性"这条时序性质**——那要一组跨轮的 DOM 断言，
  // 不是"没有通道"。**登记为已知盲点，不许再用那个已经作废的理由搪塞过去。**
