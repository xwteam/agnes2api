 ...
 * 「新增一个会写存储的代码路径」，而那一轮一条都没新增——它只是让**已有**的
 * 那条路径被按得更频，所以当时这笔账只登记在这里）。
 * ⚠️ **它今天已经进了五语言 DEPLOY.md 的配额账**（Playground 那一条），
 ...
 * ⚠️ **再上一版写着「这条没有进 DEPLOY.md 的配额账」，那句话后来就变假了，
 * 已随那一轮一并改真。** 下面那两条仍以本处为准。
 ...
 *
 * ── 流式：开关已经接上，而且两种运行时都是真的逐块 ────────────────────────────
 * 这个开关一度是 `disabled` 的（读流那一半还没写）。**现在它是真的。**
 ...
 * 判据是「**正文首块与末块的到达间隔**」不小于门槛（门槛 = (块数−1) × 间隔 ÷ 2，
 * 今天算出来是 1500ms），外加一条佐证「正文首块到达 早于 上游末块发出」。
 * 实测（2026-08-29 北京时间，Docker 与 wrangler dev 起的真 workerd 各一次）：
 * **两侧的正文都铺开了约 3 秒**（Docker 2984ms / Worker 2991ms，= (4−1) × 1 秒）
 * ⇒ 两种形态都逐块透传。
 * ⚠️ **判据刻意不是「拿到了几块」**：整体缓冲的实现最终也会把全部内容交出来，
 * 只看总量是零鉴别力。这条与明令禁止的「往单块表里加一行 CRLF 样本」是同一条。
 * ⚠️⚠️ **它也刻意不是「第一行到达」**——上一版就是那么写的，而那句话是假的：
 * 这条流的头两行是 `message_start` 与 `content_block_start`，
 * `src/core/protocol/anthropic.ts` 的 `toAnthropicStream()` 里逐字写着它们
 * 「必须在读取上游之前产出」⇒ 它们的到达时刻与「正文有没有逐块流出来」在构造上无关。
 * 实测（把正文增量攒完再一次性吐、preamble 保持早发）：那一版的 ③ **照样全绿**；
 * 换成现在这条判据之后同一个变异下**两侧都红**（正文只铺开 11ms / 17ms，差门槛两个数量级）。
 ...
 * ⇒ 另行登记，理由全文在 `js/pure/playground.mjs` 的 `prettyJson()` 上方。
 *
 * ── 流式那一轮为什么不显示 token 用量（**论证按协议分档，已改写**）────────────────
 ...
 * ⚠️ **而「多画一行就红」这句话，一度也没有任何东西守着**：闭集判据自己
 ...
 *
 * ⚠️ **双运行时：非流式长请求在两种形态下可能表现不同，面板不许承诺它们一样**（待验证清单里那一条的落点）。
 ...
 *
 * ⚠️ **早先那一轮只拿掉了放大它的 60 倍乘数，没有动无上界本身**
 *（通读评审点名的那一条）：视频档一度带着一个**每拍都整版 `render()`** 的 60 拍循环，
 ...
 * 而它当时**是假的**，复评实测证伪）⇒ 重建量回到 `每次运维自己按一下 × 全部轮次`。
 * **本轮封的是后一个因子。**
 *
 ...
 *
 * ⚠️⚠️ **`loadAbort` 是通读评审补的，补之前这里是全面板唯一一个读不可中止的板块。**
 ...
 *
 * ⚠️⚠️ **`rel="noopener noreferrer"` 是显式写的，不是靠浏览器补**（评审提出）：
...
function fillMediaResult(box, turn) {
  ...
  if (turn.pollState === "noTaskId") {
    ...
    // **变异实测**：把下面这两行换回 `elI18n` 那条写法 ⇒ 那道门禁 **exit 0**。
...
function buildRight() {
  ...
  if (trimmedTurns > 0) {
     ...
     * 而他丢掉的正是屏幕上刚刚少掉的那几轮。
     * ⚠️⚠️ **代价明写：标了也未必念得到**（复评补的；上一版只写了「不标一定听不到」那一半，
...
function render() {
  ...
  // ⚠️ **这一行是防御性的，今天走不到**（复评实测：删掉它 2735 全绿）：
 ...
 *
 * ⚠️ **「与 `sec-models.js` 同一条裁定」这句话有边界，本轮补写清楚**（评审点名要求的）：
...
/**
 * 作废目录那条读。**与 `cancelInFlight()` 是两件事，不能合并**（评审点过名）：
 ...
 * 状态码那一格已经修成跟着这一拍走（见下面那段 ⚠️⚠️），**而「还在轮」那句话
 * 至今没有把上一拍的成败说出来**——要改就得给 `pg.media.polling` 那一族加一句
 * 「上一拍回的是 N」，那是五语言 + 渲染 + 判据的一整个任务，**不在这一轮的射程里**。
 */
async function pollOnce() {
  ...
  try {
    ...
    /**
     * ⚠️⚠️ **这一拍只重填正在轮的那一个盒子，不整版 `render()`**（通读评审点名的同一条）。
     *
     ...
     * 单看它便宜——**它与另外两件事相乘才有牙**：
     * ① 媒体两档让 `turn.body` 可能是一张 base64 图（`mediaEmbeddable()` 放行
     ...
     *    ——那是一句假话，复评逐节点 diff 当场证伪**：轮询回 500 时屏幕上那一格
    ...
    ? streamFromGateway(req, token, {
      ...
      onPayload: (payload) => {
        ...
        if (piece === null) {
           ...
           *（第三轮修复的定向复评实测；写在这里免得有人「为了少写一个函数」改回去）：
...
export const playgroundSection = {
  ...
  onHide() {
    ...
    //    ——一句全称句，而目录那一次不在其中（评审实测）：`cancelInFlight()`
