 ...
 *
 * ⚠️ **这段话订正过（出站探测护栏落地之后）**：上一版写着「这条端点**后端没有冷却也没有预算闸**」
 ...
 *
 * **前端这一行仍然要留着，但理由不是「两者不重叠」——那句话是假的**（定向复评）：
 ...
 * 就是去切通道 —— 切完照样被限。
 * 第三档（`cluster`）的证据类型与前两档不同：**我们的词表一个字都没命中**，判据是
 *「这一轮的域名成片被判屏蔽 + 这一轮零产出」这个形状，所以它的文案不能照抄
 * `reg.backoff.app` 那条（`admin-ui/js/pure/registrar.mjs` 的 `backoffView` 逐档分派）。
 * ⚠️ **也不许写成「上游没说过限流」**：那一档要治的头号场景恰恰是上游说了、我们没认出来。
 * 全文与判据见 `admin-ui/js/i18n-dict.js` 那条文案上方那段。
 */
