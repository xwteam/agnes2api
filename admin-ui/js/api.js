 ...
 * 全站一共**四个** `fetch` 调用点，另三个是 `js/app.js` 的登录探针
 *（`probe()`）、`js/gw-api.js`（Playground 打对外那棵树）与 `js/health.js`
 *（顶栏状态徽章的健康探针）。
 ...
 * `health.js` 绕开本模块的理由又不同：`/health` 是**不鉴权**端点，走本模块会给它
 * 带上 `x-admin-key` —— 把口令送去一个不需要它的端点，等于凭空多一条外发路径。
 * 说准这件事很要紧：**本模块正是那个把「唯一出口」当成安全论证前提的地方**
 *（下面「口令只走请求头」那段），顺着假的那句走，审计者会漏掉口令实际离开
 * 页面的另外三条路径。四个出口这件事由 `tests/ui/api-session.test.ts` 的
