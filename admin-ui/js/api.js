 ...
 * 那格数着调用点钉住。
 * ⚠️⚠️ **它会漏，而且是沿两条互不相干的轴各漏一族。别把它读成「所有出网方式」。**
 *
 * **轴一：枚举表**（P3d Task 7 评审 F-11，逐条实测）。表里只有 `fetch` /
 * `XMLHttpRequest` / `EventSource` / `sendBeacon` 四个名字，**`new WebSocket(`、
 * `await import(`、以及把 fetch 先存进变量再调（`const f = globalThis.fetch; f(u)`）
 * 三种，它一个都数不出来**（实测三种各得 0，而 `sendBeacon` / `XMLHttpRequest` 各得 1）。
 *
 * **轴二：它先抠掉模板串的字面文本，而那一步会连真代码一起抠掉**
 *（P3d Task 7 定向复评 H-1，落盘实跑）。最要命的一族是**带花括号的插值**：
 ...
 *
 * ⇒ 准确的说法是：「**照那四种写法写、并且不在带花括号的插值里**，加第三个出口会让它变红。」
 * ⚠️ **上一版只沿轴一收窄，于是收窄之后那句话仍然是假的，而反例就写在同一个提交里
 * 我自己那一段**（本仓第 34 句实测为假的全称句）。⭐ 记一条形状：
 * **收窄一句全称句时，先把它可能失效的轴列全，再逐条验。**
 *
 * 两条边界都在 `tests/ui/api-session.test.ts` 里有会红的登记：轴一目前只写在这段注释里
 *（枚举表登记给 P3d Task 10，Playground 是第一个可能要流式的消费者）；
 * 轴二见那里的「已知抓不住的写法确实抓不住（边界是断言，不是散文）」三条，
 * 与「插值捞不齐的那些行今天恰好一处 —— 这道扫描在那一行上是瞎的」那一格。
