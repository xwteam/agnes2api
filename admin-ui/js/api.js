 ...
 *（`probe()`）与 P3d Task 10 的 `js/gw-api.js`（Playground 打对外那棵树）。
 ...
 *
 * **轴一：枚举表**（P3d Task 7 评审 F-11 发现，**P3d Task 10 补齐**）。
 ...
 * Task 10 把三种一并加进表里，并为每一种各种了一次探针——
 ...
 * ⇒ **P3d Task 10 新写的那个出口因此刻意不写在任何模板串的插值里。**
 *
 * ⇒ 准确的说法是：「**照那七种写法写、并且不在带花括号的插值里**，加第四个出口会让它变红。」
 * ⚠️ **P3d Task 7 那一版只沿轴一收窄，于是收窄之后那句话仍然是假的，而反例就写在
