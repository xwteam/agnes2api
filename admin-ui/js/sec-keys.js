/**
 * Key 池板块。最初只做了只读部分（状态筛选 → 4 张汇总卡 → 分页列表 → 新鲜度提示）；
 * **后来在此之上加了写操作**：行内动作（停用/启用/清冷却/清连续失败/解除剔除/
 * 删除）、批量条（选中才出现）、导入弹窗。
 *
 * ⚠️ **「清连续失败」是控制端追加裁定补的**：设计 §10.2 的行内动作清单本来就有
 * 「清 strikes」，后端 `PATCH` 的 `clearStrikes` 字段也早已实现全，
 * 是本任务简报第一版的动作清单漏列了它。
 *
 * ⚠️ **后来又加了一颗「验活」**，它与上面那六颗**不是同一类东西**：
...
/**
 * 「验活」那一颗按钮 + 它的行内结果。**整段每次 `render()` 重建，
 ...
 * 复评实测 `scripts/check-comment-refs.mjs` 这道门禁在这种位置上**不校验标识符锚**，把它换成一个不存在的名字
 ...
 * 原样投给 ja / en / ko 的面板。**后来把另一半也关掉了**——后端多回一格
 ...
 * 【自动注册】那两项**先占位、禁用、挂一句说明性 tooltip**，留给注册机板块接线
 * 到注册机端点。**容器与两组平级的结构现在定死，到时候只需要把这两颗按钮的
 * `disabled` 摘掉、接上真实的 onClick**，不必回头重构这个工具栏——那正是
 * 「现在不建容器，后面那两个板块就要拿掉一颗按钮重新长出一个菜单」这条风险的解法。
 *
...
function buildAddKeyMenu() {
  ...
  // 要做的话先让夹具支持冒泡，那与之前记下的「替身方案 B」是同一个量级的改动。
