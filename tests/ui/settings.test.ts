 ...
 *
 * ⚠️⚠️ **这一段的机制改过两次，别照上一版读。** 上一版写的是
 * 「那一族今天全部被报成「未被引用」的**警告**，而那道门禁在警告上从不 exit 1」——
 * 两句今天都不成立：
 * · 后来 `scripts/check-i18n.mjs` 把 `` `set.field.${…}` `` 收成一条**拼键前缀**，
 *   那一族因此落进横幅的「拼键覆盖」桶，**根本不进「未被引用」**；
 * · 再后来第 ④ 条是**硬错**，不再有「警告」这一档（`warnings` 数组已删）。
...
describe("建实例时读一次的那两个旋钮", () => {
   ...
   * 复评那条发现就是这么冒出来的：`USAGE_STATS_ENABLED` 那块注释里也写着这句话，
...
describe("i18n：门禁看不见的那两族，在这里补上", () => {
   ...
   * 那条护栏实测是假的**（评审发现，我自己复现过）：`satisfies` 只做**单向可赋值检查**
...
describe("buildPatch：三条规则", () => {
  ...
  /**
   * ⚠️⚠️ **真机冒烟抓出来的两条缺陷之一（「把验收当一等交付物」那条纪律）。**
   *
...
describe("危险区的取值决策", () => {
  ...
  /**
   * ⚠️⚠️ **「读不到 `resetBlocked`」与「`resetBlocked` 是空数组」必须分得开**（复评回填）。
   *
