...
/**
 * **单把 key 的验活入口（P3d Task 9）的 DOM 行为覆盖。**
 * 端点、出站探测护栏、以及「响应体里一个字节上游正文都不回来」那条，是 Task 8
...
describe("在飞态与并发", () => {
  ...
  /**
   * ⚠️⚠️ **验活必须有自己的 `AbortController`，不许复用模块级那个 `abort`**（评审 I18）。
   ...
   * 这不是推的，是量的**：本任务变异 M15 往 `verifyOne()` 开头插了一句
...
// ───────────────────────────────────────────────────────────────────────────
// 只读探针（订正 F8）＋ 明文 key 一个字节都不许出现在这一层
// ───────────────────────────────────────────────────────────────────────────
...
describe("它是只读探针", () => {
   ...
   */
  it("验活成功后这一行的 strikes 与分档徽章一个字都没变 —— 它不许借着「顺手刷新一下」改屏幕上的数（订正 F8）", async () => {
    let listCalls = 0;
    const h = await openKeys((url) => {
    ...
    // **结果只在这一行就地显示，不落盘**（订正 F8）：localStorage 一个键都没多。
   ...
   * 回给面板」（Task 8）；**这一层不许把拿到的东西再暴露出去**——尤其别把请求或
   * 响应整份塞进任何调试出口或 `title`。
   *
