...
/**
 * 注册机板块的**行为**覆盖。纯函数那一半在 `tests/ui/registrar.test.ts` 的
 * 「第 3 条：通道顺序恒为 moemail, yyds（字母序，唯一可辩护的中立规则）」那一族；
 * 这里验的是「板块文件真的把那些判据接上了 DOM」——那一半没有任何纯函数测试
 * 能替代（`errorMessage()` 就是这么逃逸的：退回不调用判据的老实现，
 * 566/566 仍然全绿）。
 *
...
describe("设计 §10.3 第 2、3 条：两张通道卡", () => {
  ...
  it("样式层不许给某一条通道开小灶 —— sections.css 里不许出现按通道名区分的选择器", () => {
    ...
    // `background: url(//cdn…)` 之后的样式整段吃掉。第一版正是这么把这一格
...
describe("立即补池：设计 §10.2 第 3 条护栏（确认弹窗必须明示消耗）", () => {
   ...
   *（设置页那一半）。这一格钉的是同一条纪律在本任务这个下拉上的落点：
   * **初始选中的是占位符，两条通道都不预选。**
   */
...
// ───────────────────────────────────────────────────────────────────────────
// Key 池板块「添加 Key」菜单里的【自动注册】两项（接线）
// ───────────────────────────────────────────────────────────────────────────
