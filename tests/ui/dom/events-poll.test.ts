...
/**
 * **面板行为覆盖目标 ④：事件板块的轮询调度**——它守的是**发货给用户的那条读配额包线**。
 *
...
describe("事件板块的轮询间隔：DEPLOY.md 那条读配额包线的分母", () => {
  ...
  /**
   * **「清空」按钮之后列表区那句话必须说真话**（评审发现的**行为形态**）。
   *
   * ⚠️ **这一格是自查补的：面板行为覆盖第一轮做完之后拿这条变异实测，它完整逃逸。**
 ...
 *
 * ⚠️ **这一组是第三轮补的，成因如实登记**：`shouldWarn` 收下 `cursorBroken`（评审发现）
...
describe("事件板块的黄条：后端字段 → 真的显示出来", () => {
   ...
   * 这正是那条畸形事件条目实测到的响应体形状：`c.json` 把 `undefined` 整个丢掉。
