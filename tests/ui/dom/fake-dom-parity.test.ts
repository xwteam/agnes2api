 ...
 * ——名字本身没有问题，问题在语义。
 *
 * ⚠️ **但"抓不到"不等于"没法抓"**（评审发现 m3）：前两个的**具体危险写法**
...
const FAKE_ONLY_MEMBERS: Array<{ pattern: RegExp; label: string; why: string; fixtureName?: string }> = [
  ...
  /**
   * ⚠️⚠️ **下面两条是评审发现 m3 补的，形态与上面六条不同：名字是真的，
   * 被当成假的用的是它的返回值类型。**
   *
   * 账本在复评时把这两个列为「九个残余里最危险的两个，因为写法最自然」，
 ...
 *
 * ⚠️ **抠注释走 `tests/helpers/strip-comments.ts` 转出去的真源（收编过来的那一份）。**
...
/* ─────────────────────────────────────────────────────────────────────────────
 * **settle() 抽干的是整条队列，不是数着 tick**（复评顺手抓到的）。
 *
