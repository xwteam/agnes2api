 ...
 * 不是真扫描能看见。
 *
 * ⚠️⚠️ **射程从 `.pg-turn` 子树放宽到整个右栏，是复评实测逼出来的（回填 F3）。**
 ...
 *
 * ⚠️ **它被下面每一张闭集表共用**（回填 F5）：这几张表原来是**各抄一份**的，
...
describe("左栏：档位与模型全部来自协议目录", () => {
   ...
   * 不是另拼的一份）。
   *
   * **变红条件（逐条实测；①②③ 见 progress note 的 M8/M9/M10，④ 是回填时补的）**：
  ...
  /**
   * **`aria-pressed` 的值真的跟着点击走 —— 协议那一排**（复评 F6 补的那一组）。
   *
...
describe("Playground 板块：流式", () => {
  ...
  it("流式那一轮不显示任何 token 数字 —— Anthropic 的流里带着一个恒为 0 的 usage", async () => {
     ...
     * ⚠️ **射程是右栏那整张卡片，不是 `.pg-turn` 子树**（回填 F3：只扫盒子里的话，
   ...
   *（回填 F3 补的），两条各挂各的位置，别拿其中一条当另一条的证明。
   *
  ...
  it("pg.turn.noTokens 五种语言一个 {占位符} 都不许有 —— 它是 elI18n 裸标签用法，规则 ⑧ 对这种调用点结构性地看不见", () => {
    ...
    //
    // ⚠️ **报文分两句写，别只写「判据瞎了」**（回填 F4）：复评正当地给
