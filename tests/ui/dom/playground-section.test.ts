...
// `useRealTimers()` 是媒体那一轮加的：视频轮询那几格装假定时器，忘了收就会把
// 后面每一格的 `setTimeout` 一起冻住（而症状是「某一格莫名超时」，不指向这里）。
afterEach(() => { vi.useRealTimers(); vi.unstubAllGlobals(); vi.restoreAllMocks(); });

// **与 `harness.ts` 的那一份保持同形**：`raw` 是为流式加的
...
function respondWith(opts: {
   ...
   *
   * ⚠️ **它收得到 `url`（媒体那一轮加的）**：视频是两段式，建任务与轮询打的是
 ...
 * 不是真扫描能看见。
 *
 * ⚠️⚠️ **射程从 `.pg-turn` 子树放宽到整个右栏，是复评实测逼出来的（回填）。**
 ...
 * 这是「口令扫描面」那段的 ⭐（**写下的覆盖面小于宣称的范围**）在本文件里的第三次发作。
 ...
 * · **左栏**（请求表单那张卡片）不在射程里；
 * · **板块之外**（`#toast-host`、登录闸、侧栏）不在射程里 —— 那一层归「口令扫描面」那段的整页扫描，
 *   但它扫的是口令、不是「多画了一行」。
 ...
 *
 * ⚠️ **它被下面每一张闭集表共用**（回填）：这几张表原来是**各抄一份**的，
...
/**
 * **取消那一轮跑完之后，整个右栏的闭集**（闭集断言同一条纪律，取消这一档同样不许多画）。
 *
 ...
 * **实测（复评那两条各自跑过）**：把 `js/app.js` 那句「当前板块只跑 `onShow()`」改成「不重建」
 * ⇒ **用到这份装置的每一格**都红在「前置条件塌了」上；
 * 改成「重建但顺带跑 `onHide()`」⇒ 同样每一格都红，红在输出 diff 上。
 * 两种退化各有一条红线接着。
 * ⚠️ **这里刻意不写总格数**（与 `strip-comments.ts` 同一条）：
 ...
 * 而被质疑的一直是就地那条快路。反过来写的话（复评抓到的上一版）：把 `onPayload`
...
describe("左栏：档位与模型全部来自协议目录", () => {
   ...
   * ── **另两档不再是摆设** ─────────────────────────────────────────────────────
   *
   * 早先图片 / 视频两档是 `disabled` 的（媒体那一半还没写），那一格断言的是
    ...
    const h = await openPg(respondWith({
      catalog: {
        ...
        body: {
          ...
          // 这一格是媒体那一轮加的。**空数组不是「省事」**：这份合成目录里一条媒体
  ...
  /**
   * **`aria-pressed` 的值真的跟着点击走 —— 协议那一排**（复评补的那一组）。
   *
 ...
 * ⚠️ 走手写目录的代价同仓刚付过一次：那等于把刚从设置页拆掉的抄件风险
 * 原样搬到 Playground 来。
 *
 * ── **为什么改名之后这份目录还读得进来** ───────────────────────────────────────
...
describe("形态名一漂，媒体那一档的三处兜底文案逐条上屏", () => {
   ...
   * 少一次 `await settle()` 这一格就退化成一条**对时序敏感**的断言——本仓刚因
  ...
  it("端点行 0（真源为 1）、模型下拉 0 项（真源为 2）、发送按钮停用且两句文案逐字上屏", async () => {
    ...
    // （回填补上；紧邻的上一个提交 `1c3ea70` 修的是同一个形态的毛病。）
...
describe("网关口令：粘贴、就地校验、绝不外泄", () => {
  ...
  it("面板上任何一处都不出现网关口令 —— 输入框的值不许漏进标题、属性或任何一句错误文案", async () => {
    ...
    // ── 档 ⑤：**流式中途断掉 + 带畸形块**（评审发现）。档④只走了流式的**成功**那条路
    ...
    // ── 档 ⑦：**媒体** —— **不在这一格里**，在下面那一格
    //    「媒体那几个渲染函数里这道判据认得出的每一个出口，都被口令扫描跑过 —— 覆盖面按出口数算，不按用例数算；认不出的那几条射程写在 mediaOutputsInSource() 上方（评审发现）」。
    //    ⚠️⚠️ **它为什么被搬出去，是那条评审发现的落点，写清楚**：媒体那条渲染路径
  ...
  /**
   * ── **`/config` 失败一次之后，hint 校验不许在整个会话里永远停在「比不了」** ──────
   *
...
describe("Playground 板块：流式", () => {
  ...
  it("流式那一轮不显示任何 token 数字 —— Anthropic 的流里带着一个恒为 0 的 usage", async () => {
     ...
     * ⚠️ **射程是右栏那整张卡片，不是 `.pg-turn` 子树**（回填：只扫盒子里的话，
   ...
   * 红的正是这一格与下面那条盒子外的兄弟格）。
   *
   * **做法**：手工往那一轮的外框上多挂一个 `p.pg-tokens`——这正是评审当年那条
  ...
  /**
   * ⚠️⚠️ **评审发现：这一档原来同时说了两句互相矛盾的话，而其中一句是编的。**
   *
  ...
  /**
   * ⚠️⚠️ **评审发现：取消之后 `turn.pending` 永不回落。**
   *
  ...
  it("取消一条一个字都没到的流：明说是被取消的 —— 既不许留白，也不许谎称「读完了但没有正文」", async () => {
    ...
    // ③ 形状闭集：取消那一轮同样不许多画任何一行（闭集断言同一条纪律）。
    // ⚠️ **期望表不再就地手抄**（回填）：它现在与流式那一格共用
    // `RIGHT_FRAME_SHAPE` / `TURN_HEAD_SHAPE`，只把两处真差别留成字面量。
    // 上一版是整张十行的第二份手抄，而「分两份抄 ⇒ 改动之后只有一份跟着改」正是
    // 流式那张表自己写下的理由。同轮补上报文：变异实测它红的时候只有一串裸 diff。
   ...
   * **整块重画之后开关还得是开着的。**
   *
   * ⚠️ **这一格是「先把那一行删一次试试」立出来的**（交接的那条纪律）：
   ...
   * `signal`（交接第 3 条 / 登记的边界一），所以 `abort()` 本身在这里
 ...
 * ── **第二轮修复定向复评：流式在途那一拍** ────────────────────────────────────
 *
 * **它防住的真实故障（复评实测，不是设想）**：`onPayload` 每来一块读不出来的
 ...
 * 「**静默丢弃就是撒谎**」正是这个文件自己写的。
 *
 * ⚠️ **它与轮询那次（复评发现，`.pg-status` 编了一个状态码）是同一个结构性 bug**：
 ...
 * ⚠️⚠️ **这份清单上一版三条错了两条**（复评发现）：「非流式那一轮一格都没走过」
...
describe("流式在途（有坏块那一支）：那一拍的屏幕不许比整版重建少说一句话", () => {
  ...
  /**
   * ── **第三轮修复定向复评：坏块那一拍同样不许整版重画** ──────────────────────────
   *
   * **它防住的真实故障（复评实测，不是设想）**：`0 → 1` 那一档上一版走的是
  ...
  /**
   * ── **收口：等价性装置对 `nodes.streamText` 那条路一格都没量到** ────────────────
   *
   ...
   *
   * ⚠️ **线材复用上面那份 `STEPPED_WIRE`，不另抄一份**（复评发现）：它的字节序
   ...
   *   ——那时红的是它，不是等价性那一条。
   * · 「那一行没跟着变」是**夹具自证③**，与上一条**不重叠**（复评抓到的洞）：
...
describe("媒体模式：地址、链接与不内嵌", () => {
  ...
  it("媒体结果只出现地址与链接，一个内嵌远端资源的元素都没有 —— "
    ...
    // ⚠️⚠️ **标签名必须一起断言，这是实测补上的**（变异实测）：上一版只比对 `src` 列表，
   ...
   *
   * **变红条件（实测，见当时的变异表）**：把 `buildMediaRow()` 里那句
  ...
  it("结果链接带 rel 的 noopener noreferrer 两条 —— "
     + "隐式补那一份是浏览器的默认值，不是这段代码的性质（评审发现）", async () => {
    const { sec } = await sendImage(respondWith({ gateway: () => ({ status: 200, body: MIXED }) }));
   ...
   *
   * **变红条件（实测，见当时的变异表）**：把 `syncSendButton()` 里那个
 ...
 * ⚠️ **这里刻意不写「哪几个」的那个数**（复评点过名）：上一版写死成「这两个函数」，
 * 而调用点当时就已经是三个（`buildMediaRow` / `buildMediaResult` / `fillMediaResult`）、
 * 后来又加到五个（`pushTurn` / `clearTurns`）——**那个数每加一格就旧一次**。
 ...
 *
 * ⚠️⚠️ **上一版的判据只有 `turns.push(`，而它守的那句话是全称句**（复评那两条）：
 ...
 *    ⚠️⚠️ **`fillMediaResult` 这个名字就是这么来的，如实记一笔**：那条评审发现
...
describe("媒体渲染路径的口令扫描：按出口数算覆盖面（评审发现）", () => {
  /** 把这一轮真的渲染出来的媒体出口收上来。 */
   ...
   */
  it("媒体那几个渲染函数里这道判据认得出的每一个出口，都被口令扫描跑过 —— 覆盖面按出口数算，不按用例数算；认不出的那几条射程写在 mediaOutputsInSource() 上方（评审发现）", async () => {
...
describe("视频两段式：建任务 + 轮询，以及那三条护栏", () => {
   ...
   * `prettyJson()`（无长度上限）。与「`turn.body` 可能是一张 MB 级 base64 图」
   ...
   * ⚠️⚠️ **反过来说，这一格证明不了「两条路径输出等价」，别把它读大了**（复评发现）：
  ...
  /**
   * **搬运风险 ③ 的复核**：`pg.media.noTaskId` 住在 `fillMediaResult()` 里，
   ...
   * 流式那一档栽的是**同一个** bug（复评发现），它也要用同一份装置
 ...
 * ① **有上限**——`turns` 原来只进不出，而单次整版重建的临时串量与轮数成正比
...
describe("对话轮数上限：截断要可见，清空要显式，还在收的那一轮不许被切掉", () => {
   ...
   *   —— **第二层替第一层挡住了变异。**
   *   **只改一半不算**（复评实测）：只把 `live` 改成 `[]`、不动 `done`
   ...
   * 而判据只扫 `turns.push(`——赋值形态是一个无人守的真洞。** 复评逐字实测：
   ...
   * ⭐ 这是本文件那条形状的**第三次发作**（「写下的覆盖面小于宣称的范围」）
   ...
   *   下面那组逃逸样本里有它）。
   *
   * ⚠️⚠️ **上一版那条 `inside.length === 1` 已经去掉，这是有意的**（复评那两条）：
  ...
  it("⑦ `turns` 的写只许出现在 pushTurn() / clearTurns() 与那句声明里（判据认得的四种写形态）—— 绕过它们的那条路径截断与披露都不会发生", () => {
    ...
    // **反向控制②**：那条洞的回归。逃逸样本是把发货代码那一行逐字改写出来的。
    for (const escape of [
