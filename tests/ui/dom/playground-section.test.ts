 ...
 * · **非流式那一轮**（`turn.stream !== true`）：这一组一格都没走过，它在别处；
 * · **流正常收尾之后那一拍**：这一组的夹具刻意全都挂住不结束 ⇒ 收尾之后的屏幕
 *   这一组一格都没看过；看它的是既有那格「中间夹一块读不出来的数据」
 *   （它等到收尾之后才断言，理由见上一段）。
 */
describe("流式在途（有坏块那一支）：那一拍的屏幕不许比整版重建少说一句话", () => {
   ...
   *
   * ⚠️ **夹具另起一份，不许往上面那份 `WIRE` 里加正文**：那一份被上面好几格共用，
   * 其中一格手写整句期望值「这条流里有 2 块数据读不出来」，而它上方那段逐字论证了
   * **「两块坏数据缺一不可」**（一块钉「那一行长出来」、一块钉「那一行跟着改」）。
   * 往里加一块正文 = 把那条刚钉死的论证搬松。
   *
   * **这一份夹具的字节序是刻意的：甲 → 坏块 → 乙 → 挂住。**
  ...
  describe("流式在途（零坏块尾巴：最后一拍是就地写字）：就地写字与整版重建输出逐字相同", () => {
    ...
    /**
     * 逐块放行到**最后一块正文落地为止**，中途把「夹具还是不是我说的那个形状」
     * 逐条断言掉。⚠️ 这些断言全是**夹具自证**，不是被测性质本身：
     * 少了它们，夹具哪天被改成「最后一拍是坏块」，等价性那一条照样全绿。
     */
    ...
    it("最后一拍是就地写字时，屏幕与整版重建仍然逐字相同", async () => {
      const { h, sec } = await startSteppedTextLast(WIRE_TEXT_LAST);
