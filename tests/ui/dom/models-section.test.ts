...
describe("读不出来 ≠ 一个模型都没有", () => {
  /**
   * **变红条件（本任务新建的这一格）**：把 `load()` 的 `catch` 分支从
...
describe("四模型 × 四协议矩阵", () => {
  /**
   * **变红条件（在 DOM 这一侧的落点）**：把 `protocolBadges` 改成
...
describe("端点那一列", () => {
  /**
   * **变红条件**：把 `endpointCell` 的 `for (const e of model.endpoints)`
...
describe("按协议筛选（工具栏）", () => {
   ...
   * 那一档**都恰好筛出 1 行，永远不为空（复评发现：上一版写的是「每一档」，
