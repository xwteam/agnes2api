...
/**
 * **DOM 垫片扩到 Key 池板块（待办第 9 条的 sec-keys 那一半）+
 * 写操作的行为覆盖。**
 *
 ...
 * duplicated.length）与两条强制变异（删除按钮判据、全选边界）落到行为
 * 断言上。
 */
...
// ───────────────────────────────────────────────────────────────────────────
// 删除按钮的可用性 —— 判据必须同时看 disabled 与 evicted
// ───────────────────────────────────────────────────────────────────────────

describe("删除按钮只看 evicted 是一个会咬人的错误判据", () => {
  /**
...
// ───────────────────────────────────────────────────────────────────────────
// offsetMs 三份合一之后，改一份的变异应当"改不动"（单一真源）
// ───────────────────────────────────────────────────────────────────────────

describe("本地时区偏移只有一份实现（admin-ui/js/pure/overview.mjs）", () => {
...
describe("裁定：添加 Key 分组下拉——容器与两组平级结构现在定死", () => {
  ...
  /**
   * ⚠️⚠️ **这一格后来换了守的东西，如实登记换的是什么。**
   *
   * 起初它守的是「两项是**禁用的占位符**，点了不会发请求」——那是当时的
   * 正确形态（后端那条端点还没有 `channel` 参数）。后来把两项真的接上了，
   ...
   *   确认弹窗必须明示消耗）。这一条与起初那条断言的观测点其实是同一个
...
// ───────────────────────────────────────────────────────────────────────────
// 复评发现：面板一侧的接线守卫退回成了源码字符串比对，仓里现成的输出层预言机没被用
// ───────────────────────────────────────────────────────────────────────────

/**
 * ⚠️⚠️ **复评发现**：Task 22A 给 `errorMessage()` 补的接线守卫是
 ...
 * **复评已经为同一个 `errorMessage()` 补过一格 DOM 用例**
 ...
 */
describe("复评发现：ja 面板上写失败画的是日文那句 —— 不是中文原话，也不是裸占位符", () => {
   ...
   *
   * 后端**漏了 `params`** 时（复评实测的那两条变异的形状），屏幕上真的会出现
