 ...
 * `onUnauthorized`、没有 `ApiError`），后来又加了 `js/gw-api.js`
 ...
 *
 * ⚠️ **为什么必须抠**：`admin-ui/js/pure/examples.mjs` 要生成一段 Node
 ...
 * **出网写法的枚举表。后来从四种补到七种。**
 *
 * ⚠️⚠️ **补进来的后三种是评审 F-11 实测出来的活口子，上一轮把它们
 * 原样交接了下来**（理由：Playground 是本期唯一新写前端出网代码的任务，
 ...
 * ⚠️ **这与 `164f6ea`「收窄那句全称句时把两条轴都列全」是同一族的第二次发生。**
 *
 ...
 * ⚠️⚠️ **抠注释这一步已经收编成 `scripts/lib/strip-comments.mjs` 那份真源。**
 * 上一版这里自持一份正则，旁边写着「今天 `admin-ui/js` 下零处含斜杠星号的字符串
 *（实地数过），所以这里保持原样不动；哪天有了，两道扫描会一起变瞎」——
 * **那句话描述的正是一条没有机器钉子的休眠洞**，而开工勘察当场把它点着了：
 ...
 * 而本仓注释极爱复述代码。这条不是纸面风险，BASE 就撞过一次：`admin-ui/js/api.js`
 * 里那段按名字锚引用下面那一格的注释，用例名一旦带上反引号包着的插值起始符，
 ...
 * 两个文件，现场复量对不上，同一句话前后两个值，而**两次都没有任何断言会因为它错了而红**。
...
const BLIND_SPOTS: ReadonlyArray<{ probe: string; why: string }> = [
  {
    ...
    why: "同一行里反引号数目是偶数、配对却是错的 —— `/`[^`\\n]*`/` 会把两个反引号之间的"
      ...
      + "——正是评审当时栽过的那个形状（一句只靠人记得去数的全称句）。登记在案。",
  },
  {
  ...
  {
    // **收窄造出来的一条**：`import` 与括号之间允许空格是合法 JS。
  ...
  },
  {
    ...
    why: "**运算符位置扫不到**：判据要求名字后面紧跟一个结束符，而这里跟的是 `|`。"
      ...
      + "**今天真有人可能顺手写出来**的（给 `fetch` 绑 this 是浏览器里常见的写法）。登记在案。",
  },
  {
    // **新造的一条**：别名那条规则只写了 `fetch` 一个名字。
  ...
  // ⚠️ **这里原来还有一条，后来把它修好了，所以整条删掉（不是放宽，是补上了）**：
...
describe("面板的网络出口清单", () => {
   ...
   * 四条样本分别钉住四件事，**期望值全部手写**。
   *
   * ⚠️ **第四条是评审补的，它才是这次改动真正的风险方向**：前三条里的真调用
  ...
  /**
   * **第 10 种假阳性：一根长得完全正常的毒刺让网络出口扫描变瞎。**
   *
  ...
  it("字符串里的 /* 不再让网络出口扫描变瞎 —— 毒刺与跨源 fetch 同时在场时仍要数到", () => {
    ...
    // **反向控制（「我对 X 不乱红」那一半）**：把跨源出口那一行拿掉、毒刺与闭合记号原样留着，
    // 处数必须是 0。少了这条，上面那条 `toBe(1)` 也可能是扫描器把毒刺自己数成了一处。
    const poisonOnly = [
      'const ADMIN_API_GLOB = "/admin/api/*";',
      "/* 提供闭合记号的普通块注释 */",
    ].join("\n");
    expect(egressSites(poisonOnly), "跨源出口不在场时处数必须是 0 —— 否则上面那条是恒真的").toBe(0);
  });
   ...
   * 写进注释里的同款也不再让它红**（实测：插一行注释形态的 ⇒ 这一格仍绿）。
   */
 ...
 * 而 `admin-ui/js/sec-playground.js` 有两处注释**专门讨论过 `window.open()`**
 ...
 *   （`ASSIGN` = `/(?:\|\||&&|\?\?|\+)?=(?!=)/`）。
 *   **这是第二轮修复定向复评补进来的，补之前这一族完整逃逸，而且当时被一句
...
describe("面板的导航出口清单（CSP 拦不住的那一族）", () => {
  ...
  it.each([
    ...
    // ── 方括号 + 字面量属性名这一族（修复定向复评：补之前整族逃逸，实测 2716 全绿）─────
    ...
    // ── 一族赋值运算符（第二轮复评：补之前整族逃逸，而且被一句「送不到外站去」降级封存）─────
  ...
  /**
   * ── **已知会「多数一处」的写法，连同期望值一起登记（第二轮复评发现）** ─────────
   *
