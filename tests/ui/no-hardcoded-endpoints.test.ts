...
describe("前端不许硬编码网关端点", () => {
  ...
  /**
   * ── **全局约束 15 的另外两轴今天没有任何机器在守（评审发现）** ─────────────────
   *
   * 这道扫描的名字里写着「端点只许来自 /admin/api/models」，而全局约束 15 管的是**三样**：
   * **端点路径 / 请求体形状 / 协议名**。这道扫描只覆盖第一样。
   *
   * ⚠️⚠️ **评审实测过后两样的现状**：往板块文件里种一段四协议的 id 对照表
   *（`{ openai: …, anthropic: …, responses: …, gemini: … }`），
   * **全仓唯二变红的是 `tests/unit/ui-assets.test.ts` 的
   * 「源目录里每个文件都在生成物里，且内容一字不差」那种逐字节相等断言**
   * ——而那一格问的是「改了 `admin-ui/` 有没有重跑 `build-ui.mjs`」，
   * **对「这段代码在语义上违没违反约束 15」零信息量**（重跑一次生成器它就绿了）。
   * ⇒ **协议 id 与请求体形状这两轴，今天靠的是代码评审，不是机器。**
   *
   * **今天的代码是干净的**（`js/pure/models.mjs` / `js/pure/examples.mjs` /
   * `js/pure/playground.mjs` / 三个消费它们的板块文件里一个协议 id 字面量都没有），
   * **所以这是一条边界登记，不是一个活缺陷**。写成会红的断言而不是散文，
   * 理由与本文件 `BLIND_SPOTS` 那一组完全相同：**一条只写在注释里的边界，
   * 改判据的人不会读到**，而下一个人会把这道扫描的绿读成「约束 15 有机器拦着」。
   *
   * **这一格变红意味着有人把这两轴也补上了——那是好事**，把它删掉即可。
   */
  it("已知抓不住：协议 id 与请求体形状这两轴 —— 约束 15 的绿不等于三样都被守住了", () => {
    // 一段**货真价实**的违规：四个协议 id 硬编码成一张前端自己的对照表。
    const probe = 'const PROTO_LABELS = { openai: "OpenAI", anthropic: "Anthropic",'
      + ' responses: "Responses", gemini: "Gemini" };';
    // 这道扫描按定义看不见它（它扫的是引号紧挨着端点路径这个形态）。
    expect(hit(probe), "这条写法现在被抓住了，请把这一格连同上面那段说明一起删掉").toBe(false);
    // 反向自检：探针本身必须**真的**含着那四个 id，否则上面那句 `false` 什么都没证明。
    for (const id of ["openai", "anthropic", "responses", "gemini"]) {
      expect(probe, `探针里没有 ${id}，它证不了什么`).toContain(id);
    }
  });
