 ...
 * 板块文件、pure 里那份留成死代码」。评审实测过：
 * 把 `pure/keys-write.mjs` 新增的全部函数抄回 `sec-keys.js`，138/138 全绿。
 *
 * **后来把它从「一个模块 + 一种声明形式」扩到「全部模块 + 两种形式」**，
 * 起因同样是一次实测，而不是预防性扩容：交接时量过——
 ...
 *
 * ⚠️ **它原来住在 Key 池板块那个 DOM 用例文件里**——一道全仓级的
...
describe("硬规则 1 的另一半：sec-*.js 不许重新声明 pure/*.mjs 已导出的函数名", () => {
  it("扫描范围本身不是空的——十六个 pure 模块、九个板块文件都得真的被扫到", () => {
    // 反向自检：下面那条相等断言在「一个文件都没扫到」时同样是绿的。
    // 数字手写，改动 admin-ui 的文件结构时必须回来表态。
    // 设置页那一轮各加一个（`pure/settings.mjs` 与 `sec-settings.js`）⇒ 10 / 5。
    // 用量那一轮各再加一个（`pure/usage.mjs` 与 `sec-usage.js`）⇒ **10 → 11、5 → 6**。
    // 模型那一轮各再加一个（`pure/models.mjs` 与 `sec-models.js`）⇒ **11 → 12、6 → 7**。
    // 集成示例那一轮**只加了 pure 那一侧**（`pure/examples.mjs`）：集成示例是设置页的
    // 第 4 张卡，住在已有的 `sec-settings.js` 里，没有新板块 ⇒ **12 → 13，板块数不变**。
    // 调试台那一轮各再加一个（`pure/playground.mjs` 与 `sec-playground.js`）⇒ **13 → 14、7 → 8**。
  ...
  it("全部 pure 模块的导出函数名，一个都不许在 sec-*.js 里被重新声明", () => {
    ...
    // **手写下界，不从扫描结果推导**：旧的那一版只扫一个模块时是 5 个以上，
