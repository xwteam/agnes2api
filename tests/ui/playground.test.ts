 ...
 * **Playground 的请求构造。**
 *
 * 被守护的核心性质只有一条，它就是那一期的核心设计决定（全局约束 15）：
 * **四个消费者只许有一份「怎么调这个网关」的知识。**
 * 集成示例卡教运维怎么调，Playground 真的去调一次——
 * 设计文档那条订正把这两件事挪进同一期的逐字理由是
...
/**
 * 合成目录里那句占位文本。**故意不是真源那个 `"ping"`**。
 *
 * 早先面板自己存着一份 `PROMPT_SLOT_SAMPLE = "ping"` 的副本，那份副本
 * 只能靠一格逐字比对兜着。**后来把它消掉了**：占位文本现在跟着
...
describe("协议目录窄化：读不出来 ≠ 一条协议都没有", () => {
  ...
  it.each([
    ...
    // ── 后来加的那一格（`streamTextPath`），四种坏法各一条 ──────────────────────
  ...
  /**
   * **`samplePrompt` 缺席同样是「整份读不出来」，不是「少一格」**。
   *
...
describe("withPrompt：判据是形状，不是协议 id", () => {
   ...
   *
   * 这是真源文件头点名的那个失效：谁把某条协议的样例文本改成别的，
  ...
  /**
   * **登记项 ③ 已经不存在了（后来消掉的），这一格是它的替代品。**
   *
   * 早先在面板侧存着一份 `PROMPT_SLOT_SAMPLE = "ping"` 的副本，靠一格逐字比对
...
describe("buildRequest：URL 全部由 origin + 真源路径拼出", () => {
   ...
   * **一个硬编码路径都没有。**
   *
   * 判据与集成示例卡是同一条：喂一份**改过 `pathTemplate` 的**假目录，
...
/**
 * ── **评审发现：两份 SSE 帧解析在 `[DONE]` 上到底对不对齐** ───────────────────────
 *
 * `js/pure/playground.mjs` 的 `sseFrames()` 文件头一直写着「两份实现共享同一套判据
 *（`data:` 前缀、`\n\n` 分帧、**`[DONE]` 终止**），是刻意对齐的」，而当时登记的
 * 遗留是「**没有任何机器绑住这个对齐**」——**登记的是「没有机器绑住」，没有人去核过
 * 它们今天到底对不对齐**。评审核了：`[DONE]` 之后那一帧，网关丢掉、面板照收。
 *
...
describe("两份 SSE 帧解析在 [DONE] 上逐字对齐", () => {
  ...
  function panelPayloads(wire: string): string[] {
    ...
    //    「`rest` 必须交空」那一半，而那正是「`[DONE]` 之后那一帧」的分叉能从后门被捡回来的路径。
    for (const p of sseFrames(`${buf}\n\n`).payloads) out.push(p);
    return out;
  }
  ...
  it.each([
    ...
    // ── 那条分叉：`[DONE]` 之后还有一帧带正文的 data。
    [
      'data: {"a":1}\n\ndata: [DONE]\n\ndata: {"post":"DONE"}\n\n',
      ['{"a":1}'],
      "[DONE] 之后那一帧：网关丢掉，面板从此也丢掉（修之前面板会多收一条）",
    ],
...
describe("deltaText：三态（读不出来 / 不带正文 / 正文）", () => {
  ...
  it("数组下标只认十进制整数 —— 裸 Number() 会把 \" 1\" / \"0x2\" / \"1e0\" 静默解析成别的下标", () => {
    ...
    /**
     * ⚠️⚠️ **这一组输入是变异实测挑出来的，不是想出来的（第一版 ESCAPED）。**
     *
