...
export const GLOSSARY: readonly GlossaryTerm[] = [
  ...
  },
  {
    term: "速查",
    note:
      "ja 的 `早見表`（ADMIN）vs `一覧`（README，**已销账**：那份 README 的"
      + "`## エンドポイント一覧` 换成了章节标题常量表的 `## 📡 API エンドポイント`，"
      + "残留在 ```bash 块里的一行 `# …一覧する` 注释也一并改掉——"
      + "⚠️ `HEADING_LINE` **不剥围栏**，围栏里以 `# ` 开头的行照样算标题）。"
      + "`一覧` 这一格**留着不删**：它今天在 ja 标题里零命中，"
      + "哪天有人把它写回任何一份 ja 文档的标题，这一条会当场把冲突叫出来。**只扫标题**："
