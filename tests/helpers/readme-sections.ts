/**
 * W38 —— README 的章节标题常量表（根 16 节 / 语言版 12 节 × 五语言）。
 *
 ...
 *
 * ⚠️ **本表只是常量，它不证明仓里的 README 已经长成这样。** agnes2api 的六份 README
 * 今天还是 9 节的旧骨架，扩到 16 / 12 节是后续工作项的事。这里能立刻上锚的只有
 * `SPONSORS.md` 那一节（下标 11）——五份 `docs/{lang}/SPONSORS.md` 的 H1 正是这一行，
 * 判据拿它当阳性对照，保证这张表**今天就有一个真实消费者**，不是一张写完就锈的死表。
 */
