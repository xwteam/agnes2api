...
describe("GET /admin/api/keys", () => {
   ...
   * 第 1 页（越界即取 fallback）。实施时按**实现的真实行为**钉住并提请裁定，
   * **裁定见设计文档的订正 R1：保留 `intParam` 的回落语义、订正那句括注**
   * （`size` 与 `page` 共用同一个 helper；而唯一能触发越界的现实路径是
   * 「筛选把结果集缩小了、前端手上还攥着旧页号」，那时回第 1 页本来就比跳到
   * 一个任意的末页更对）。
   */
