...
describe("面板轮询的配额账", () => {
   ...
   */
  it("?after=0（评审点名的敌意输入）单次请求的 get 次数恰好为 48，与冷读相同（不再放大）", async () => {
    const st = new CountingStorage();
   ...
   */
  it("游标冻结在很早以前、之后 20 次轮询 now 持续推进（评审点名的安静场景），get 总数恰好为 960，不随陈旧度增长", async () => {
    const st = new CountingStorage();
  ...
  it("面板轮询模式（第 2 次起带 cursor）：20 次里第 1 次冷读、其余 19 次暖读，get 总数恰好为 86", async () => {
    ...
    // MemoryStorage 必须与 `now` 共用同一个假时钟（评审发现，理由同
