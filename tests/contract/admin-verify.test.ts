...
describe("明文 key 一个字节都不许回到面板（全局约束 11a）", () => {
  it("上游 401 的响应正文一个字节都不回给面板 —— 凭据无效的错误体正是各家 API 最爱回显 key 片段的地方", async () => {
    ...
    // ⚠️ **响应头也要纳进判据（本轮评审）**：上一版只看正文，而把上游那份
    // 错误体塞进一条自定义响应头（`x-upstream-detail: …`）**照样全绿**——
    // 「一个字节都不许回来」说的是整份响应，不是它的 body 那一半。
    const headerDump = [...res.headers.entries()].map(([k, v]) => `${k}: ${v}`).join("\n");
    // ⚠️ **非空锚（定向复评）**：把 `headerDump` 换成 `""` ⇒ **24/24 全绿**
    // ——响应头那一半当时什么都没验。同一个提交里，下面「第二条出口：事件环」那条发现
    // 刚立下「必须有非空锚」这条规矩，而它在这一格没落地。
    // `content-type` 是每条 `c.json()` 都一定带的那一条。
...
describe("出站探测护栏：与通道测试共用的那一套（全局约束 14）", () => {
  ...
  /**
   * ⚠️⚠️ **这一格补的是「在途去重」那道闸在 handler 层唯一的观测点（本轮评审）。**
   *
   ...
   * 让时序性质不可观测）。实测代价：那一次变异（**把 429 支也写进 `try/finally`，
