 ...
 *
 * ⚠️⚠️ **后者今天仍然没有被验过，而这是一条带理由的登记、不是一句「回头再说」：**
 * 验它**必须联网打真上游**，而真上游要真凭据 —— 本仓的全部纪律建立在
 * 「仓库零内置凭据」上（`scripts/scan-secrets.sh` 那道门禁会当场打红），所以双形态真机验收
 * （`scripts/smoke-dual-runtime.sh`）**从一开始就把「一个字节的真上游都不联网」
 * 写成硬约束**：它的上游是脚本自己起的 stub，验的是「两个形态自己跑不跑得起来、
 * 行为一不一致」，不是「上游长什么样」。⇒ **这一条不在那份冒烟的射程里。**
 * ⚠️ **措辞订正（本任务实测）**：不许写成「依据强度已登记在 `upstream-facts.ts`」——
 * 那张表今天只有三条，讲的都是**响应形状**（任务标识落在哪一格 / 什么字符集 /
 * 流式末帧带不带 usage），**没有一条讲「示例请求会不会被上游接受」**。
 * 它承载的是另一件真话：那三条的 `status` 全是 `assumed`，`source` 逐条写着依据来自
 * **我们自己写的假上游** ⇒ **本仓至今零份真上游样本**。
 * ⚠️ **这条欠账的登记位置在 `CHANGELOG.md` 的「已知限制」那一节**（那一条逐字写着
 * 「本仓至今零份真上游样本」）——它是一条**面向读者**的限制说明，不是内部待办：
 * 谁拿这份目录去对上游，都该先知道它没有被真上游验过。
 */
describe("协议目录的示例请求真的调得通", () => {
  it.each(PROTOCOLS.map((p) => [p.id, p] as const))(
    ...
    + "这一格是那条订正『漂了没人会发现』的唯一护栏，也是 upstreamPath 的唯一护栏（评审发现）",
    async (_id, p) => {
      ...
      // 存储（本仓在这上面栽过一次，结论从「无缺陷」翻成 Critical）。
      ...
      // ★ 评审那条的落点：观测点在**真实出站 URL**上，不是比对目录自己的两个字段。
   ...
   * 双形态真机验收把「不联网打真上游」写成了硬约束，所以**它也不归那份冒烟**，
   * 至今没有被验过）。**但它可观测的那一半在这里**：目录声称必填的那个字段，
   * 有没有活着穿过网关的协议转换、真的到达上游。掉了它，示例在生产上就是一条 400，
   * 而面板照样把它印出来教用户怎么调——这正是那条订正「漂了没人会发现」的形态。
   *
   ...
   * ⚠️ 这段注释一度被后来插进来的媒体那两格顶得离它的用例很远，已挪回来。
   */
