...
describe("枚举式鉴权矩阵（路由 × 凭据状态，笛卡尔积）", () => {
  ...
  const EXPECTED = [
    ...
    // ── 上游此刻有哪些模型 ────────────────────────────────────────────────────
    //
    // 用 `admin.get()` 注册（**不是 `use()`**）⇒ 不产生 ALL 条目，
    // `EXPECTED_MIDDLEWARE` 保持不变。**每一次新增端点都要在这里明确表一次态。**
    //
    // ⚠️ **它比上面那条静态目录更不能免鉴权**：那一条只是把一张静态地图交出去，
    // 而这一条**按一下就拿池里的一把 key 去打一次真实的上游请求**。免鉴权意味着
    ...
    // 上游凭据，这一族是**我们签发的**、别人拿来向我们证明身份的凭据。两个名字很像
    // 是已知代价，完整对照表在 `src/core/admin/api-keys.ts` 的文件头。
    //
    // 五条都用 `admin.get/post/delete/patch()` 注册（**不是 `use()`**）⇒ 不产生 ALL
    // 条目，`EXPECTED_MIDDLEWARE` 保持不变。**每一次新增端点都要在这里明确表一次态。**
    //
    // ⚠️ **`PUBLIC_PATHS` 不增长，而这一次「不增长」要论证**：这五条里 `GET` 那条吐的是
    // 全部子密钥的掩码、名称与到期时间——**一份「这台网关发出去过哪些密钥」的清单**；
    // 而 `POST` 那条**按一下就签发一把能直接调用 `/v1` 的凭据**。一个鉴权失效的它
    // 不是泄露数据也不是销毁数据，是**让任何人给自己发一把长期有效的网关钥匙**
    // ——这是这张表上第一条「鉴权失效即等于把网关免费送出去」的端点。
    //
    // ⚠️ **五条的注册都没有任何条件，不看接没接线**——理由与 `registrar/tend`、
    // 用量那三条逐字相同：做成条件注册的话这张快照会随夹具装配变化，而默认夹具恰好
    // **没接**子密钥存储，于是这五条会从整个矩阵里消失。「没接」是 handler 里的一条
    // 503，不是「这条路由不存在」。
    //
    // ⚠️ **矩阵会拿正确的管理口令把每条路由真的打一遍**，而 `purge` 打通了就是清掉
    // 一批密钥——它安全的理由有两层，与 `POST /admin/api/keys/purge` 那条同源：
    // ① `version` 必填，矩阵发的是不带请求体的 POST，在 `readAdminJson` 那一步就 400；
    // ② 默认夹具**根本没接**这张表，那五条在矩阵里拿到的是 503。
    // **别把 `version` 改成可选**，第一层保险就是这么没的。
