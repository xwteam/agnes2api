 ...
 * **流式那条路的双运行时契约。**
 *
 * ── **这一组能给那条待验证项结案吗：不能，而且这句话必须写在最前面** ───────────
 * 那条待验证项问的是「**workerd 的 HTTP 服务层**会不会把一条 SSE 响应整体缓冲」。
 * 本文件走的是 `app.request()`，**它根本没有经过任何 HTTP 服务层**——请求对象直接
 * 交给 Hono，响应对象直接拿回来。它能证明的是「Hono / dispatcher / 四条协议转换
 * 这一段不缓冲」，**证明不了 workerd 那一段**。拿它给那条待验证项结案就是本仓登记的
 * **第 7 种假阳性（测的是抄件不是原件）**，而且踩在本期自评的头号风险上。
 *
 * ⇒ **它由真机仪器结案，不由本文件结案**：`wrangler dev` 起真 workerd，
 ...
 * ⇒ 它在 workerd 下对**正确实现**恒红（当时的变异表）。**判据用错工具，
 * 这次是恒红；上一次（评审发现）是恒绿——两种都得靠实测才看得见。**
 */
 ...
 *
 * ⚠️ **这一组同样不给那条待验证项结案**（理由见本文件开头那段）：`app.request()` 不经过
