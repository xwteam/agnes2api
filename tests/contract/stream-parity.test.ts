 ...
 * **流式那条路的契约：网关这一段不整体缓冲。**
 *
 * ⚠️ **文件名里那个 `parity` 是历史残留。** 它原来叫「流式那条路的**双运行时**契约」
 *（同一份断言在 node 与 workerd 下各跑一遍），v0.4.0 摘掉 Worker 形态之后没有
 * 第二个运行时可比。**没有改文件名是刻意的**：改名会让 `git log --follow` 之外的
 * 追溯全断，而本仓好几处注释按名字锚指向它。**里面一格都没删**——下面每一条钉的
 * 都是「Hono / dispatcher / 四条协议转换这一段逐块交出去」，与跑在哪个运行时上无关。
 *
 * ── **这一组能证明什么，不能证明什么（原样保留，今天换了个对象仍然成立）** ─────
 * 本文件走的是 `app.request()`，**它根本没有经过任何 HTTP 服务层**——请求对象直接
 * 交给 Hono，响应对象直接拿回来。它能证明的是「Hono / dispatcher / 四条协议转换
 * 这一段不缓冲」，**证明不了 HTTP 服务层那一段**（从前那一段是 workerd，
 * 今天是 `@hono/node-server`，外加运维自己那层反向代理）。
 * 拿它去给「整条链路不缓冲」结案，就是本仓登记的**第 7 种假阳性（测的是抄件不是原件）**。
 *
 * ⇒ **那一段由真机仪器结案，不由本文件结案**：假上游 1 秒/块 × 4 块，
 * `curl -N` 逐块打时间戳（`scripts/prepush.sh` 的真机冒烟那一格就是这么跑的）。
 * **本文件是那条结论的回归网**，不是它的证据。
 *
 * ── **第 8 种假阳性：不许用零延迟替身** ────────────────────────────────────────
 ...
 *
 * ⚠️ **末尾那一块 `finish_reason` 不是装饰**（评审发现）：没有它，
...
describe("流式：网关不整体缓冲（那条待验证项的回归网，不是它的证据）", () => {
  /**
   ...
   * ③ **`app.request()` 不经过 HTTP 服务层**（真机上请求先落在
   *    `@hono/node-server` 那儿，再由它转交 `app.fetch`；线上还多一层反向代理）。
   *    那一层只能靠真机结案（起真进程 + 假上游 1 秒/块 + `curl -N` 逐块打时间戳，
   *    `scripts/prepush.sh` 的真机冒烟那一格就是这么跑的），本文件是那条结论的
   *    回归网、不是它的证据。
   */
 ...
 *
 * ⚠️ **它为什么在 `tests/contract/` 而不是 `tests/unit/sse.test.ts`：那条理由已经没了。**
 * 原话是「『不整体缓冲』是双运行时性质，而 `tests/unit/**` 只有 node 那份配置收集，
 * 只写在 unit 里等于只装一半网」——**v0.4.0 之后只有一份配置，两个目录都是它收集**，
 * 放哪儿在「跑不跑得到」这件事上完全等价。**没有搬走是范围取舍**：`git mv` 一个
 * 测试文件会把本仓好几处按路径锚指向它的注释一起打断，而收益是零。
 *
 ...
 *
 * ⚠️⚠️ **不许用「数 `next()` 调用次数」那把尺子**（实测撞掉的一版，教训今天照样成立）：
 * 派发单原本写的是「第一块交出来时生成器只被 `next()` 过 1 次」。**当时的实测是
 * node 下 1、workerd 下 2 —— 而 `toSseStream` 是同一份逐块实现、一个字没改。**
 * 那把尺子量的是**宿主对流的 pull 调度**（「交付第一块之后什么时候再 pull 一次」），
 * 不是「第一块什么时候交给下游」⇒ 它对**正确实现**都可能恒红。
 * workerd 那一侧今天不在仓里了，**但换 Node 大版本、换 `ReadableStream` 实现是同一类
 * 变化**，这条禁令与有几个运行时无关。**判据用错工具，那次是恒红；
 * 上一次（评审发现）是恒绿——两种都得靠实测才看得见。**
 */
describe("流式：toSseStream 这个原语自己是逐块的", () => {
   ...
   *
   * **变红条件（实测，当时的变异表）**：`src/core/protocol/sse.ts` 的
 ...
 *
 * ⚠️ **「必须写在 `tests/contract/`」那条理由已经没了**（与上面那一组同源）：
 * 原话是「这是一条双运行时性质，而 `tests/unit/**` 只有 node 那份配置收集」，
 * 而那份 workers 专用配置随 Worker 形态一起在 v0.4.0 删掉了，只剩一份配置。
 * 留在原地是为了不打断按路径锚指向它的那些注释。
 *
 * ⚠️ **这一组同样不给「整条链路不缓冲」结案**（理由见本文件开头那段）：`app.request()`
 * 不经过任何 HTTP 服务层。它守的是「Hono / 四条协议转换这一段在 CRLF 上游下仍然逐块交付」。
 *
