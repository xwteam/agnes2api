...
describe("GET /admin/api/usage 的查询参数契约", () => {
   ...
   * 查询串的地方是一个测试里的占位 `?from=…&to=…`，而 Task 5 的 `rangeToQuery`
...
// ───────────────────────────────────────────────────────────────────────────
// ⑤ 30d 那一档必须失败得诚实（U-H 的落点）
// ───────────────────────────────────────────────────────────────────────────

describe("30d 那一档失败得诚实", () => {
  /**
   * U-H 的裁定：Cloudflare 的两页官方文档在「一次调用能发多少次子请求」上互相对不上
   * （Workers limits 页免费档 **50**，KV limits 页 **1,000**，而前者从头到尾没定义
   * KV 算哪一行）。⇒ **在 P3e 的双形态真机验收把它了结之前，任何注释都不许写成
   * 「60 次是安全的」**，能做的只有让它**失败得诚实**。
   *
...
describe("note 是 code 不是句子", () => {
  ...
  /**
   * **同一份知识只许有一个出口**（评审 I20）：`tokensCoverage` 只从
...
// ───────────────────────────────────────────────────────────────────────────
// ⑨ record 期与 flush 期是两条错误通道（P3d Task 4 认账修正）
// ───────────────────────────────────────────────────────────────────────────
