 ...
 *
 * 事件板块那一轮就是这么栽的：`wire.ts` 把真实通道参数换成 `null` ⇒ 1939 条全绿，
...
/** 遍历一个 JSON 值的**全部叶子**，逐个交给调用方。判据抄评审发现的形状。 */
...
describe("凭据只写不读（设计 §8.6）", () => {
  /**
   * **【不出网类】变异的靶子：把 `gatewayToken` 放进 `stored` 或 `effective`。**
   *
...
describe("写入前校验（设计 §5.4 第 1 条）", () => {
   ...
   * **【落盘类】变异的靶子：先写后校验。**
   *
   * ⚠️ **只断言状态码是 400 抓不住「先写了再返回 400」**（与另一条发现同一个形状）：
   ...
   *
   * 这一格是评审之后才补的。**旧那一格的用例名声称「与 PUT 用的是同一套
...
// ───────────────────────────────────────────────────────────────────────────
// Step 5：接 invalidate()，再谈 propagation
// ───────────────────────────────────────────────────────────────────────────

describe("保存之后同一个进程立刻回读到新值", () => {
  /**
   ...
   *
   * ⚠️⚠️ **这一格的第一版被变异完整逃逸（17/17 全绿），成因值得记下来。**
   ...
   *（复评发现）。后端哪天顺手给 `GET` 也塞一个 `changed: []`，
   * 面板会把**读取态**当成一次空回执 ⇒ `saved = []` ⇒ `touchesLiveField([]) === false`
   * ⇒ **传播上界那整句从页面上消失**，而它是当初论证出来的必须显示项
...
/**
 * ⚠️⚠️ **这一组是评审那两条的护栏。改动之前，这两条路径一格用例都没有**
...
describe("清掉一条在链上的通道凭据", () => {
  ...
  it("真的存储故障不许被吞成诊断视图 —— 没有 blocker 就原样抛", async () => {
     ...
     * · 第一版（每次都抛）配的是「blockers 为空就抛」那版实现，**变异完整逃逸**
     *   ——`catch` 里那次「再读一遍」也抛、直接冒出去，被测分支从头没走到；
     * · 第二版（只抛一次）配的是「存储读得出来就是配置问题」那版实现，
     *   而那版把**瞬时抖动**误判成配置问题（评审之后的自查）。
   ...
   *
   * ⚠️ 这一格是我自己在那条发现收口时补的：那一版判据写成「存储读得出来 ⇒ 就是配置问题」，
  ...
  /**
   * ⚠️⚠️ **`configLoadBlockers` 不完备，所以它不能当「装得起来吗」的判据。**
   *
 ...
 * ——评审自己也如实写了这一条（置信度 MEDIUM）。
 *
 * **那它为什么还要补**：这道护栏本期建过两次（Key 写那四条 ——
 * `tests/contract/admin-auth.test.ts` 的「鉴权失败的非幂等请求必须零副作用」；
 * `POST /registrar/tend` ——
 * `tests/contract/manual-tend.test.ts` 的「鉴权失败的『立即补池』必须零副作用」），
 * **config 写这两条没有** —— 而这两条恰恰是全仓
 * 危害最大的写：`PUT /admin/api/config` 能改掉 `gatewayToken`，
 * `secrets/clear` 能删掉凭据（`router.ts` 那句「一个鉴权失效的 `PUT /admin/api/config`
 * 等于把整台网关交出去」说的就是它）。
 *
 * ⚠️ **这段理由订正过一次（修复轮复评发现①）。** 原文写的是「它防的是 `router.ts`
 * 那段 ★ 已实测的排序 bug：handler 先跑完、鉴权再生效，**状态码仍是 401 而副作用已发生**」
 * —— **那是把已实测的东西说反了**：`router.ts:190-192` 逐字写着那个形态量出来是
 * **200，鉴权静默失效且不报错**，而 200 是状态码断言本来就拦得住的。
 * 复评自己复现过：把 `adminAuth` 改成 `await next(); return c.json(…, 401)`，
 * Hono 回的是 **200**（handler 的响应赢了）；要造出「401 且副作用已发生」，
 * 必须显式覆写 `c.res` —— **那不是那条排序 bug 会自然产生的形状**。
 *
 * 这一格钉的是**比那条 bug 更弱、也更长命的一个假设**：任何让 handler 在鉴权判负之后
 * 仍然跑到的实现变更（中间件顺序改动、提前覆写 `c.res`、将来换掉路由器），
 * 此时状态码可能仍是 401 而副作用已经发生。**只断言 401 对这一类完全无感。**
 * 判别力已被复评实测过：强行构造「handler 跑完但仍返 401」时，
 * 红的是**计数**断言（`expected {puts:2} to deeply equal {puts:1}`），不是状态码那格。
 */
describe("鉴权失败的两条 config 写端点必须零副作用", () => {
  /**
