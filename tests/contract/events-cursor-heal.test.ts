...
/**
 * **评审三审(a)**：这是本文件第一次用"逐轮真的驱动 `poll()` 那一整套纯函数
 ...
 */
describe("事件面板轮询：持续跨 isolate 时钟偏移下的自愈稳态（评审三审 / 四审）", () => {
  /**
   ...
   *
   * ⚠️ **两边都用生产的 `ConfigHolder`**（`realConfigHolder: true`，评审发现）。
  ...
  it("交替命中两个时钟不同步的 isolate：退避收敛到 60 秒上限并稳住，不是永远卡在 15 秒", async () => {
    ...
    // **退避必须真正爬到上限并稳住**——评审三审(a) 的核心断言。最后 10 轮
    ...
    /**
     * **诚实记录，不是掩盖**（评审三审要求确认、四审要求按实测订正措辞）：
     ...
     *    是刻意构造的最坏情形压力测试（与用 `after=0` 压测钳位同一个方法论）。
     ...
     * 逐轮交替」这个根因，不是生产的黄条渲染。** 把 `renderWarnings()` 改成粘性
     *（亮了就不灭），全套一条都不红。所以别把这几条读成"黄条闪烁这件事有守护"：
     * 被守护的只是它的输入。
     *
     * ⚠️⚠️ **上一版给的理由是「本仓没有 DOM 测试通道」——那句话早就不成立了**
     *（复评抓到）：`tests/ui/dom/` 有 harness 与五组用例，
     * `tests/ui/dom/events-poll.test.ts` 的
     * 「响应体缺 cursor 字段时，契约破坏那条黄条真的显示出来」
     * 就是在真 DOM 上钉黄条接线的。**真实的缺口只剩"逐轮覆盖 vs 粘性"这条跨轮
     * 时序性质**，那需要一组跨轮 DOM 断言——**是"还没写"，不是"没法写"。**
     */
   ...
   *
   * ⚠️ **这组数字在评审之后整体抬高了 1,440**：夹具原来用的是
  ...
  /**
   * **对照组：这条自愈最初设计要处理的现实场景——一次性的时钟异常（运维手动改错
