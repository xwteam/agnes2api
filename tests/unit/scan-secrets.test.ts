 ...
 * 这不是假想：同一道门禁刚刚才因为 `-I` 的二进制盲区吃过一次亏（评审发现）。
 *
...
describe("scripts/scan-secrets.sh 第 6 条规则：裸 IP", () => {
  ...
  /**
   * **复评发现的绊线：点号相邻的公网裸 IP 从这条规则整个逃走，而且两档一起逃。**
   *
  ...
  /**
   * **复评发现的绊线：这条规则对二进制文件失明，而它上面那段注释逐字保证"不会失明"。**
   *
...
describe("scripts/scan-secrets.sh --history", () => {
  ...
  /**
   * **复评发现的绊线：`--history` 第一版只收 blob，提交信息里的凭据一个字看不见。**
   *
...
describe("scripts/scan-secrets.sh 的注释", () => {
   ...
   * 断言。它不是死断言这件事由本任务的变异证明：往 `scripts/scan-secrets.sh`
   * 里写一句「第 N 道门禁」，这一格当场红。
   *
   ...
   * 这里要不要跟着动」。
   *
   * ⚠️ **上一版这里自称「逐字节孪生体比对」，那是一句假的全称句（复评发现）**：
