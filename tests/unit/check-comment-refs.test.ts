 ...
 *
 * ⚠️ **这条理由被改写过，旧的那条是假的**：原文写的是「夹具**字符串**
 * 会被扫描器当成真注释」，依据是「扫描器不解析字符串字面量」——那句话现在不成立了
 * （改过之后它逐字符扫、跳过字符串）。实测把这行标记停掉，夹具字符串一条都不再泄漏，
 * 剩下的全是这份文件自己的真注释。**豁免仍然需要，但需要它的理由换了一个。**
 *
 * 它守的那件事：本仓台账已记二十余次「注释里写下一句假断言」，评审这一轮又新
...
describe("规则 E：注释里的门禁绝对序号", () => {
   ...
   * 写的是**保险 / 筛子 / 闸 / 关口 / 护栏**，甚至「第二道：」后面
   * 直接跟冒号，`防线` 只是其中一种。一条只排除「防线」的前瞻会当场制造成片的假红。
   * 下面每一条都逐字抄自仓里的真句子（去掉了会被规则 A 一起判的仓内路径）。
   *
   * ⚠️ **上一版这里写的是「`防线` 只占其中三处」，那个数落地即假**：本任务复评
   * 逐字面量量出来是四处，而**其中一处正是下面这张表自己新添的夹具**——
   * 一个连自己都会当场推翻的硬编码计数，还没有任何测法。这条反向控制的力量
   * 来自「下面每一句都是仓里的真句子」，不来自「一共有几句」。
   */
  it.each([
  ...
  /**
   * **判据有一族会打红本来正当的句子，那就必须给出路**（复评 MED-3）。
   *
   * `CI 的第 N 道` 那一支只要求「前面挨着 CI」，不要求「道」后面跟「门禁」——
   * 于是「CI 的第二道保险」这种**在说保险、不在说门禁**的正当句子会被打红。
   * 收窄它就要退回「排除哪些名词」那条路，而那正是这条规则实测推翻掉的方向。
   * **所以留着假红、但报文必须告诉人正当出路是段级豁免**：报文原来只给
   * 「改写成脚本名」和「改成写对的序号一样红」，撞上的人只剩「把一句正确的话改坏」
   * ——**报文亲手把人引进坑**，正是这条规则自己拿来推翻需求书的那一族缺陷。
   */
  it("规则 E 的假红有正当出路：报文必须给出段级豁免", () => {
    const body = "// 这条不变量由 CI 的第二道保险兜着。\n";
    const red = run({ files: { "src/a.ts": `${body}export const a = 1;\n` } });
    expect(red.status, "这一支现在不误报了？那就把这一格改成反向控制并去改规则 E 的边界说明").toBe(1);
    expect(
      red.stderr,
      "会误报的判据 + 不给正当出路的报文 = 逼人把一句正确的话改坏",
    ).toContain("@refs-ignore");

    // 报文说的那条路必须真的走得通——只写在报文里、实际不生效，比不写更糟。
    const out = run({
      files: {
        "src/a.ts": "/**\n * @refs-ignore（这里说的是保险，不是门禁序号）\n"
          + " * 这条不变量由 CI 的第二道保险兜着。\n */\nexport const a = 1;\n",
      },
    });
  ...
  /**
   * **这道门禁的射程边界，钉成断言**（复评 MED-4）。
   *
   * `walk()` 只收 `.ts` / `.js` / `.mjs`。`scripts/scan-secrets.sh` 与
   * `.github/workflows/ci.yml` 里同样写得下门禁序号，而它**一行都扫不到**
   * ——`scan-secrets.sh` 那两处就是手改的，写回去机器一声不吭。
   * 规则 E 的 docblock 原来只声明排除 `.github/` 与 `docs/`，读下来的结论是
   * 「`src`/`tests`/`scripts`/`admin-ui` 全被守住」，而那不是真的。
   *
   * `.yml` 那一半今天由 `tests/unit/scripts-guard.test.ts` 的
   * 「ci.yml 的注释行里不许写门禁的绝对序号（这道门禁的射程够不着 .yml）」补上；
   * `.sh` 那一半没有补，理由与去处写在 `scripts/check-comment-refs.mjs` 规则 E 那段里
   *（一句话：给 shell 单写一套抠注释判据 = 第二份抠注释真源，本仓已裁定过那是更坏的病）。
   */
  /**
   * ⚠️ **夹具里的注释刻意写成 `//` 而不是真 `.sh` / `.yml` 惯用的 `#`**，
   * 这一条是本轮自查抓到的：第一版夹具用 `#`，而 `commentBlocks()` 只认
   * `//` 与 `/* … *\/`——于是**就算 `walk()` 被放宽收了 `.sh`，这一格照样绿**，
   * 断言里那句「walk() 现在收这种后缀了？」是一句做不到的承诺。
   * 用 `//` 夹具才能把变量隔离到「后缀」这一层：实测把 `walk()` 的后缀表
   * 加上 `sh|yml` ⇒ 这一格当场红。
   *
   * **真实的射程是两层，两层都不在**：① 后缀不收；② 就算收了，`#` 那种注释
   * 方言也认不出来。放宽任何一层都不足以扫到 `scan-secrets.sh` 与 `ci.yml`。
   */
  it.each([
    [".sh", "scripts/x.sh"],
    [".yml", "scripts/x.yml"],
  ])("已知扫不到：%s 里的门禁序号 ⇒ 今天放行（边界是断言，不是散文）", (_ext, rel) => {
    const r = run({
      files: { [rel]: "// 这条由第 12 道门禁当场抓住。\n", "src/a.ts": "export const a = 1;\n" },
    });
    expect(
      r.status,
      "walk() 现在收这种后缀了？把这一格挪走，并去 `scripts/check-comment-refs.mjs` "
      + "规则 E 那段「这条规则够不着的地方」改掉",
    ).toBe(0);
  });

  /**
   * **第二层：注释方言。** 上一格隔离的是后缀，这一格隔离的是 `commentBlocks()`
   * 认得哪几种注释——`.ts` 文件里写成 `#` 的那一行同样是隐身的。
   * 两格合起来才说得清「为什么 `scan-secrets.sh` 与 `ci.yml` 手改了也没人吭声」。
   */
  it("已知扫不到：`#` 那种注释方言 ⇒ 今天放行（边界是断言，不是散文）", () => {
    const r = run({ files: { "src/a.ts": "# 这条由第 12 道门禁当场抓住。\nexport const a = 1;\n" } });
    expect(
      r.status,
      "commentBlocks() 现在认 `#` 了？把这一格挪走，并去改规则 E 那段「这条规则够不着的地方」",
    ).toBe(0);
  });
});
...
describe("规则 F：注释里没人能核的裁定计数", () => {
  it.each([
    ...
    // ⚠️ **这一格是复评抓到的漏网（MED-2）**：第一版的字符类是
    // `[一二三四五六七八九十]`，**不含「两」**。而这条规则要防的正是
    // 「删干净之后重新长回来」，重新写时中文里最自然的数词恰恰就是「两」
    // ——`裁过两次` 当场放行、`裁过二次` 才红。一条防复发的规则漏掉了
    // 复发时最可能用的那个字，等于**在它最要紧的那条路上失明**。
    ["「两」那种数词也算", "// 那正是本仓已经裁过两次的那个形态。\n"],
...
`,
  ...
  /**
   * **规则 F 的能力边界，同样钉成断言**（复评 MED-4 的同族：规则 E 有这张表，
   * 规则 F 上一版一张都没有，只在 docblock 里写成散文 —— **同一条纪律在相邻
   * 两条规则上执行不一致**，而散文那一半不会自己红）。
   *
   * 判据只认「裁过 + 数量 + 次/回」这一种句式，换个动词或量词就绕过去了。
   * 这不是"还没做完"：收进「裁定过 N 轮」「判过 N 回合」要么靠一张动词名册
   * （会漂），要么放宽到「任何动词 + 数量 + 量词」（当场误伤本仓大量正当计数句）。
   * **哪天判据放宽收进了其中一条，这一格会变红**，逼人把它从表里挪走并去改
   * `scripts/check-comment-refs.mjs` 规则 F 那段「它认不得什么」。
   */
  it.each([
    ["换动词：「裁定过 N 轮」", "// 那正是本仓已经裁定过三轮的那个形态。\n"],
    ["换动词：「判过 N 回合」", "// 那条边界判过两回合了。\n"],
    ["换量词：「裁过 N 遍」", "// 那正是本仓已经裁过三遍的那个形态。\n"],
    ["换时态：「裁了 N 次」", "// 那正是本仓已经裁了三次的那个形态。\n"],
  ])("已知认不得：%s ⇒ 今天放行（边界是断言，不是散文）", (_label, body) => {
    const r = run({ files: { "src/a.ts": `${body}export const a = 1;\n` } });
    expect(
      r.status,
      "这种写法现在被认出来了？把它从这张表里挪走，并去 `scripts/check-comment-refs.mjs` "
      + "规则 F 那段把「它认不得什么」改掉",
    ).toBe(0);
  });
});
...
/**
 * ── 规则 E / F 的报文指到**命中那一行**，不是整段的段首（复评 LOW-1）─────────
 *
 * 本仓的文件头 docblock 动辄四十行，报段首等于让人自己进段里翻。
 * 规则 E/F 的命中是一个短字面量、落点唯一，报得准就该报准
 * ——**报文是唯一会被看见的护栏**，一条指偏的报文会让人先怀疑量具。
 *
 * ⚠️ 规则 A/B 仍报段首，那是既有约定，不在这一轮里改：它们的命中是路径与锚，
 * 同一段话里常常跨行复述同一个路径，"哪一行"本身没有唯一答案。
 */
describe("规则 E / F：报文指向命中行", () => {
  it("命中在 docblock 中间时，报的是那一行而不是段首行", () => {
    const r = run({
      files: {
        "src/a.ts": `/**
 * 第一行说明。
...
`,
    ...
    expect(r.status, r.stdout).toBe(1);
    expect(r.stderr, "命中在第 4 行，报段首(:1)等于让人自己进段里翻").toContain("src/a.ts:4");
  });

  it("同一段里写了两遍，两条报文各指各的行", () => {
    const r = run({
      files: {
        "src/a.ts": `/**
 * 那正是本仓已经裁过三次的那个形态。
...
`,
    ...
    expect(r.status, r.stdout).toBe(1);
    expect(r.stderr, "第一条命中在第 2 行").toContain("src/a.ts:2");
    expect(
      r.stderr,
      "第二条命中在第 4 行——不带出现序号的话，两条报文会一起指到第一条那里去",
    ).toContain("src/a.ts:4");
  });

  /**
   * **认不出来时退回段首，不许猜。** 命中是在压平后的整段上匹配的，一条写法上
   * 跨了行的命中在任何**单行**里都找不到。那时必须退化成旧口径（段首），
   * 而不是报一个凑出来的行号——**报错的行号比不报行号更能把人引进坑**。
   */
  it("跨行写的命中退回段首行，不报错行号", () => {
    const r = run({
      files: {
        "src/a.ts": `/**
 * 占位说明。
...
`,
    ...
    expect(r.status, r.stdout).toBe(1);
    expect(r.stderr, "跨行命中在任何单行里都找不到，必须退回段首(:1)").toContain("src/a.ts:1");
  });
});
...
const COVERED: ReadonlyArray<{ claim: string; why: string }> = [
  ...
  // ⚠️ 上一版这条的理由写着「且是本仓最常用的说法」，**复评当场数了全仓行数、实测为假**
  //    （实测数留在那一轮的报告里，不写回这里：一个没有消费者的计数迟早会漂）。
  //    它真正的性质是**误报面最大**——那一条写在门禁源码 `CLAIM_MARKERS` 那一段里，
  //    代价由下面「按整段连坐」那格钉着。
  { claim: "那条链由 `tests/x.test.ts` 里那一整份用例负责。", why: "由 X 负责（与「由 X 保证」同型）" },
];
...
describe("scripts/check-comment-refs.mjs 元测试：规则 B（带断言性措辞就必须给得出指向）", () => {
   ...
   *
   * ⚠️ **这一格是评审逼出来的，成因如实登记**：判据原来是「整份文件压平后子串
  ...
  /**
   * **登记在案的漏报边界**（回填当场用真夹具量出来的）。
   *
   * 规则 B 的粒度是「一段」——`CLAIM_MARKERS` 与裸路径**落在同一段**才会撞上。
   * 上面那格证明紧邻的两行 `//` 会被 `mergeAdjacent()` 合成同一段；而**块与块之间
   * 不合段**，于是把措辞和路径拆进两个块就绕过去了。
   *
   * ⚠️ **这一格不是在说这样是对的，是在把今天的行为钉住**（同上面「登记在案的射程
   * 边界」那一格的用法）：`scripts/check-comment-refs.mjs` 文件头把「连坐的代价是误报
   * 不是漏报」写成了一句无条件的话，复评点名它没覆盖跨段拆分这条路径。现在那句话
   * 加了「只在一段之内」的限定，**限定的真伪就靠这一格**——哪天规则 B 被收紧成跨块
   * 也看得见，下面两条会当场变红，逼下一个人回去改那段说明。
   *
   * ⚠️ **三条夹具只差「怎么断开」，措辞与路径一个字都不差**：不这么派生的话，
   * 红/绿的差别可能来自别处（少了个反引号、换了个措辞），这一格就什么都证不了。
   */
  it("登记在案的漏报边界：把断言性措辞与裸路径拆进两个注释块 ⇒ 今天绕得过去", () => {
    const claim = "// 这件事已经钉住了。";
    const bare = "// 对应的那份是 `tests/x.test.ts`。";
    const on = (body: string) => run({ files: { "tests/x.test.ts": TARGET, "src/a.ts": `${body}export const a = 1;\n` } });

    // 控制端：同样两行、紧邻 ⇒ 合成一段 ⇒ 照旧红。它证明红/绿的差别只来自「怎么断开」。
    const merged = on(`${claim}\n${bare}\n`);
    expect(merged.status, "紧邻两行 `//` 不再被合成同一段 —— 那条射程漂了，下面两条也就不作数了").toBe(1);
    expect(merged.stderr).toContain("只给了裸文件名");

    // 漏报①：中间空一个源码行 ⇒ 两个块，带路径的那个块里没有措辞。
    expect(on(`${claim}\n\n${bare}\n`).status,
      "空行分隔的两段 `//` 现在被抓住了 —— 盲点补上了是好事，回去改 check-comment-refs.mjs 文件头那段登记").toBe(0);

    // 漏报②：docblock 之后紧跟一段 `//`（两种方言不合段）。措辞仍是同一句，只换方言。
    expect(on(`/**\n * ${claim.slice(3)}\n */\n${bare}\n`).status,
      "docblock 后紧跟的 `//` 现在被抓住了 —— 同上，回去改那段登记").toBe(0);
  });
   ...
   * **登记在案的射程边界**（回填当场实测出来的）。
   *
   * 规则 B 的「带没带锚」走的是 `nameAnchorsAfter(flat, 文件名)`——它在**整段**里找
   * 那个文件名的**每一次**出现，把后面跟着的锚全收上来。于是判据实际问的是
   * 「这一段里这个文件**有没有**带锚的提法」，**不是**「眼下这一次提法带没带锚」。
   *
   * ⚠️ **这一格不是在说这样是对的，是在把今天的行为钉住。** 发现它的过程正是本轮
   * 差点写下的第二句假话：`scripts/check-i18n.mjs` 文件头本来要写「少一个锚当场
   * exit 1」，实测把那一段第一处的锚删掉 ⇒ **EXIT=0**。
   * 哪天有人把规则 B 收紧成逐处校验，这一格会当场变红，逼他回来改那两段说明。
   *
   * ⚠️ **收紧不是零成本**：本轮实测过，改成逐处校验会当场打红一批今天合规的写法
   *（实测清单留在那一轮的报告里，不写回这里——一个没有消费者的计数迟早会漂）。
   * 那是一整个任务，不是顺手改一行。
   */
  it("登记在案的射程边界：同一段里同一个文件，有一处带锚就放行其余几处", () => {
    const bare = " * 另外 `tests/x.test.ts` 也钉着别的事（这一处只有裸文件名）。\n";
    const anchored = " * 这条由 `tests/x.test.ts` 的「被指向的那一格」钉着。\n";
    const r = run({
      files: {
        "tests/x.test.ts": TARGET,
        "src/a.ts": `/**\n${anchored}${bare} */\nexport const a = 1;\n`,
    ...
    expect(
      r.status,
      "规则 B 变成逐处校验了？那是好事，但请回来改 `scripts/check-comment-refs.mjs`"
      + " 文件头与 `scripts/check-i18n.mjs` 文件头那两段边界说明，别让它们继续说旧话",
    ).toBe(0);

    // **反向控制**：把那唯一一处锚也拿掉，同一段当场红。
    // 少了它，一个「这一段整个看不见」的实现同样能让上面那格绿。
    const noAnchor = run({
      files: {
        "tests/x.test.ts": TARGET,
        "src/a.ts": `/**\n * 这条由 \`tests/x.test.ts\` 钉着。\n${bare} */\nexport const a = 1;\n`,
      },
    });
    expect(noAnchor.status, "一处锚都没有时也放行 ⇒ 上面那格绿得没有意义").toBe(1);
    expect(noAnchor.stderr).toContain("只给了裸文件名");
  });

  /**
   * **规则 B 的报文必须同时给出第二条出路**（复评 [中]）。
   *
   * 命中是**按整段**算的，于是一句纯描述性的「本模块只负责 X」会连坐同一段里一条
   * 纯描述性的指向。上一版报文只给「去把行号或用例名找出来」一条路，撞上连坐的人
   * 就只剩「给一句描述句硬安一个锚」——**Task 15A 本轮那处假锚正是这么来的**。
   *
   * ⚠️ 与 `规则 E 的假红有正当出路：报文必须给出段级豁免` 是同一族、不是同一条：
   * 那一条的出路是**加豁免标记**，这一条的出路是**改那句描述**（不加豁免）。
   * 两条都遵守同一句：**报文说的那条路必须真的走得通**，只写在报文里、实际走不通
   * 比不写更糟。
   */
  it("规则 B 的假红有正当出路：报文必须给出「改那句描述」", () => {
    const desc = "本模块只负责组装报文，形态照抄 `tests/x.test.ts` 的 `fakeCtx()`。";
    const red = run({
      files: {
        "tests/x.test.ts": TARGET,
        "src/a.ts": `/** ${desc} */\nexport const a = 1;\n`,
    ...
    expect(red.status, "这一支现在不连坐了？那就把这一格改成反向控制，并去改「按整段连坐」那段边界说明")
      .toBe(1);
    expect(
      red.stderr,
      "报文只给「去找一个锚」一条路 ⇒ 撞上连坐的人只剩「给一句描述句硬安一个锚」，"
      + "而硬安出来的锚指的往往不是它在说的那件事",
    ).toContain("不要给它硬安一个锚");

    // 报文说的那条路必须真的走得通——把那句描述里的断言性措辞改掉，同一段就不再红。
    const rewritten = desc.replace("只负责组装", "只组装");
    expect(rewritten, "夹具没被真的改写").not.toBe(desc);
    const out = run({
      files: {
        "tests/x.test.ts": TARGET,
        "src/a.ts": `/** ${rewritten} */\nexport const a = 1;\n`,
 ...
 * **下一个人加了一个新说法，清单照样不会响。** Task 15A 补否定式一族时，
 * 缺的正是这一层——`拦得住` / `抓得住` 在表里躺了几期，而 `拦不住` / `抓不住`
 * 一个都不在，**没有任何东西为这件事红过**。
 *
 * ⇒ 这一组守卫，三层，各管各的一件事：
 * ① **结构不变量**：凡是收了 `X得住`，就必须同时收 `X不住`；
 * ② **完备性**：每个 marker 都得有一个 `COVERED` 标本，且没有空转的标本
 *（复评补的：少了它，往词表里加一个新词全仓零红）；
 * ③ **行为兜底**：抠出来的每个词逐个喂进真门禁跑一遍，认得才算数。
 * 三层都从**门禁源码里那张表**逐字抠出来再自己推导，不是抄一份期望值
 *（回填出来的期望值恒等于实际值，那条断言永远绿——本仓登记的第 6 种假阳性）。
 *
 * ⚠️ **它们能与不能，都在这里说清**：①②③ 合起来守的是「这张表内部自洽、
 * 且真的是门禁在用的那张」，**守不了「有人发明了一个既不是 `X得住` 也不是 `X不住`
 * 的新说法、而且干脆不往表里加」**——那需要读得懂中文的判据，本仓没有。
 * 那一半仍然由评审与变异验证承担。
 */

/**
 * 从门禁源码里逐字抠出 `CLAIM_MARKERS`。
 *
 * ⚠️⚠️ **第一版「抠到一半」时会静默放行，正是本仓反复裁过的那同一种死法**
 *（「判据用错工具时不会报错，会静静地放行」。复评实测，
 * 下面两条真实写法各让封闭不变量假绿一次）：
 * · 只按「整行以 `//` 开头」过滤注释 ⇒ 行尾注释里的 `"…"` 串被当成表成员
 *   （`"挡得住", // 与 "挡不住" 成对` ⇒ 凭空多出一个 `挡不住`，封闭不变量绿）；
 * · 只认双引号 ⇒ 单引号写的成员被漏掉（`'挡得住'` ⇒ 封闭不变量看不见它，
 *   **而真门禁认得它**，这一半更危险：机器全绿而射程真的变了）。
 *
 * 现在三条一起治：
 ...
 *    此时**抛异常，不许当成「今天没有那个词」**。
 *
 * ⚠️ **这三条仍然是「文本形状」层面的**。真正把它钉到行为上的是下面那格
 * 「抠出来的每个词，真门禁当场都认得」——它把抠出来的词逐个喂进真门禁跑一遍。
 */
function claimMarkersFromSource(raw: string = readFileSync(SCRIPT, "utf8")): string[] {
  const src = blankComments(raw);
  const HEAD = "const CLAIM_MARKERS = [";
  const start = src.indexOf(HEAD);
  const end = src.indexOf("\n];", start);
  // **认不出要吵，不能装没看见**：抠不出来时静默返回空表的话，下面几格会一起变绿。
  if (start === -1 || end === -1) {
  ...
  if (residue !== "") {
    throw new Error(
      `\`CLAIM_MARKERS\` 里有一段没被认出来：「${residue}」。`
      + "抠表判据只认三种引号写的字面量成员，认不出的写法一律抛——"
      + "静默跳过它就等于封闭不变量与完备性不变量对那个成员集体失明",
    );
  }
  return words;
}
...
describe("CLAIM_MARKERS 这张手写清单自己的守卫", () => {
   ...
   *（本仓评审发现 M1 的原形：判据认不出任何东西 ⇒ 真仓全变绿，只有反向控制红）。
   */
  ...
  it("反向控制：抠出来的确实是那张词表（认不出会抛，不会静默放行）", () => {
    ...
    expect(words, "后补的否定式没被抠到").toContain("拦不住");
    expect(words, "后补的归属式没被抠到").toContain("负责");
  });

  /**
   * **抠表判据自己的三条反向控制**（复评：上一版这三种写法全部假绿）。
   *
   * 夹具是**合成的最小表**，不是去改真门禁源码：这一格答的是「抠表这个判据认不认得
   * 出这几种写法」，与真仓今天写成什么样无关。它与上面那格是两件事——上面那格答的是
   * 「抠到的是不是真的那张表」。
   */
  it("反向控制：抠表判据对三种真实写法都不放行（行尾注释 / 单引号 / 认不出的写法）", () => {
    const table = (members: string) => `const CLAIM_MARKERS = [\n${members}\n];\n`;

    // ① 行尾注释里的引号串**不是**表成员。上一版按「整行以 // 开头」过滤，于是
    //    `挡不住` 凭空进了表，封闭不变量当场假绿。
    expect(
      claimMarkersFromSource(table('  "挡得住", // 与 "挡不住" 成对，下一轮补')),
      "行尾注释里的词被当成了表成员 —— 封闭不变量会因此假绿",
    ).toEqual(["挡得住"]);

    // ② 单引号写的成员**是**表成员（真门禁认得它）。上一版只认双引号，于是这种写法
    //    在封闭不变量眼里根本不存在，而射程真的变大了。
    expect(
      claimMarkersFromSource(table("  '挡得住', \"挡不住\",")),
      "单引号写的成员没被抠到 —— 真门禁认得它，这张不变量却看不见",
    ).toEqual(["挡得住", "挡不住"]);

    // ③ 认不出的写法要**吵**，不许静默跳过（跳过 = 对那个成员集体失明）。
    expect(
      () => claimMarkersFromSource(table('  "钉", ...LEGACY_MARKERS,')),
      "认不出的成员写法被静默跳过了 —— 「认不出要吵」只做了一半",
    ).toThrow(/没被认出来/);
  });

  /**
   * **把抠表钉到真门禁的行为上，而不是钉到文本形状上**（复评的建议）。
   *
   * 上面几格全部建立在「我从源码里读到的这张表 = 门禁真正在用的那张表」这个假设上。
   * 那个假设本身没有任何东西守着：`indexOf("const CLAIM_MARKERS = [")` 找错一张表
   *（比如有人留了一张同名的旧表在前面）、或者抠出一个门禁根本不认的词，
   * 上面三条不变量照样绿，而它们守的其实是一张幽灵表。
   *
   * 这一格把抠出来的**每一个**词各自造一段注释喂进真门禁跑一遍（**一次进程**，
   * 每个词一个文件），要求**每个文件都被报出来**——报不出来就说明那个词门禁不认得。
   * ⚠️ 不能改成「按词名断言报文内容」：报文给的是 `CLAIM_MARKERS.find(…)` 命中的
   * **第一个**词，而 `钉` 是 `钉住`/`钉着`/`钉死` 的子串，那样断言会永远拿到 `钉`。
   */
  it("抠出来的每个词，真门禁当场都认得（不是照着文本形状猜的）", () => {
    const words = claimMarkersFromSource();
    const files: Record<string, string> = { "tests/x.test.ts": TARGET };
    words.forEach((w, i) => {
      files[`src/m${i}.ts`] = `/** 这条由 \`tests/x.test.ts\` ${w}。 */\nexport const a${i} = 1;\n`;
    });
    const r = run({ files });
    expect(r.status, r.stderr).toBe(1);
    const unseen = words.filter((_, i) => !r.stderr.includes(`src/m${i}.ts`));
    expect(
      unseen,
      "这几个词抠出来了，可真门禁拿它们一句都不报 ⇒ 抠到的不是门禁在用的那张表",
    ).toEqual([]);
  });

  /**
   * **`COVERED` 对词表的完备性**（复评 [中低]）。
   *
   * `CLAIM_MARKERS` 上方逐字写着那两张表「逐条钉着它」。复评实测那句话当时是散文：
   * 往词表里加一个 `"保障"`，全仓 **零红**——`COVERED` 里没有它的标本，而没有标本
   * 就意味着**没有任何一格证明门禁真的认得它**，也没有任何一格证明「带了锚就不许红」
   * 对它成立（那张派生表是从 `COVERED` 长出来的）。
   *
   * ⇒ 每个 marker 必须至少有一个 `COVERED` 标本。反方向那一半也一起钉：
   * 一条谁都不命中的 `COVERED` 标本是**空转**的夹具，它跑得再绿也什么都不证明。
   *
   * ⚠️ **它守不了「标本写得像不像真的」**：`{ claim: "…钉住。" }` 与本仓真实写法
   * 长得像不像，仍然只能靠评审。这一格只保证「一个都不许漏、一条都不许空转」。
   */
  it("`COVERED` 对词表完备：每个 marker 都得有标本，且没有空转的标本", () => {
    const markers = claimMarkersFromSource();
    expect(
      markers.filter((m) => !COVERED.some((c) => c.claim.includes(m))),
      "这几个措辞进了词表却没有 COVERED 标本 ⇒ 没有任何一格证明门禁真的认得它们，"
      + "也没有任何一格证明「带了锚就不许红」对它们成立",
    ).toEqual([]);
    expect(
      COVERED.filter((c) => !markers.some((m) => c.claim.includes(m))).map((c) => c.why),
      "这几条标本一个 marker 都不命中 ⇒ 它们是空转的夹具（正向那格会当场红，"
      + "但真正该改的是这里：要么把词收进表，要么把标本删掉）",
    ).toEqual([]);
  });
...
describe("scripts/check-comment-refs.mjs 元测试：豁免标记与它的边界", () => {
  ...
  /**
   * **正则字面量里的引号不许把后面的内容吞掉。**
   * ⚠️ 这一格是修那条时**踩出来的**：第一版的修法带了跨行的模板字面量状态，
  ...
  /**
   * ── 第二种「不止一个锚」的形态：**一条路径后面连着写好几个并列锚**（后补的） ──
   *
...
describe("本仓 @refs-ignore 的使用处，逐条列名", () => {
  it("豁免清单与手写的这份一致", () => {
    ...
    expect(used, "有人加了新的 @refs-ignore 豁免——请在这里表态说明理由").toEqual([
      ...
      // 那正是它们在讲的事。
      // ⚠️ **这里从 3 条减到 2 条，是修那条（扫描器不解析字符串字面量）的直接后果**：
      ...
      "scripts/check-comment-refs.mjs",
      // ⚠️ **这里从 3 条减回 2 条，是补进否定式与归属式之后的直接后果。**
      // 减掉的那一条是登记「词表漏了否定式一族」时开的：那段里有两种
