 ...
 *   被章节标题常量表的 `## 📡 API エンドポイント` 取代），只剩 ADMIN 一份用 `早見表` ⇒ 不再打架。
 ...
 *
 * ── 五份 `USAGE.md` 从 95–103 行扩到 334–394 行那一批动了哪六行 ──
...
const PENDING_TERM_CONFLICTS: readonly string[] = [
  // ⚠️ 后来新增 `README=repository` 那一格：一键部署按钮下方那句「它替不了你两件事」的
  ...
  // ⚠️ 排障那一条（ADJ ㊷）把 ADMIN 这一侧从 `ドキュメント+文書` 收敛成 `文書`：
...
describe("五语言术语表：同一源词在同一语言的不同文档里必须用同一译词", () => {
...
describe("术语表判官的反向控制（换掉读文件那一步，注入夹具）", () => {
  /** 在真文本上做一次定点替换，其余原样。 */
  const patched = (
