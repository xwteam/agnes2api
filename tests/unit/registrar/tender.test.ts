...
//（vitest 的 esbuild 转换不做类型检查，只有 tsc --noEmit 会揪出）。
...
describe("tendOnce", () => {
  ...
  /**
   * **一条通道失败时绝不会去碰另一条**（两条通道二选一，没有主备、没有自动降级）。
   *
   * ⚠️ **另一条通道必须真的注入进去**：不注入的话这一格测的是「没有 provider」，
   * 而不是「有 provider 但不去用它」——那是两件事，后者才是本轮要钉住的行为。
   * ⚠️ **判据是调用计数，不是返回值**：一个「调了但把异常吞掉」的实现能从返回值
   * 那边蒙混过去。
   */
  function spyProvider(name: Channel, calls: { n: number }): MailProvider {
    const bump = (): never => { calls.n++; throw new Error(`${name} 不该被调到`); };
    return {
      name: name as "yyds",
      async listDomains(): Promise<string[]> { return bump(); },
  ...
  it("一次通道级失败（provider_error）绝不碰另一条通道", async () => {
    ...
    deps.providers = { yyds: broken, moemail: spyProvider("moemail", other) };
    const out = await tendOnce(deps);
    expect(out.minted).toBe(0);
    expect(out.mintedByChannel).toEqual({});
    expect(out.failures).toEqual([
      { reason: "provider_error", channel: "yyds" },
      { reason: "provider_error", channel: "yyds" },
    ]);
    expect(other.n, "另一条通道被碰了 —— 自动降级又回来了").toBe(0);
    expect(await repo.all()).toHaveLength(0);
  });

  it("收不到验证码（code_timeout）同样绝不碰另一条通道 —— 与上一条是 switch 里两个不同的 case", async () => {
    // **必须单独写**：`code_timeout` 与 `provider_error` 在那个 switch 里是两支，
    // 只改一支另一支会静默留着降级。
    const dead = new FakeMailProvider({ domains: ["a.test", "b.test"], code: null });
    const other = { n: 0 };
    const { deps } = await makeDeps({ channel: "yyds", targetKeys: 2, mintBatch: 2 });
    deps.providers = { yyds: dead, moemail: spyProvider("moemail", other) };
    const out = await tendOnce(deps);
    expect(out.minted).toBe(0);
    expect(out.mintedByChannel).toEqual({});
    expect(out.failures).toEqual([
      { reason: "code_timeout", channel: "yyds" },
      { reason: "code_timeout", channel: "yyds" },
    ]);
    expect(other.n, "另一条通道被碰了 —— 自动降级又回来了").toBe(0);
  });

  it("反向控制：选中 moemail 时失败，同样绝不碰 yyds —— 两条通道一视同仁", async () => {
    // 两条方向都钉住才算守住「平级」：只写一条方向的话，一份「只对 yyds 生效」的
    // 实现照样全绿。
    const broken: MailProvider = {
      name: "moemail" as const,
      async listDomains(): Promise<string[]> { throw new Error("down"); },
      async createMailbox() { throw new Error("x"); },
      async pollCode() { return null; },
      async deleteMailbox() {},
    ...
    const other = { n: 0 };
    const { deps } = await makeDeps({
      channel: "moemail", targetKeys: 1, mintBatch: 1,
      moemail: { baseUrl: "https://m.test", apiKey: "k" },
    });
    deps.providers = { moemail: broken, yyds: spyProvider("yyds", other) };
    const out = await tendOnce(deps);
    expect(out.minted).toBe(0);
    expect(out.mintedByChannel).toEqual({});
    expect(out.failures).toEqual([{ reason: "provider_error", channel: "moemail" }]);
    expect(other.n, "另一条通道被碰了 —— 平级这条只守住了一半").toBe(0);
  });

  it("收不到验证码时不会在通道内换域名死等（单次尝试只烧一份 codeTimeoutMs 的预算）", async () => {
    // 不能顺手把「逐个域名重试」带进来——那会让单次铸 key 最坏耗时变成
    // MAX_DOMAIN_ATTEMPTS × CODE_TIMEOUT_MS，远超 Worker Cron 的 900 秒墙钟
    //（详见 tender.ts 那个 switch 上面的注释）。
    // 给 5 个域名、验证码永远收不到：pollCode 只该被调 1 次。
    let polls = 0;
    const dead = new FakeMailProvider({
      domains: ["a.test", "b.test", "c.test", "d.test", "e.test"], code: null,
    });
    dead.pollCode = async () => { polls++; return null; };
    const { deps } = await makeDeps({ targetKeys: 1 }, dead);
    const out = await tendOnce(deps);
    expect(polls).toBe(1);
    expect(out.minted).toBe(0);
  });

  it("收不到验证码：本次名额作废，但剩余名额继续尝试（不整轮中止）", async () => {
    // 没有出口，但也不该把整轮砍掉——下一次尝试会重新洗牌域名，
  ...
  it("网络层错误不让整轮 reject：TendResult 照常返回，剩余名额继续尝试，且不碰另一条通道", async () => {
    // 五处 fetcher.fetch 任何一处 reject 此前都会穿透 mintOne → tendOnce 整轮
    // reject，剩余名额作废、TendResult（面板要展示的数据）也拿不到。
    const provider = new FakeMailProvider({ domains: ["x.test"] });
    const backup = new FakeMailProvider();
    const { repo, deps } = await makeDeps({ targetKeys: 3 }, provider);
    deps.providers = { yyds: provider, moemail: backup };
    deps.agnes = {
      platformUrl: "https://platform.test",
  ...
  // 且零日志 → 活跃邮箱配额被吃光 → 建邮箱一律失败。那一轮把 code_timeout 改成通道级
  ...
  it("预算只够 2 次尝试而 mintBatch=5 时：只尝试 2 次，且两次都完整跑完（邮箱都删了）", async () => {
    ...
    // 单次最坏 = codeTimeoutMs = 5000。预算 12000 只装得下 2 次
  ...
  it("通道缺 provider 时记录一条失败，而不是静默空转", async () => {
    // 选中的是 yyds，但 providers 里根本没构造它——这是接线错误
