...
describe("nextBackoff：指数、封顶、以及「一串」是怎么算的", () => {
  /**
   * ⚠️ **三档各有自己的基数，逐档手写字面量。** `cluster` 那一档按的是**这一轮的形状**
   *（同一轮里好几个域名被判屏蔽、或者上游列出来的域名一个不落全被判屏蔽，且这一轮零产出，
   * 见 `src/core/registrar/tender.ts` 的 `finishRound`），取值刻意与 `app` 相同 —— 但**取值相同不等于可以共用一个常量**：
