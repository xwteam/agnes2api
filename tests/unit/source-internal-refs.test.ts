 ...
 * 🔴 **这一段是本文件对外宣称的射程边界，漏一条就等于把射程说大一分。六条，一条都不许省。**
 ...
 * ⑥ **中间不是连字号、而是井号的同类编号，一族都抓不到。** 七族认的中缀只有两种：
 *    「什么都没有」（字母紧跟数字）与「一个连字号」；井号在中间的写法从每一族底下整批溜过去。
 *    同样是实测：本轮之前仓里真有两份文件共四行写着这个形状，其中一份还是读者点得开的
 *    网关源码，而那份文件在本轮里**被改过、被人拿在手上**，判据仍然一格没红。
 *
 *    🔴 **⑤ ⑥ 这两条盲区底下今天挂着三种形状，每一种的存量都清成了零，
 *    而每一个零下面都有一格棘轮钉着**（「明写的两条盲区、三种形状」那一组）：明天再写一个，
 *    射程里当场红。⑤ 一条盲区占两行数据，因为它的两支各要一条自己认得出的形状。
 ...
 *    上面那 153 处、以及井号那一族在真实文本里的歧义面，都比今天在册的七族大；
 ...
 *
 * 🔴 **它们照样进扫描、照样在 `BASELINE` 里逐份点名**（今天各 129 与 86 处，七族合计）。
...
const SPECIMENS: ReadonlyArray<{
...
}> = [
  {
    ...
    why: "文档轴那份判据**自己的标本集**：它的族表、探针串、误报面登记，逐条都得写真串"
      ...
      + "⇒ 第二批字母那个零对它不成立（今天 28 处，七族合计 129 处）",
 ...
 *
 * 🔴 **这张表是一笔欠账的登记，不是许可。** 已经清干净的有两族：第二批字母那一族
 * （下面「新族恒为零」那一格钉着它今天在标本集之外**一处不剩**），
 * 以及「字母 + 连字号 + 字母」那一族——**本轮把它的存量清完了**，表里今天还带它的
 ...
 * 带连字号那一族的**存量本轮清完了**，第三批字母那一族的仍然欠着。
 *
...
const NEAR_MISSES: ReadonlyArray<{
  ...
  },
  {
    ...
    flags: "g",
    why: "七族认的中缀只有「什么都没有」与「一个连字号」两种，井号在中间的整批溜过去。"
