 ...
 *    收它要把尾巴放宽到任意大写字母数字，而那会把上面那批夹具串与探针常量名整批拖成假红。
 *    🔴 **别把这个零读成「多字母前缀那一支已经守住了」。** 今天没有任何一格钉着那个形状，
 *    明天照它再写一个，全仓一处都不响 —— 欠的是判据，不是存量。
 * ⑧ **下面点名的那几支写法，八族一处都抓不到，而且一格棘轮都没钉着**
 *    （存量逐支写在各自那一行上：有的这一轮清成了零，有的还是三位数）。
 ...
 *    **而且明天回潮当场红**；**这里这几支没有任何一格会替你红**——其中两支的存量
 *    这一轮清成了零，可那个零是人清出来的，明天再往注释里写一个，全仓一处都不响。
 *    · **裸写的内部评审轮次名**：那个三个大写字母的轮次缩写，后面直接跟空格、圈号或
 *      分节号，**不带任何编号中缀**。八族全部抓不到，而且不是勉强漏掉：
 *      每一族要么认「字母紧跟数字」、要么认「字母 + 一个连字号 + 一个字符」，
 *      而它身上压根没有数字、也没有中缀，一族都套不上。
 *      **今天射程内是零**：这一轮把 115 处 / 9 份逐处改写掉了（对照轴 64、排版轴 18、
 *      `scripts/prepush.sh` 13、偏离名册 11，其余五份各 1 到 3 处），
 *      编号背后那件事一律讲回句子里，讲当时实测的句子仍然讲当时。
 *      🔴 **这个零一格都没人守。** 清它的从头到尾是人，本判据一次都没红过；
 *      明天照那个形状再写一个，八族仍然一处都抓不到。**别把它读成已经收进判据了。**
 *      ⚠️ 这里按判官的**别名**点份，不写文件路径：这一段满是断言性措辞，
 *      而注释指向那道门禁要求「同一段里出现的测试文件必须带行号或用例名的锚」。
 *      这几份在这里只是**计数的落点**，不是「这句话由那一格守着」
 *      ⇒ 硬安一个锚就是造一条假指向，别去安。
 *      ⚠️ 同一个轮次名**带连字号**的那 3 处不算在这个数里——它们归上面第 ⑦ 条，
 *      两支合起来是 118 处 / 10 份，同一轮一起清的。⇒ 这两个数别相加着用。
 ...
 *      别把它数进来。
 *    · 附带一笔同性质的账：**仓里根本不存在的内部文档名**——读者点不开，也 grep 不到。
 *      它不带任何编号形状 ⇒ 归第 ① 条「认形状，不认语义」，八族同样一格不红。
 *      这一轮把 9 处 / 8 份真源连同上面那一支一起清了（面板真源、网关源码、契约测试、
 *      面板闸门测试、对照轴与排版轴各若干），其中面板那一处逐字节烧进过生成物、
 *      发给了浏览器 ⇒ 改真源之后重新生成，产物里跟着归零。
 *      🔴 **这个零同样一格没人守，而且靠八族天生守不住**：文件名身上没有编号形状。
 *      要守它得换一种判法——**扫出被引用的文件名，断言它在仓里真的解析得开**，
 *      而那一步先得给判据自己那批「按定义就该不存在」的夹具名列一张具名白名单
 *      （白名单还要反着查：指着一份真存在的文件的条目 = 名册过期，当场红）。
 *      **这一步本轮没做。** 别把这个零读成已经有东西守着。
 *    ⚠️ **为什么本轮清了存量、仍然没收族**：与第 ⑦ 条同一条路——收之前要把误报面
 *      喂真文件量出来。顺序这一半已经不欠了（先清存量、再收族，写在 `BASELINE`
 *      上面那段：反过来做会让逐份点名整片变红，那时把数字改大就是把一批存量
 *      登记成现状）；欠的是误报面那一半，以及上面那条结构化判法要的白名单。
 *      ⇒ 本条**只把话说准，不把射程说小**：清成零的是存量，不是盲区。
