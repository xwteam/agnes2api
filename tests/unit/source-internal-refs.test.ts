 ...
 *    ⇒ 这不是漏，是一笔明写在这里的取舍。
 * ⑤ **带连字号那一族只认连字号后面的「一个」字符 ⇒ 连字号后面跟两位数字的写法整批溜过去。**
 *    它的后瞻要求下一个字符不是字母数字下划线，而两位数的第二位正是数字 ⇒ 前一位那个匹配
 *    当场被后瞻否掉，整串一处不报。**这不是推出来的，是实测**：本轮之前仓里真有四处真源
 *    写着这个形状（其中三处在 `admin-ui/` 下，逐字节烧进生成物之后**发给了浏览器**），
 *    而**它们是被人看见的，不是被这份判据看见的**——判据从头到尾一格没红。
 ...
 *
 *    🔴 **⑤ ⑥ 这两种形状本轮都清成了零，而那个零下面有一格棘轮钉着**
 *    （「明写的两条盲区」那一组）：明天再写一个，射程里当场红。
 *    ⚠️ **别把那一格读成「这两种形状已经收进判据了」——它红出来只有一句「这个形状回来了」。**
 *    它们没有进 `FAMILIES`：⇒ 不进逐份点名与 `BASELINE`、不进带族名与行号的那套报文、
 *    不参与两轴对齐那一组，文档轴那边（44 份公开 markdown）也完全没有它们。
 *    ⚠️ **为什么不干脆收进族表**：两种形状的误报面都没实测过——连字号后面跟两位数字的
 *    同形真词（字符集编号、端口区间、日期区间那几类）与井号那一族在真实文本里的歧义面，
 *    都比今天在册的七族大；而本仓在「拿假红换覆盖面」上已经有一条明写的取舍（上面第 ④ 条）。
...
describe("明写的两条盲区：族表今天抓不到，而它们的存量已经是零", () => {
  const target = "src/core/config.ts";
