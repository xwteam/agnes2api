...
describe(".env.example 与真源对齐", () => {
   ...
   *（复评实测）。现在判据落在**指路本身**上——第一条声明之前的那段头部里，
 ...
 *
 * ⚠️ **分工（补漏评审登记的那条副作用）**：本仓有**两套**判据在看五语言
 ...
 *
 * ⚠️ **两份文档今天的排版不是同一种，这是裁定过的（「换形态不换内容」）**：
 ...
 * 把 REGISTRAR 的表换成围栏时，只认表格行的旧正则让五种语言一起返回空集，
 ...
 * `.env.example` 里没有的变量，这十句会在五种语言里同时变假而零红**（复评实测）。
 *
 * 三格覆盖三种漂移，各自的失败形态不同：
...
describe(".env.example 与五语言文档对等", () => {
  ...
  /**
   * **那一条的验收**：把锚点从 ```env 围栏里删掉 ⇒ 仍然必须红并点名该语言。
   *
...
/**
 * ## `.env.example` 的「不回退」下限
 *
 * 射程铁律里有一条**否定项**：25 份非 README 文档要补层级、补 alert、
 * 补 ```env 围栏，而 **`.env.example` 本身一个字都不改**（那条规格）。
 * 那条裁定在规格里被写了三遍，**却在清单、R 判官、排版基线三处都没有承载点**——
 ...
 *
 * ── 记账：197/139 → 219/160（2026-09-07）─────────────────────────────────
 * **这不是把不达标的数改成达标的数**——两条都是下限，文件长大了它们只会更容易过。
 * 逼着必须重新量的是下面「删掉 20 行 ⇒ 总行数那条红」那格探针：它删的是**固定 20 行**，
 * 而 `197` 是 2026-08-30 的量。文件从那天起长到 219 行之后，删 20 行只剩 199 ≥ 197
 *（实测：`expected 199 to be less than 197`）⇒ **探针不再证明任何事**，
 * 那才是真正坏掉的东西。重新量 = 让探针继续咬得住。
 * 这一轮长的 4 行全在 `USAGE_FLUSH_INTERVAL_MS` 那一段（把「破了启动就报错」改成
