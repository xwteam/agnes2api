 ...
 *    （下面每一格的 `assert`）。凡 R28 为止那一批判据覆盖不到、
 *    又属于「照抄即造假 / 照抄即出错」的内容点，一律必须在这里有一条。
 *    审查器构造过的反例就是这一档：
...
/**
 * 一条登记。
 * · `id`：与最初那张名册表的行号对齐，**编号不复用**（删掉一条也不把后面的号往前挪，
 *   否则历史里引用过的编号会指向别的东西）。
 * · `why`：为什么故意这么做。
 * · `until`：**什么时候这条登记该被删掉**。
...
const REGISTRY: readonly Deviation[] = [
  ...
  },
  {
    ...
    why: "裁定明令「不许为了凑满 10 行而编造版本条目」；这个仓才发到 v0.1.0",
  ...
  },
  {
    ...
    // 🔴 **这条只豁免排版轴，不豁免泄漏轴**：`what` 里那句「排版轴的射程」与 `why` 里那句
  ...
  {
    id: 18,
    what: "根级多两份参照仓没有的 dotfile（`.gitattributes` / `.npmrc`）；社区五份 md 的 `---` 只判「≥1 条」，不套 README 那套恒等式",
  ...
  },
  {
    ...
    why: "实测压到字面 0 需要全仓再插约 235 个标题，`###`+`####` 会涨到模板密度的 2.4 倍 —— 与「密度已达成并超过模板、不许再堆 `###`」那条裁定正面冲突。⇒ 判据取**绝对数棘轮**（只许降不许升），把差额如实记在这里",
...
describe("名册自守：编号不重、三样东西齐全、机器验不了的那条如实标注", () => {
  ...
  /**
   * 🔴 **第 17 条只豁免排版轴（那 16 节骨架），不豁免泄漏轴（不暴露内部路线图与阶段编号）。**
   *
   * 这条登记的原文写的是「移出**排版轴的射程**」，理由写的是「套 16 节骨架毫无意义」
   * —— 两句话说的都是**排版**。而泄漏轴是另一根轴，它一个字都没豁免过任何文档。
   ...
   * **且在泄漏射程内**（泄漏轴没有豁免它）。哪根轴被人挪到另一根上，这一格当场红。
  ...
  it("🔴 第 17 条只豁免排版轴：`admin-ui/README.md` 在排版射程外，但必须在泄漏射程内", () => {
    ...
    expect(publicDocs(), `${p} 不在泄漏射程里 —— 第 17 条那条**排版**豁免又被当成泄漏轴的豁免用了。`
      + "泄漏轴那条裁定没有豁免过任何一份公开 markdown，泄漏轴的射程必须是 `publicDocs()`").toContain(p);
  });

  it("规格名册里那两条**已经结清**的条目今天确实不在名册里（结清了就该删，不是留着当摆设）", () => {
    const ids = new Set(REGISTRY.map((d) => d.id));
    // 第 11 条：规格自己在一次修订里删掉了（「TIP 是两句而非一句」根本不构成偏离）。
