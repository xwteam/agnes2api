...
describe("面不许增长", () => {
  ...
  it("反向控制：判据在一段没有中文 message 的真代码上不乱红", () => {
    ...
    // 而下面这份名单恰好只有两条 —— 这正是本格要证的：判据没有宽到「见汉字就红」。
    //
    // ⚠️ **2026-09-10 先从「恒为空」改成「恰好一条」，同一天又改成「恰好这两条」，
    // 两次都不是把断言放宽。** 两条是同一个缺陷在两条协议上的同一个修法：
    // 上游中途断流时生成器从前**静默退出**、照常补一个收尾事件并声称正常结束
    // ⇒ **客户端把「被截断」当成「正常说完」**。
    // · anthropic 那条是 `event: error` 的载荷文案；
    // · responses 那条是 `response.failed` 里 `error.message` 的文案（同一天补的，
    //   在那之前 `/v1/responses` 断流照样发 `response.completed`）。
    // ⚠️ gemini 那条同一天也补了终帧，但它按 Gemini 协议只发 `finishReason: "OTHER"`、
    //   **不带任何文案** ⇒ 它不会出现在这份名单里，那是对的，不是漏扫。
    // 🔴 两条都**不进面板**（走的是 SSE 事件体，给的是 API 客户端，不是 `/admin` 那条
    // 渲染路径）⇒ 与上面 `ADMIN_MESSAGE_SITES` 的记账同一条口径：够不着屏幕的位置
    // 不发码、不新增要五语言维护的契约。
    // **列成逐字名单而不是放宽成「≤2 条」**：再多一条就会红，那正是这一族判据的意义。
    // ⚠️ **只钉「恰好几条 + 是哪个文件的哪句话」，刻意不钉行号。**
    // 行号会因为该文件上方**任何一次无关编辑**而漂（实测：加一行 import 就从 144 变 147），
    // 那是噪音不是信号。
    const sites = messageSites("src/core/protocol");
    expect(sites, "这个目录里冒出了第三条 HTTP 错误 message").toHaveLength(2);
    const joined = sites.join("\n");
    expect(joined).toContain("src/core/protocol/anthropic.ts");
    expect(joined).toContain("src/core/protocol/responses.ts");
    // 两条今天是同一句话（同一个失败讲给两种客户端听），所以只出现一次子串也算数——
    // 这里要求它出现**两次**，一条被改掉就红。
    expect(joined.split("上游流式响应中断，本次回答不完整").length - 1,
      "两条断流文案不再是同一句话了 —— 要么有一条被改了，要么有一条没了").toBe(2);
  });
});
