 ...
 * amend / 再重写一次历史，它们会**静默腐烂**，而 CI 那一串门禁一道都不会响。
 *
 * ⚠️ **本期不新增门禁编号**（`tests/unit/scripts-guard.test.ts`「CI 恰好十三道门，编号 1/13 到
 * 13/13 各出现一次」那一格钉着），所以这条守卫写成 `tests/unit/` 下的用例，
 ...
 *     ⚠️ **别把这条读成「纯十进制的串不可能是 sha 引用」——那是一句假全称句**，
 *     它只是**今天**碰巧成立：本仓在这个射程里实有 9 个纯十进制串（冷却毫秒数、
 *     `20260815` 这种日期、`1234567` 这种千分位夹具），眼下**一个都不是** sha 引用。
 *     ⚠️ **这个数完全取决于下一批 sha 长什么样，别把它当结论**：上一版这里实有**两个**
 *     真的 sha 引用，是第二轮历史重写把它们的新缩写换成了含 a–f 的写法才归零的。
 *     ⚠️ **所以这一格是一条真盲区，明写在这里**：一个**恰好不含 a–f** 的 7 位 sha 缩写，
 *     一旦腐烂就与一个普通十进制数**形状上无法区分**，靠形状永远认不出来。
 *     ⇒ 这一族不靠形状，而靠下面 `DECIMAL_SHA_REFS` 那张**双向校验**的登记表兜着。
 *
 * ── 三张表，每一张都有让它变红的路 ────────────────────────────────────────────
 *
 * · `DESTROYED_OBJECTS`：那次重写**有意销毁**的 blob，正文引着它们的 sha 作为史实记录。
 *   它们按构造解析不开，且**没有后继**（commit 有 commit-map 可映射，blob 没有），所以不可回填。
 *   会红的路有两条：条目在扫描结果里**找不到**了（正文改写过 ⇒ 名册发霉）；
 *   条目**居然解析得开**了（那句「已销毁」是假话）。
 *   ⚠️ **今天它是空的**：原来那三条的正文引用全部落在一批内部设计文档里，那批文档已移出本仓，
 *   三条随之一起删（连那条靠自引不发霉的也删了——正文引用没了，留着只是一颗定时炸弹）。
 *   两条路眼下都没东西可红；**上膛的是主扫描**——哪天再出现一个解析不开的 sha 引用，
 *   它会当场红并逼人连理由一起登记进这张表。**空表不等于空跑，但也别把它当成一格活着的断言**
 *  （与下面 `DECIMAL_SHA_REFS` 的空表同一条口径）。
 * · `DECIMAL_SHA_REFS`：纯十进制的 sha 引用登记表。
 *   会红的路有两条：条目解析不开了（sha 腐烂了）；扫描发现了**没登记**的同族条目。
 *   ⚠️ **今天它是空的**（见判据 3 的说明），所以第一条路眼下没东西可红；**上膛的是第二条**
 *   ——哪天有一处引用的新缩写恰好不含 a–f，扫描会当场把它报成「没登记」并逼人登记进来，
 *   这张表随即自己长出第一条路。**空表不等于空跑，但也别把它当成一格活着的断言。**
...
/**
 * 历史重写时**有意销毁**的对象。它们是 blob，没有后继 sha，机械回填这条路按构造不存在，
 * 所以只能连理由一起登记在这里，由 `offenders()` 当作解析不开的合法豁免。
 *
 * ⚠️ **这张表今天是空的，而且是刚刚被清空的，别把它读成「从来没有过条目」**：
 * 原来那三条（两枚泄漏载体 blob + 一枚第一版设计文档 blob）的**正文引用全部落在一批
 * 内部设计文档里**，那批文档已随本次改动移出本仓。正文一走，名册就发霉，所以三条一起删。
 * ⚠️ **连那条「靠自引不发霉」的也删了**：`shaRefsIn()` 的 `GLUE` 含引号，被双引号包住的
 * token 不算引用，于是那条裸标识符键当时是**自己引着自己**才没红——正文引用没了之后，
 * 它留着只是一颗定时炸弹（哪天有人把它改成带引号的写法就当场发霉）。
 *
 * ⚠️ **空表意味着下面 (b) 那一格今天是空集比空集**——它跑了等于没跑，
 * 那一格的绿不作数（口径与 `DECIMAL_SHA_REFS` 的空表逐字相同）。
 * 上膛的是 (a) 那一格：没有任何豁免之后，任何一处解析不开的 sha 引用都会当场红。
 */
 ...
 *
 * ⚠️ **今天为空，而且这是量出来的不是想出来的**：第二轮历史重写之后，全 tracked 文件里
 * 24 个提交 sha 引用**每一个的新缩写都含 a–f**，射程内剩下的 9 个纯十进制串一个都不是
 * sha 引用。空表不许读成「这一族不存在了」——它随时会因为下一个缩写恰好没有 a–f 而回来，
 * 那时 (c) 的「表外同族」那一半会先红。
 */
const DECIMAL_SHA_REFS: string[] = [];
