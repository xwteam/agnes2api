 ...
 *   写明它住在哪儿。
 *   R22a/b/c/d/e/e2 同理：分隔行补齐 / 列表嵌套 / 表格长度三组已经在那份文件里跑着，
 ...
 *   是另一把尺，两者在 CJK 上差一倍，谁也不能顶替谁）。
 *
 * ── 它整体验不了什么 ──────────────────────────────────────────────────────
 * 一份文档**写得好不好**。本文件全部是形态判据：alert 在不在该在的位置、
 * 标题之间有多长、表格的必填格是不是二值。**alert 选的类型贴不贴合语义、
...
// 评审发现：`includes` 型判定之前必须先做载体过滤。围栏本来就剥，
 ...
 * ⚠️ **实现挪到了 `tests/helpers/ship-docs.ts`**（评审发现）：
...
/** 社区文件：`---` 只判「≥1 条」，不套 README 那套恒等式（裁定如此）。 */
...
describe("射程自守：40 份出货文档、逐份读得到，分组不重不漏", () => {
  it("射程 40 份，且 README/SPONSORS/非 README 三组加起来正好盖住 `docs/` 那 35 份", () => {
    expect(SHIP_DOCS.length, `出货文档从 40 份变成了 ${SHIP_DOCS.length} 份 —— 射程是从磁盘现算的，`
      + "数变了就该有人来确认新增/删除的那份该不该进射程（这个数是裁定过的）")
      .toBe(40);
 ...
 * **它验不了什么**：那一条 alert 选的 TYPE 贴不贴合语义；也不管全仓 alert 总数。
 * ────────────────────────────────────────────────────────────────────────── */
...
describe("R20/P2 `<details>` 的位置与数量（README 这一半）", () => {
  ...
  it("② 五份语言版 README 的同一节 0 个 `<details>`（平局裁决，登记在名册第 19 条）", () => {
...
describe("R20/P4 分隔线的位置（README / SPONSORS / 25 份 / 社区文件四档，剥围栏后统计）", () => {
  ...
  it("② 六份 SPONSORS.md：`hr-before-h2` 100%，`^---$` 总数 == 节数（**不是** 节数 + 2）", () => {
  ...
  it("③ 25 份非 README 的那一档由 `docs-parity.test.ts` 的「位置不对的 `---`」那一组守着 —— 这里只做交接控制", () => {
 ...
 * R22 的 a/b/c/d/e/e2 六条已经在 `docs-parity.test.ts` 的分隔行补齐 / 列表嵌套 / 表格长度三组里
 * 跑着，本组只补那三组没覆盖的两条。
 * ────────────────────────────────────────────────────────────────────────── */
...
describe("R22f/g 必填列是二值格、无默认值写全角破折号", () => {
  ...
  it("f-② 五份 API.md 的参数表：必填列写「是/否」的五语言译法，**出现 `✅`/`❌` 即红**", () => {
    const wrong = LANGS.flatMap((lang) => {
 ...
 * f（25 份非 README 标题 emoji 恒为 0）在 `docs-parity.test.ts` 的「位置不对的 `---`」那一组里，
 ...
 * ⚠️ **落点是裁定定下的那一个，不是会话临时目录**（评审发现）：
 * 上一版把它只留在本次会话的 scratchpad 里，而这段注释写的是「在当时的工作区」——
 * 会话一散，16 个指标的口径列、两参照仓实测值、20 行证据列全部失去出处，
 * 仓里只剩 `VOLUME_FLOOR` 这类孤立数字，而「口径必须留下来」正是排版基线验收① 的全部理由
...
describe("R28 排版基线不回退（那 16 个指标里，能进仓的那几条）", () => {
  ...
  it("指标 1/2/3：极简分隔行、4 空格与 6 空格嵌套列表恒为 0（分隔行补齐与列表嵌套两组的回归面）", () => {
  ...
  it("指标 15：`.env.example` 不许缩水（「一个字都不改」那条规格的落地形式）", () => {
...
/* ══════════════════════════════════════════════════════════════════════════
 * 下半 —— 依赖那一期成果的那一批（R20/P5、R21、R23'、R26）
 *
...
describe("R21 出货文档正文里的裸警告 emoji 恒为 0（白名单已取消）", () => {
  ...
  it("40 份出货文档：标题之外一处裸 `⚠️`/`⚠`/`❗`/`🚫` 都没有（195 处转完之后的状态）", () => {
 ...
 *
 * 🔴 **规格给的目标是 0，今天是 66，差 66 —— 这是一笔如实登记的欠账，不是达标。**
 * 为什么不硬冲 0：实测把全仓压到字面 0 需要每种语言约 47 个新标题、全仓约 235 个，
 * `###`+`####` 会从 1095 涨到约 1330 = **模板密度的 2.4 倍**，与那条裁定
...
describe("R23' 结构分层：相邻标题间的长度、薄标题占比、五语言层级序列", () => {
  ...
  it("A（棘轮）：>1200 字符的区间数不许比登记值多 —— 今天 66，规格的目标是 0（欠账，名册第 21 条）", () => {
  ...
  it("B（棘轮）：薄标题（相邻标题间 <40 字符）占比 ≤15% —— 余量有多大由下面 ② 那一格从磁盘现算", () => {
    ...
    expect(ratio, `薄标题占比 ${ratio.toFixed(2)}%（${thin}/${ints.length}）超过了 ${R23B_RATIO}%。`
      ...
      + "空壳标题**只会让这个比例变坏**。⚠️ 反过来也不许为了过线去删 `###`（同上面那段裁定的第 ④ 条）")
  ...
  it("② 该红时红：往一份出货文档尾巴上加**最小顶穿量**的空壳 `### 占位` —— B 被顶穿；少一个则还没穿", () => {
    ...
    // 复评实测：`eb9f79c` 给 `CHANGELOG.md` 的 `### Added` 下面插了三个 `####`
    ...
    //   · 23 → 30（一键部署按钮）：**只有分子变**，232 → 226，分母 1674 没动。
    //   · 30 → 29（本次 `eb9f79c`）：**分子分母同时变**，227/1677，与前两次都不同。
    //   · 五份 REGISTRAR 补上页脚节那次：**只有分母变**，1669 → 1674。
 ...
 * d（无语言切换行）在 `docs-parity.test.ts` 的「非 README 不带切换行」那一组里、
 * e'（末节标题 == 译名表同一下标）在 DEPLOY / API / USAGE 三组的页脚那几条里，本组不重复实现。
 * ────────────────────────────────────────────────────────────────────────── */
...
const P5_OUTSIDE_ALERT: ReadonlyArray<readonly [path: string, no: number, why: string]> = [
  ...
  //    952 → 958、936 → 942；这一轮的评审回填把**第五把**每轮写的键（退避键
  //    `registrar:backoff`）也补进了同一笔账、并改写了「点一次立即补池」那一条的单价，
  //    958 → 973、942 → 954；同一轮补齐 ja / ko 三栏脚注里缺的那句（域名台账那 48 是
  //    上界、稳态为 0）之后，ja 那一处又被推了两行，954 → 956，en 那一处不动）。
...
describe("R20/P5 风险语义句必须住在 alert 块里（内容锚定的下限，不可灌水）", () => {
  /** 六份 README 里裁定钉死的那两节的标题（`SECTIONS` 下标 14 / 15）。 */
  ...
  it("名册双向（二）：裁定钉死的那两节今天确实含着风险词（否则那条豁免也在守空气）", () => {
 ...
 * ① **过滤**：`pairsOf`（本文件）、`read`（`docs-deviations`）、「文档真值锚」那一组的 `truthDocs`
...
describe("R19' 载体过滤：40 份出货文档的正文里一个 HTML 注释都没有", () => {
  ...
  it("该红时红（其一）：把 `README.md` 里那条 `> 📖 详细面板文档` 整行包进 `<!-- -->` —— 本组点名，且过滤层让那一行对别的判据消失", () => {
    const target = "README.md";
  ...
  it("该红时红（其二）：把头部那条 `> [!IMPORTANT]` 包进 `<!-- -->` —— R20/P1① 必须从 4 块掉到 3 块", () => {
    const target = "README.md";
    const raw = readFileSync(target, "utf8");
    ...
    expect(alertTypes(headOf(blankHtmlComments(hidden))).join(","),
      ...
      + "这正是评审那一格 702 全绿的死法").toBe("NOTE,WARNING,TIP");
  });
});
 ...
 *
 * 🔴 **这是用户唯一逐字点名「要有机器判据」的那一条**，裁定原话是
 * 「五份各自命中本语言的建议式措辞，且全仓零命中禁用词表」。
 * 规格有整节 `## R14`，而**阶段 6 只建了 R27、漏了 R14**：
 * 评审把六份 README 的那句话全部改成禁止式，`Tests 4181 passed (4181)`。
 *
 * ⚠️ **为什么这条洞一定会被踩**：当时实测 kiro2api 模板的 ja/ko 原文就是
 ...
 *
 * **它验不了什么**：「不建议」这句在整段语境里读起来像不像条款；也不管这句话
...
describe("R14 商用措辞：六份 README 的头部 NOTE 块一律建议式（用户裁定，唯一点名要机器判据的那条）", () => {
  ...
  it("该红时红（规格的反向控制①）：ko 改回模板原文 `사용하지 마십시오` —— 负面表点名", () => {
 ...
 *
 * 🔴 **本仓的鉴权语义与 kiro2api 相反，这一组是专门为「抄错」设计的**（硬约束 4）。
 ...
 * 源码锚兜不住：它只现算 `ADMIN_TOKEN_MIN_LENGTH` 的**数值**，对句子的**方向**零覆盖。
 *
 * **分工写在这里，别再合并成一句**：
...
describe("R27 fail-closed 语义的词表两侧：方向由本组管，数字由源码锚那一组管", () => {
  ...
  it("该红时红（评审发现）：把 README 头部那句改成「照常启动，未配置即开放访问」 —— 正负两侧同时点名", () => {
    const target = "README.md";
  ...
  it("该红时红（评审发现）：把 `docs/zh-CN/README.md` 那句改成「以开放模式启动」 —— 必须点名", () => {
    const target = join("docs", "zh-CN", "README.md");
