...
/**
 * **登记在案的五族。每一族都必须是「今天真的泄漏过 / 真的有形状」的那一类**，
...
const FAMILIES: ReadonlyArray<{
  ...
  },
  {
     ...
     * 而 `R`/`D`/`S`（`R2` / `D1` / `S3` 那三个真词的首字母）一个都没进来。
 ...
 *   真词。**这一条今天仍然成立，发现号那两族都不是 `[A-Z]`**，而是逐字母挑出来的
 *   两个子集：`C`/`I`/`Q`/`W` 与 `F`/`G`/`H`/`L`/`M`/`N`。`R`/`D`/`S` 一个都没进来。
 *
...
describe("该红时红：往任一份公开 markdown 塞一个内部标识符 ⇒ 只红一条并点名那一份那一行", () => {
  ...
  it.each(FAMILIES.map((f) => [f.id, f.evidence] as const))(
    "五族逐族认得出：%s 的真串「%s」塞进 `docs/ja/DEPLOY.md` ⇒ 点名那一份",
    (_id, evidence) => {
      probeBase();
   ...
   *
   * 本文件的五族用的是**零宽前后瞻**，天然没有这个毛病；这一格把「没有这个毛病」钉死，
 ...
 *
 * 「第二批字母」那一族（`F`/`G`/`H`/`L`/`M`/`N`）与上面那四个字母有一处硬差别：
 * **它有同形真词**。这张表把每一个已知的同形真词写下来，并且**拿探针钉住它真的会咬**
 * ——这不是「大概会误报吧」的散文，是可执行的代价清单。
 *
 * 🔴 **别把这张表读成「所以这一族该删掉」**，三条理由缺一不可，都是实测：
 * ① **今天代价为零**：44 份公开 markdown 里这一族**零命中**，下面每一句都是**构造**的，
 ...
 *    **不是**把这一族整族删掉，也**不是**把边界放宽。
 * 这一格红了不等于判据坏了——先分清是踩到了登记在案的哪一条，还是真的漏了一处。
 */
describe("登记在案的误报面：这一族确实会咬到的真词，代价写死在这里", () => {
