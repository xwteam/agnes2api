...
describe("scripts/check-i18n.mjs 元测试：十条判据逐条", () => {
  ...
  /**
   * ⚠️⚠️ **上面那张表与门禁那张前缀表的双向咬合，本轮补上（补漏评审 H3）。**
   *
  ...
  it("⑨ usage.range.retention 被软化成「这些子请求是安全的」⇒ 当场红", () => {
    ...
    // **报文里必须带处置指引**（复评 F2 的裁定：读 CI 输出的人不会回头读源码注释）。
  ...
  /**
   * **两条反向控制：钉住「命中口径没有退回裸 `includes`」**（补漏评审 M1 / L4）。
   *
   * 两条都是实测抓到的真误报，不是假想：
  ...
  /**
   * ⚠️⚠️ **词表的真守卫：「概念 × 语言」的完备性**（补漏评审 H1/H2）。
   *
...
describe("scripts/check-i18n.mjs 元测试：第 ① 条判据换形状之后", () => {
  ...
  /**
   * ⚠️ **上面那条绊线扫的必须是抠完注释的源码，不是原文**（复评 F1）。
  ...
  /**
   * ⚠️⚠️ **少一个 `-->` 不许静默吞掉文件尾**（复评 F2，这是本轮最要命的一条）。
   *
   ...
   *（复评 F4；落点逐字取自 `admin-ui/js/pure/settings.mjs` 那两条配置路径模板）。
   *
   * 这一格自带两半，**两半缺一不可**：
