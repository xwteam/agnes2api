...
/**
 * **i18n 门禁（`scripts/check-i18n.mjs`）自己的元测试**（评审发现）。
 *
 ...
 * 五语言齐全的一行字典。
 *
 * ⚠️⚠️ **`en` / `ko` 两侧不能是同一段中文，这是后来定的硬要求。**
...
interface Fixture {
  ...
  tplPrefixes?: string[];
  /**
   * 这棵树上规则⑨（未核实红线）的 key 白名单（脚本的第四个入参）。
   *
 ...
 * 扫到 15 处引用」。复评之后那道门槛已经删掉（死区 97%，且它给自己写的
...
function run(fx: Fixture): { status: number; stdout: string; stderr: string } {
  ...
  try {
    ...
    // 才问得出话。（上一版这里写的理由是"第 ④ 条是 exit 0 + 一条 stderr 警告"，
...
/**
 * 上面那个的**两语言**版。第 ⑩ 条判据问的是 `zh-CN` 与 `ko` 的**关系**
...
/**
 * 横幅上那三个分桶（后来新增）。**逐字解析，不许各处自己 `toContain` 一个数字**：
...
describe("scripts/check-i18n.mjs 元测试：十条判据逐条", () => {
  ...
  // ⚠️ **这里曾经有一格「① 引用数掉到门槛以下（扫描本身坏了）：exit 1」，后来
  ...
  it("④ 字典里谁都不引用的 key ⇒ 当场红（后来翻转：从警告升成硬错）", () => {
    // ⚠️⚠️ **这一格的语义被刻意翻转过，翻转理由必须留在这里。**
    // 旧断言是「④ 必须是警告，不是错误」，旧理由是「动态拼接的 key 抓不到，报错会误伤」。
    // 那条理由后来**不再成立**：模板拼键现在由「模板拼键前缀」这条路认得出来，
    // 而那张前缀表与实扫做**双向**相等比较 ⇒「未被引用」这个集合第一次变得可信。
    // ⚠️ **这里刻意不写「今天全仓有几处拼键」**（上一版写的是「三处」，那是当时复评
   ...
   * 而同一轮还做了「处置真 0 命中 key（删 / 改）」，那条处置套上去删掉的是活文案。
   */
  ...
  /**
   * ⚠️⚠️ **作用域后来扩到「两条通道平级」直接相关的那几个 `set.*` 前缀。**
   *
  ...
  /**
   * ⚠️⚠️ **上面那张表与门禁那张前缀表的双向咬合，本轮补上（补漏评审）。**
   *
  ...
  it("⑨ usage.range.retention 被软化成「这些子请求是安全的」⇒ 当场红", () => {
    ...
    // **报文里必须带处置指引**（复评发现的裁定：读 CI 输出的人不会回头读源码注释）。
  ...
  /**
   * **两条反向控制：钉住「命中口径没有退回裸 `includes`」**（补漏评审）。
   *
   * 两条都是实测抓到的真误报，不是假想：
  ...
  /**
   * ⚠️⚠️ **这一格守的是真仓那两张表本身，不是夹具**（计划要求：新增的每一张
 ...
 */
describe("scripts/check-i18n.mjs 元测试：第 ① 条判据换形状之后", () => {
  /**
   ...
   * 而紧接着就把「未被引用」升成了硬错 ⇒ 会 exit 1 在 11 个正在用的 key 上。
   *
  ...
  /**
   * ⚠️ **上面那条绊线扫的必须是抠完注释的源码，不是原文**（复评发现）。
   ...
   *（复评发现；落点逐字取自 `admin-ui/js/pure/settings.mjs` 那两条配置路径模板）。
   *
   * 这一格自带两半，**两半缺一不可**：
 ...
 * 与收编抠注释器、共用 `unverified-claims.mjs` 是同一条裁定。
 */
