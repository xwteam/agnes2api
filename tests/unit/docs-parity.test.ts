...
// 复评回填（F5）：危险区那张表的**行序**期望值从字典里 `titleKey` 那一行的
...
describe("五语言 DEPLOY.md 的关键数字对等", () => {
  const NUMBERS: ReadonlyArray<{ token: string; why: string }> = [
    ...
    // 而定位成本正是这道门禁存在的意义（同 `48` 那条不加的理由）。
    //
    // ⚠️⚠️ **这两个 token 从真源常量现算，不写字面量**（复评 H2）。上一版写死了
    ...
    // 逐字相同（复评 H2 那次教训）：写死字符串的话，端点路径一改，五份文档里
 ...
 *
 * ⚠️ **报文里两处都点名 `DEPLOY.md`**（复评 H2）：本文件另有一组
 ...
 * · 五份 DEPLOY.md 里那两笔「没在真机上了结过」的配额账，**逐段**扫描（复评 H3）。
 *
 ...
 */
describe("五语言 DEPLOY.md 的两笔「没在真机上了结过」配额账不许被软化（复评 H3）", () => {
...
describe("五份 DEPLOY.md 不许再写「某一期会提供某条重置路径」这种不兑现的承诺", () => {
   ...
   * 二是正则在这五种语言上压根认不出东西——而后者正是本仓评审发现 M1 那次
...
/**
 * ── 「改一把 key 能改哪几件事」那份动作枚举从 `PATCH_FIELDS` 现算（复评回填 H2）──
 *
 ...
 * 重写那五笔（`20f8394` / `ba506f6` / `ec632fd` / `b4f5455` / `0078ea7`）在重写各自那份
