...
// 抠注释走 `scripts/lib/strip-comments.mjs` 那一份真源（收编过来的），不在这里手写第二份。
import { blankComments, blankHtmlComments } from "../helpers/strip-comments.js";
// R11 的根那一半：16 节骨架的期望值取章节标题常量表，不在本文件手抄第二份标题清单。
...
// 评审发现：文档里那几个硬编码数字一律 import 真源常量，不手抄第二份。
 ...
 *
 * 查词的教训：早先查对等时用简体「保证」grep，漏掉了繁体「保證」，于是报告说
 ...
 * `100000`/`1000` 去核对，本该在创建时就全五语言落红——结果因为 K3 那段
...
describe("五语言 DEPLOY.md 的关键数字对等", () => {
  ...
  /**
   * ⚠️ **这一格存在的唯一理由是「让别处那句注释指得住『那一格』」**（复评发现）。
   *
  ...
  /**
   * ⚠️ **同一条理由的第二格**（危险区那两条）：`src/http/admin/handlers/config.ts`、
 ...
 * 每份的出现次数都是 0，对等照样成立（评审发现——加 `all_disabled` 时五份就是
 ...
 *
 * ⚠️ **API 那一批把层级降了一级**：五份 `API.md` 的端点从 `##` 变成协议族
 ...
 *
 * ⚠️ **报文里两处都点名 `DEPLOY.md`**（复评发现）：本文件另有一组
...
describe("五语言 API.md 逐份写着「这条上游事实未经核实」", () => {
   ...
   * 方向指（老教训：报文可以亲手把人引进坑）。⇒ 每格先过这一道闸：真文档今天
 ...
 * · **派生轴**：R1 的「从磁盘派生」原来只派生自 `docs/zh-CN` 一个目录 ⇒ 一份只在 `en` 下的
 *   孤儿文档实测 285 格全绿。今天逐个语言目录各比一次（见 `DOCS` 上方那段）。
 * · **语言轴**：**它本身**原来是一张手写表 ⇒ 新建 `docs/fr/` 实测 285 格全绿。今天由
 *   `langAxisFailure()` 把 `LANGS` 钉在磁盘上（见 `NON_LANG_DOC_DIRS` 上方那段）。
 * · **平凡相等**：R2–R6 一格都没继承数字锚点那组的「平凡相等」护栏 ⇒ `R3 × REGISTRAR`
 ...
 *
 * ⚠️ 补漏评审（缩进围栏）：第一版三条判据全用 `^```` 顶格锚，于是**列表项里的代码块整个在射程外**。
 ...
 *
 * ⚠️ 补漏评审（围栏内的 `#`）：`headings()` 第一版不分围栏内外，于是 ```bash 块里的 `# 注释` 被当成
 ...
 * = 红，表里有某种语言磁盘上没有的 = 红。
 *
 * ⚠️ 这张表最初是五项（不含 `ADMIN`），它当时留下的原话是「`ADMIN.md` 由后续
 * 任务创建，那时把 `"ADMIN"` 加进来，**R1 的第一条断言会强制那一步**（不加就红）」。
 * 落地五份 `ADMIN.md` 时先复现了那条测法：**只把 `"ADMIN"` 加进本表、
 * 一份文件都不写** ⇒ R1 当场红并逐字点名，`DOCS` 表这一条不是靠人记得回来加。
 *
 * ⚠️⚠️ **补漏评审（派生轴）：上面这句「从磁盘派生」曾经只对 `zh-CN` 一个目录成立。**
 ...
 *
 * ⚠️⚠️ **补漏评审（语言轴）：语言轴本身原来是一张不会自己红的手写表。** `LANGS` 五项手写，
 ...
 *
 * ⚠️ **提到模块级是有意的，不是整理**：这两个字面今天有两个消费方——「跨文档指认」那一组的 (B)
 * 「提到按钮的文档，根 README 里必须真有它」，和「版本条目真源锚」那一组里「CHANGELOG 那条部署 bullet
 * 写的份数得是现扫出来的份数」。复评第二轮实测：把 CHANGELOG 里
 ...
 * **这是全仓唯一一处把 `LANGS` 钉在磁盘上的地方**（补漏评审的语言轴那条），返回失败报文或 `null`。
 */
...
function inventoryFailure(root: string, table: readonly string[]): string | null {
  ...
  // ⚠️ 逐个语言目录各比一次（补漏评审的派生轴那条：第一版只比 zh-CN 一个目录）。
 ...
 *
 * ⚠️ 这是复评抓到的：两侧长度不同时 `firstDiff` 返回的正是**短的那一侧的长度**
 ...
 * `R3 × ADMIN.md`（加进 `DOCS` 之后新增的一格，同样整份无围栏）——30 格里
 * **2 格结构上不可能变红**，而文件里原本一个字都没说这件事。
 * 🔴 **`ADMIN` 那一格已经随「ADMIN 补围栏」那一条销账**：五份 `ADMIN.md` 各补了
...
const EMPTY_BY_DESIGN: ReadonlyArray<readonly [rule: string, doc: string]> = [
  ...
  // **分两批删掉**（销账的两半，各自与补围栏那一批同提交）：
  // 前者随五份 ADMIN.md 各补 4 段围栏，后者随五份 REGISTRAR.md 各补 5 段。
 ...
 * 它存在的理由是：当时把五份 `docs/{lang}/README.md` **分步**换成 12 节形态，
 ...
 * 是同一份文档的六个语言版；后来把**根** README 单独重写成模板的 16 节
 ...
 * 不是一条豁免。
 *
 * ✅ **它已经过期并被清空了（`docs/zh-CN/README.md` 扩成 12 节那一步）。**
...
describe("五语言文档的派生结构对等（R1–R6）", () => {
   ...
   * 「这一份 README 的 `## ` 序列必须逐字等于章节标题常量表那 12 行」这条**射程更大**的
  ...
  it("跨行 span 那条今天仍然承重：真文档里确实有跨行的 code span", () => {
...
describe("R1–R6 的反向控制（临时目录夹具）", () => {
  ...
  function fixtureDoc(lang: Lang, doc: string): string {
    ...
    return [
      ...
      "",
      // ⚠️ 补漏评审（跨行 span）：这一行是**故意照着真 `docs/*/DEPLOY.md` 那处摆的**——一个多词
      ...
      "```",
      "",
      // ⚠️ 补漏评审（缩进围栏）：夹具里**必须有一个缩进围栏**，否则「`fences` 认缩进围栏」那条
  ...
  function pristineTree(): Tree {
    ...
    // ⚠️ 补漏评审（语言轴）：夹具树的 `docs/` 子目录集合必须与真仓同构，否则 R1 语言轴那条
  ...
  const MUTATIONS: ReadonlyArray<{ why: string; hits: readonly string[]; mutate: (f: Tree) => void }> = [
    ...
    {
      // ⚠️ **补漏评审派生轴那一格**：`inventoryFailure` 第一版只 `readdirSync(docs/zh-CN)`，
    ...
    {
      // ⚠️ **补漏评审语言轴那一格**：`LANGS` 五项手写、全仓没有一处拿磁盘钉住它，
    ...
    {
      why: "R2 某一份多一个 ###",
      // ⚠️ `越界` 这一项是那条复评发现的落点：ja 比参照多一项，`firstDiff` 返回的下标恰好
    ...
    {
      // ⚠️ **补漏评审缩进围栏那一格**：顶格锚（`/^```(\w*)/gm`）看不见缩进围栏，
  ...
  it("不乱红：围栏里多一行 `# ` 开头的注释——那不是标题，R2 不该红", () => {
    // 围栏内 `#` 那条的原始证据：往真 `docs/ja/DEPLOY.md` 的 ```bash 块里加一行 `# …` ⇒ R2 当场红，
  ...
  it("不乱红：一处 code span 换行重排（内容一个字不改）——R6 不该红", () => {
    // 跨行 span 那条的原始证据：真 `docs/zh-CN/DEPLOY.md:553` 那处跨行 span 让按行截断的旧判据
  ...
  /**
   * 那条复评发现的两格：**报文里不许出现字面的 `undefined`**。
 ...
 *
 * 当初立的红线：**真机了结之前，任何文案都不许把一个从没量过的上限写成「足够 / 安全」**。
 * 它今天有两个消费者，**共用这一张表**：
 * · 五份 ADMIN.md 的**整份**扫描（Task 26A）；
 * · 五份 DEPLOY.md 里那两笔「没在真机上了结过」的配额账，**逐段**扫描（复评发现）。
 *
 ...
 * 「足夠/夠用」、韩文的「안전」一个都不在表里。这正是本文件开头登记的那条查词教训
...
describe("五份 ADMIN.md 的措辞与数字守卫", () => {
  ...
  // ── ① 软化词表 ────────────────────────────────────────────────────────────
  //
  // ⚠️ 词表本身与打平表**搬到了模块作用域**（复评发现）：同一条红线在 DEPLOY.md 那一侧
   ...
   * ⚠️ **这道闸不是照抄来的，是实测补的**：本任务做那次变异（往 zh-CN 那份里塞一句
  ...
  const ADMIN_NUMBERS: readonly NumRule[] = [
    ...
    // ⚠️⚠️ **下面四条是后来落的，前两条就是「轮询上限」那条红线的机器化。**
    //
    // 这条红线一直立着：`admin-ui/js/pure/playground.mjs` 自己登记着「5 分钟 / 60 次
   ...
   *
   * ⚠️ **「兄弟文档咬合」那一组：这张表原来是一张与 `DOCS` 脱节的手写清单。**
  ...
  it("咬合：`SIBLING_DOCS` 恰好等于 `DOCS` 去掉 ADMIN 自己 —— 手写表不许与文档全集脱节", () => {
  ...
  /**
   * 本组的变异夹具：往 `docs/zh-CN/ADMIN.md` 顶上插一条**指向 API.md 某一节**的引用，
  ...
  it("不许乱红：ADMIN.md 引 API.md 的一节，而那一节今天真的在 —— 一条都不许红", () => {
    probeXrefBase();
    const { failures } = xrefScan(withApiXref(false));
    expect(failures, `报文：\n${failures.join("\n")}`).toEqual([]);
  });

  it("该红时红：API.md 那边把被引的那一节改了名 —— 红并点名 API.md 与那个名字", () => {
    probeXrefBase();
   ...
   * 不是当计数用。
   *
   * ⚠️⚠️ **认不全就抛，不返回一个少数了一张的计数**（复评发现）：这个函数是真扫描与
   ...
   * 旧判据自己在注释里预言过「将来往同一份文档里加表、撞上这两个
   * 数量时它会吵」。**那句预言只对了一半，实测当场证伪另一半**：模拟危险区建出第 5 张
  ...
  it("不乱红：注释里写一句真代码片段（`card(\"set.card.danger\")`）不算调用点 —— 抠注释这一步是必需的", () => {
    const src = readPanelSource();
    // 本仓的注释密度极高，随手写一句 `card("…")` 来解释「将来会怎么建」是随时会发生的事，
    // 裸数 `card(` 会把它一起数进来 ⇒ 完备性锚会在**判据什么都没坏**的那天红。
    // ⚠️ **上一版这里举的例子今天是假的**：它写着「`sec-settings.js` 卡 4 上方那段就在讲
    // 第 5 张卡」——搬卡之后那段里既没有那个编号，整个面板的注释里也一处 `card("` 都没有
    //（实测：`stripComments()` 前后 `card("` 的处数相同，三份面板源码逐份为 0）。
    // ⇒ 例子撤掉，只留「为什么必须抠注释」。**这一格要防的是下一处出现的那天，
    // 与今天有没有无关**，所以不把「今天零处」写成一条断言（写了就等于要求它永远为零）。
    const mutated = {
      ...src,
      settings: src.settings.replace(
        'const examples = card("set.card.examples");',
        '// 将来会写 const danger = card("set.card.danger");\n    const examples = card("set.card.examples");',
      ),
    };
    expect(mutated.settings === src.settings, "变异没落到 sec-settings.js 上——这一格控制是空的").toBe(false);
   ...
   * ⚠️ **它挡不住「认不出来的那一张」**（复评发现）：找不到的名字它按定义看不见，
   * 那一族归上面那三格完备性锚。
   */
  ...
  // ── ⑥ 设置卡表与调试台模式表：同一条派生法，多两张表 ──────────────────────────
  //
  // ⚠️ **上一版这里写着「第 12 节（危险区）今天只有一句『这张卡还不存在』，靠的就是
  // 下面那条设置卡变异」——那句话今天是史实，不是现状。** 那条变异当时
  // 模拟的正是后来落地的危险区那张卡：它落地那天，第 12 节也从占位改成了实节 +
  // 一张两行的表。**那条绊线按设计响过了**
  //（当时实测：只加卡不改文档 ⇒ 这一组 9 格红，其中 5 格逐份点名「设置卡」）。
  // 下面那条变异因此上移一档：模拟的是**在设置页现有的卡之后再多建一张**。
  //
  // ⚠️⚠️ **这里刻意不写「多出来的是第几张」**：那个序号是从源码现算出来的，
  // 危险区落地时它是一个数、注册机那张卡搬去注册机板块之后又是另一个数
  //（这一段自己就因为手抄那个序号漂过：上一版写着一个比当时真值大一档的数）。
  // ⇒ 散文里一律不落编号，要看今天是第几张就读下面那一格 —— 它把现算出来的
  // 建卡顺序打进报文，并断言变异那张卡确实落在现有卡之后。
   ...
   * **撞号真的消失了**（搬家当天实测 8 / 5 / 3 / 4，四个数互不相同），那一格按设计红了。
   * ⇒ 按它自己的报文处置：删掉它。
  ...
  it("该红时红：设置页多出一张卡而五份文档没跟着加行 —— 五份一起红", () => {
    ...
    // 「多出来的是第几张」由这里现算并打进报文，**不写在上面那段散文里**：
    // 手抄一次就会漂一次（那一段自己漂过）。这一条同时钉住变异的落点：
    // 那张卡必须排在设置页现有的卡**之后**，否则模拟的就不是「末尾多一张」。
    const order = settingsPageCards(codeOnly(mutated.settings));
    expect(
      order.at(-1),
      `变异那张卡没落在设置页现有卡之后 —— 现算的建卡顺序是 ${order.join("、")}`
      + `（共 ${order.length} 张；这一格要求多出来那张排在最后，也就是第 ${order.length} 张）`,
    ).toBe("set.card.extra");
    const failures = tableSeqFailures(expectedTables(c), realAdminDoc);
    expect(failures, `报文：\n${failures.join("\n")}`).toHaveLength(LANGS.length);
    for (const h of ["设置卡", "第 4 张"]) {
  ...
  /**
   * ⚠️ **危险区那张表是新加的第 5 张，它与前四张走同一条派生法。**
  ...
  // ── ⑦ 三句「今天是什么样」的话，各自的测法（复评回填）──────────────────────────
  //
  // 这三句都是复评实测抓到的「五份一起说错」：结构判据、数字锚、
  ...
  /**
   * 「保存之后不说已生效」那一句：设置那一节原来写的是「保存之后面板**不说**「已保存并生效」」，而屏幕上
  ...
  /**
   * 「登录看起来成功却进不去」那一句：排障那一节原来写的是「登录看起来成功了，却永远进不去」。
 ...
 *   ⚠️ **射程是整份文件，也包含 HTML 标记里的链接**：改写时把首屏换成了模板的
 ...
 *   指针行贴在用得到它的那一节首行（`K/README.md:76,196,409`），据此删掉了
 ...
 *   是两份逐字复制」。改写之后**这个关系被有意解除**：根那份改成简体中文的
 ...
 *   文档判据管不了的是**文档**，不是那个行为。
 */
describe("根 README 首屏的一行五链与 16 节骨架", () => {
   ...
   * 当时在**任何** README 里都没有入口（删掉矩阵表时一并没了）。
  ...
  /* ── ④B 五份语言版 ☕ 节里的同目录 SPONSORS 入口（那个空档的结清判据）──────────
   *
   ...
   * 也不问 `docs/<lang>/SPONSORS.md` 里面的内容对不对（那是 R2–R6 与根 SPONSORS 那一组的活）。
   */
  ...
  it("④B 五份语言版的 ☕ 节都链着**同目录**的 SPONSORS.md —— 语言版 SPONSORS 没有入口那个空档的结清判据", () => {
  ...
  it("④B 该红时红：ja 那条改指**根**那份 SPONSORS.md —— 文件在，① 一个字都不吭，只有 ④B 看得见", () => {
    probeBase(sponsorEntryFailures(readLangReadme), "④B 五份语言版的 ☕ 节都链着**同目录**的 SPONSORS.md —— 语言版 SPONSORS 没有入口那个空档的结清判据");
   ...
   *
   * ⚠️ **⑦ 只管根那一份，五份语言版那 45 条由下面的 ⑦B 管**（那一笔的验收是 6 × 9 = 54 条，
  ...
  /* ── ⑦B 五份语言版头部块的那 45 条锚点（另一半）───────────────────────────────
   *
   * 那一笔的验收是 **6 × 9 = 54 条**：根那 9 条归 ⑦，语言版这 45 条归 ⑦B。
...
/**
 * ── 五语言 DEPLOY.md 的三笔欠账，逐笔各配一条会自己红的锚 ─────────────────────
 *
 ...
 *
 * ⚠️ **这三张表都是「清单」，所以各自配了会让它变红的断言**（头号纪律）：
 ...
 *
 * ⚠️ **它们住在模块作用域，不住在某一个 `describe` 里**：欠账那一组与
 ...
 * 就会被人放宽或删掉。
 * ⭐ 这段里的计数**不是判据**（同本文件上面那条 ⭐）：会变红的是下面那几格，
 * 数字会过期，要数就当场自己数一遍。
 ...
 *
 * ⚠️ **锚点从真源常量现算**（同 `NUMBERS` 那两条，复评发现）：常量一改，锚点就落空，
 ...
 */
describe("五语言 DEPLOY.md 的两笔「没在真机上了结过」配额账不许被软化（复评发现）", () => {
...
/**
 * ── 推公开仓之前第一个访客会看到的三处「空白 / 假话」各自上锚 ─────────────────────
 *
 * 这三处是同一族毛病：**十二道门禁全绿，而文字说的是假的、或者干脆什么都没说。**
 *
 * · `admin-ui/README.md` 末尾原来写着「第一期的范围 —— 只有登录闸和一个空壳，
 *   `i18n` 字典、`theme.js` / `ui.js` / `api.js` 与 8 个功能板块都在后面」。
 *   那句话在下一期落地当天就过期了，却一路活了很久。**没人守是有原因的**：
 ...
 *   （首屏那一组的 ③ 盯的是徽章上的版本号，盯不到链接落地之后是不是一张白纸）。
 ...
 *   （`src/entry/node.ts` 用 `FileStorage`），与刚修掉的那句同型；②「上游一个都
 ...
 *  于是一条测试内部的命名把泄漏轴要挡的东西**钉死**在了一份公开自述上——
...
describe(FIRST_VISIT_GROUP, () => {
  ...
  it("不乱红：同一个板块被第二个按钮指向 + README 合法地多提一次板块名 —— 都不许让上面那一格红", () => {
    // ⚠️ 这一句是实测补的：不加基探针时，真文件一脏这一格也会跟着红，而它的报文说的是
   ...
   * 正是裁定明令禁止的那件事。实测：把 `VERSION` 改成 `0.1.1`、按 Keep a Changelog
  ...
  it("该红时红（改标题）：CHANGELOG 里那条版本条目的标题被改掉 —— 点名 VERSION 里的那个版本号", () => {
    const v = realVersion();
  ...
  it("该红时红（掏空正文）：条目还在但正文被掏空 —— 这正是需求书那条恒绿断言放不出的那一枪", () => {
    const v = realVersion();
    probeBase(changelogFailures(v, realChangelog), CHANGELOG_CELL);
    ...
    expect(logSection(mutated, v), "掏空之后标题还得在，否则这一格测的是改标题而不是掏空正文").not.toBeNull();
    const failures = changelogFailures(v, () => mutated);
    expect(failures, `报文：\n${failures.join("\n")}`).toHaveLength(1);
   ...
   *
   * ⚠️ 改写时把首屏换成了模板的 HTML 头部块：切换行从 `**Language:** [English](…) | …`
  ...
  it("CHANGELOG 顶上那句语言提示逐个点名五种语言的自称（从根 README 的语言切换行现算）", () => {
    ...
    // 补这一组之前那是一张空页，之后 en / ja / ko 的访客点进来看到的是整页中文。至少说清楚。
 ...
 *
 * ⚠️ **三格是互补不是重复，别合成一格**（与「下标不变式与 EXPECTED 互补」同一条道理）：
...
/**
 * **不经 `Storage` 端口**的那一族键的**封闭登记**（复评回填）。
 *
...
describe("「重置到底重置了什么」：全部存储键的封闭登记", () => {
  ...
  it("不乱红：`_KEY` 结尾但不是存储键的那一族 —— `USAGE_OTHER_BUCKET` 那次改名的反面，不许扫成键", () => {
    ...
    // 里 `USAGE_OTHER_BUCKET` 今天的值，当时正是为了这条约定把它从 `_KEY` 改名的。
 ...
 * ── 那句「某一期会提供一条正式重置路径」──────────────────────────────────────
 *
 * 那句承诺的原形：五份 DEPLOY.md 在 `POOL_TOUCH_INTERVAL_MS` 那一行**同步承诺**了
 * 「P3c 会提供一条经过 repo 的正式重置路径」，而那一期已经完成、
 ...
 * 同样形态的空头承诺**。
 */
describe("五份 DEPLOY.md 不许再写「某一期会提供某条重置路径」这种不兑现的承诺", () => {
  /**
   ...
   *    字符都对不上；需求书那条用的变异串 `P3f 会提供一条正式重置路径`（这一串来自本期需求书）
   *    里的 `f` 同样在字符类之外 ⇒ **照抄原式做那个变异，打上去这一格照绿**，
   ...
   * 二是正则在这五种语言上压根认不出东西——而后者正是本仓评审发现的那次
   * 「判据认不出任何东西 ⇒ 真仓五格全变绿」的形态。
   *
   * 这一格同时就是需求书那条（「只改四份、漏掉某一份」）：它逐份把那一份改回原话，
   * 每一次都必须**只红一条**并点名那一份。
   */
  ...
  it("该红时红：需求书那句「P3f 会提供一条正式重置路径」塞进任一份 ⇒ 点名那一份", () => {
    probeBasePromise();
    // ⚠️ **这一串不是现编的**：它逐字来自本期需求书那一行。
  ...
  it("不乱红：期号与「重置」各在自己那一行时不许命中 —— 40 字窗口是按行算的", () => {
    probeBasePromise();
    // ⚠️ **这两行曾经都是仓里真实存在的原文**（`docs/zh-CN/DEPLOY.md`：一行是小节标题、
    // 一行是危险区那笔配额账），**今天不是了**：那一轮把出货文档里的阶段编号全清了，
    // 小节标题现在写的是「设置页能改什么」，`P3c` 在 40 份出货文档里恒为 0 命中
    //（`tests/unit/docs-internal-refs.test.ts` 的「真扫描：零命中」那一格守着这件事）。
    // ⇒ 这一格的串**从此是构造出来的探针，不再是照抄的原文**——它要问的那件事没变：
    // 期号与「重置」各在自己那一行时不许命中。**这一格把那件事变成一条会自己红的断言**：
  ...
  // 少了这半句，文档就在说「重置之后立刻且永远干净」——而那正是那句承诺
  // 当年被写下来的原因（先清零后回弹）。
  ...
  it("不乱红：五份一起合法地多写一句无关的话 —— 上面那几格不许因此假红", () => {
    // ⚠️ **这一格断言了两族判据，所以两族的探针基都要先取一遍**（复评发现）：
...
/**
 * ── 「改一把 key 能改哪几件事」那份动作枚举从 `PATCH_FIELDS` 现算（复评回填）─────
 *
 ...
 */
describe("「改一把 key」那份动作枚举从 `PATCH_FIELDS` 现算（复评回填）", () => {
  /**
...
/* ────────────────────────────────────────────────────────────────────────────
 * 「某一份根本没翻译」—— 字典侧规则⑩ 在 docs 轴上的移植（评审发现）
 *
 * **缺口是怎么被发现的**：评审把 `docs/en/ADMIN.md` 里一整句英文换成中文（结构一个字
 * 不动）⇒ `pnpm test` 全绿、七道门禁全 exit 0。字典那一侧早就建过
...
describe("「某一份根本没翻译」：en 与 ko 的正文里不许有汉字假名", () => {
  ...
  it("夹具正反两侧：围栏 / 行内 code / 切换行 / 谚文汉字注不许红，正文里的中文必须红", () => {
    ...
    // 模板头部块那一种载体：同一行同一件事，写法是 HTML。当前语言按规则不带链接。
...
/* ────────────────────────────────────────────────────────────────────────────
 * 五份 SPONSORS.md 的字面恒等式（评审回填：那一笔的验收 ①②③⑤）
 *
 * 这五份文档落地那一批声称验收过四项恒等式——
 ...
 *   五条方向 bullet 里除末两条之外的部分，今天都没有判据（末两条由根 SPONSORS 那一笔的
 ...
 *   文件**：`#锚点` 那一段一律截掉不查（跨文档锚点要 slug 化，那要统一治理，
 ...
 * · 根 `SPONSORS.md`（不在 `docs/` 下的那一份）**不在本组射程**：它归根 SPONSORS 那两组。
...
describe("五份 SPONSORS.md 的字面恒等式（那一笔的验收 ①②③⑤）", () => {
  ...
  const brokenDocLinks = (read: (p: string) => string, files: readonly string[]): string[] => {
    ...
    for (const f of files) {
      for (const t of relTargetsOf(read(f))) {
        scanned += 1;
        // `#锚点` 那一段截掉不查：跨文档锚点要 slug 化，那要统一治理，本轮不做。
  ...
  /* ── ⑥ 跨文档**锚点**也得解析得开（统一治理的另一半，评审发现 16 的回填）─────
   * 上面 ⑤ 的注释自己写着「`#锚点` 那一段截掉不查…那要统一治理，本轮不做」。
   * **这件事必须有人守**：「第 1 版只对文件路径
   * `existsSync`，`[参数表](API.md#不存在的锚点)` 这种跨文档死锚点零覆盖…那几批改写
   * 都会造这类链接，必须有人守」。
   *
   * ⚠️ **今天仓里真有 10 条这类链接**（API 与 USAGE 那两批造的，五份 API.md 与五份
   * USAGE.md 各一条，全部指向 `DEPLOY.md#{环境变量的五语言 slug}`）。逐条核对过它们
   * 今天都解析得开 ⇒ 这是「一族没人守的链接」，不是「已经坏掉的链接」：
   * **改一次 `## 环境变量` 的标题**（译名表名册里 DEPLOY 的第 6 槽正是它）
  ...
  const brokenDocAnchors = (read: (p: string) => string, files: readonly string[]): string[] => {
    ...
    if (scanned === 0) {
      throw new Error("docs/ 下一条跨文档锚点链接都没抽到 —— 判据坏了，"
        + "不许静默当成「零死锚点」（API 与 USAGE 那两批造了 10 条，它们应当都在射程里）");
    }
    return out;
  };
  ...
  it("⑥ 射程自守：今天扫得到的跨文档锚点链接不少于 10 条（API 与 USAGE 那两批造的那一族）", () => {
 ...
 * 重写那五笔（`86f6234` / `5456dae` / `4217986331081a` / `7c6bad3` / `cb8e175`）在重写各自那份
 ...
 *
 * ⚠️ 那条「被弃用的字面只出现在判据里」的裁定**对这颗按钮已经失效**：它不再是弃用件，
 ...
 */
describe("跨文档指认的真实性：文档里说「那份 README 里有 X」，X 就得真在", () => {
   ...
   * · `../../README.md` —— 页脚形态 A 的**根 README 回链**（裁定：模板每份页脚
   *   都是「兄弟文档 × N + 根 README 回链 + GitHub Issues」，评审发现 9 / 21
   *   的回填把这一条补进了 20 份文档）。
  ...
  it("(A) 闭合该红时红：往 zh-CN/DEPLOY.md 里加一条指向**另一种语言** README 的链接 —— 它解析得开，⑤ 不会红，本格必须红", () => {
    ...
    // 已经是登记表里的合法目标，拿它当变异这一格会恒绿（评审发现 9 的连带）。
  ...
  // 版本条目那一组里 CHANGELOG 那条部署 bullet 的份数锚也拿同一个主机名去扫，不许各抄一份。
   ...
   * **今天是五份语言版 README**（按钮加了回去）。回填本组时这个数是 0 ——
  ...
  it("(B) 不该红时不红：谁都没提按钮、根上也没有 ⇒ 前件为假，不许红", () => {
    ...
    // 那个方向归按钮形态锚（六份自等式）与偏离名册第 23 条管，不是本组。
 ...
 * 是 en/ja/ko 而不是中文两份。但「按模板走」**不是这一节的裁决规则**：用户裁定
 ...
 */
describe("⑧ 五份 README 的 `## 📄` 节末段逐字节等于登记表", () => {
...
/**
 * # 非 README 文档的五语言 `##` 译名常量表
 *
 * 那一期有四条验收判据（DEPLOY 与 API 两批里各两条）写的都是
 * 「五份 `##` 序列 `toEqual` 目标骨架」，而**规格全文没有那个右操作数**。
 * 没有它，实施者只能自己编一套 —— 而「自己编一套」正是选「五语言同构」那条裁定
 ...
 *   ⇒ 那是模板形态，逐字照抄。裁定原话就是「证据（实测）胜过规格（转述）」——
 ...
 * ## 三条已裁定的骨架决定都落在这张表里
 *
 * - **①**：`API.md` 的 `## 模型` **保留为独立 `##`** ⇒ API 是 **13 个 `##`**，不是 12。
 * - **②**：`DEPLOY.md` 的 `## 环境变量` **保留** ⇒ 叠加 §1.10 的双形态变体
 *   （第 3 槽 `## Docker 部署` 拆成 `## 选哪种形态` / `## Cloudflare Worker 部署` /
 *   `## Docker 部署`）之后是 **15 个 `##`**，不是 12 也不是 14。
 * - **③** 只管端点归族（`GET /v1/models` → OpenAI 族、`GET /v1beta/models` → Gemini 族），
 *   那是 `###` 层的事，不影响本表。
 *
 * ## 射程：**只有 DEPLOY 与 API 进这张表**（§1.9.4）
 *
 * `USAGE` / `ADMIN` / `REGISTRAR` 三类本期是「`##` 骨架维持现状、只补 `###`/`####`」，
 * 给它们钉文本 `toEqual` **等于把今天的现状当成模板** —— 那是本仓犯过的病
...
const DOC_SECTIONS = {
   ...
   * 6 `环境变量` 骨架决定第 ② 条的扩展位，**取 agnes 五份今天已有的那五个标题逐字**（实测五语言同构，漂移 0）
   * 7 `多账号配置` ｜ 8 `验证部署` ｜ 9 `常见问题` ｜ 10 `性能优化` ｜ 11 `监控和维护`
   * 12 `升级服务` ｜ 13 `备份和恢复` ｜ 14 `安全建议` ｜ 15 页脚节
   *
   * ⚠️ **第 6 槽的位置是本轮裁定的，规格没说。** 骨架决定第 ② 条只说「保留为独立 `##` 节」，
   ...
   *
   * - 第 1 节从 agnes 今天的 `## 鉴权` 改名到 `## 认证`（§1.5 的固定第一节）。
   * - 第 4 节 `## 模型` 是**骨架决定第 ① 条**保留的独立节，位置取模板实测序
   *   （kiro zh-CN API：`## 错误响应`:87 → `## 模型名映射`:189 → `## OpenAI 兼容 API`:210），
   *   标题取 agnes 五份今天已有的那五个（实测同构）。
   * - 七个协议族**都带 ` API` 后缀**（实测胜过规格那条裁定）。
   * - agnes 今天那两个专题节（`## key 池耗尽时的错误` / `## 同步端点超时`）按 §1.9.2 给的
   *   二选一，**选「并入 `## 错误响应` 之下降为 `###`」**——另一条路（当成额外的 `##`）
   *   会让 `##` 数变 15，与骨架决定第 ① 条定的 13 冲突。
   *
   * ⚠️ **`## Anthropic 兼容 API` 这一格没有照抄 K∩G 的 `## Claude 兼容 API`，理由要说清楚**：
   * 实测胜过规格那条裁定逐条列出的 K∩G 差异是三项（`管理接口`→`管理 API`、`系统`→`系统 API`、` API` 后缀），
 ...
 * 离目标骨架还有 DEPLOY / API 整整两批改写。**在文档改完之前把
 ...
 * 两者中间留了一个窗口：DEPLOY 那一批的实施者把 15 节里**做对 14 节、错 1 节** ⇒
 ...
 * 译名表验收要的那句「ja 第 7 节对不上：目标 …，实际 …」就是从这条判据里出来的。
 *
 ...
 *   —— `鉴权 → 认证` 那次改名只欠 zh-CN/zh-TW 两份 ⇒ 这三格是 **15**。
...
const SECTIONS_DRIFT_BASELINE: Readonly<Record<string, number>> = {
  // **这张表今天是空的，而空是它的终点、不是它坏了。** DEPLOY 五份在自己那一批
  // 走到全等，API 五份在自己那一批走到全等 ⇒ 十格全部搬进 `SECTIONS_APPLIED`，
 ...
 * 让译名表的验收走完「文件文本 → `sectionTitles()` → `sectionFailures()`」这条
...
describe("非 README 文档的五语言 `##` 译名常量表", () => {
  ...
  it("射程登记：只有 DEPLOY 与 API 进这张表（§1.9.4：USAGE/ADMIN/REGISTRAR 钉文本 = 拿现状当模板）", () => {
    expect(
      ...
      + "给它们钉文本 `toEqual` 等于把今天的现状当成模板（本仓犯过的病）。要加就先来推翻 §1.9.4",
    ).toEqual(["API", "DEPLOY"]);
  });
  ...
  it("这 25 份文档的射程铁律：目标标题一个 emoji 都不许带（R25f）", () => {
    ...
    expect(withEmoji, "非 README 文档的标题不加 emoji，而目标表自己带上了").toEqual([]);
  });

  it("页脚节五语言逐下标一致：DEPLOY 与 API 的末节用同一族译名（R26e' 的右操作数）", () => {
    // DEPLOY 与 API 那两条「末节标题 == 译名表同一下标的五语言译名」靠的就是这一条：
    // 两类文档的末节如果各写各的，R26e' 会退化成「每类各自内部一致」，
    // 而裁定定的是**五类子文档统一成同一种页脚形态**。
    for (const lang of LANGS) {
  ...
  it("名册双向：登记在册的那几格今天确实还没走到目标（在册却已达标 ⇒ 名册过期，回来划掉并接上真文档）", () => {
    ...
    expect(
      ...
      + "⇒ DEPLOY / API 那两批已经落地。把它们从 SECTIONS_NOT_YET_APPLIED 里划掉，"
  ...
  it("反向控制：「15 节做对 14 节」这一档今天由 `toEqual` 接住 —— 旧写法 `> 0` 在这一档是绿的", () => {
    ...
    // ⚠️ **接住它的东西换了人**（API 那一批之后）：DEPLOY/ja 已经走到全等并搬进
   ...
   *
   * `SECTIONS_NOT_YET_APPLIED` 与 `SECTIONS_DRIFT_BASELINE` 今天都是空的（DEPLOY 与
   * API 先后走到逐字全等），于是上面那两个 `for` **一次都不进循环**。
  ...
  it("名册射程自守：每一格的差距条数与实测基线逐格相等（条数一变 ⇒ `##` 骨架被动过，当场红并报出剩下的差距）", () => {
    ...
    expect(
      drift,
      `这几份文档的 \`##\` 骨架与基线对不上了：\n${drift.join("\n")}\n`
      + "⇒ 条数**变小** = DEPLOY / API 那两批在推进：把这一格重新量一遍写进 SECTIONS_DRIFT_BASELINE，"
 ...
 *
 * ⚠️ **显示宽度与排版基线指标 13/13b 的「字符数」是两把尺**：那两条量的是
...
describe("表格分隔行逐格补齐：宽度跟着表头走，不许有极简格、不许有对齐冒号", () => {
...
describe("列表嵌套只用 2 空格，深度硬上限 1 层", () => {
...
describe("非 README 文档不带 `**语言：**` 切换行（参照仓一条都没有）", () => {
...
/* ══════════════════════════════════════════════════════════════════════════
 * 25 份非 README 文档里位置不对的 `---` 删掉（R20/P4②）
 *
 * **【裁定改判】这一条不是「删掉全部 `---`」。** 模板允许**页脚节之前**留一条：
...
describe("非 README 文档里位置不对的 `---` 删掉（页脚块之前允许留 ≤1 条）", () => {
  ...
  /* ── 本组的另一半：标题 emoji 今天是 0，本组只守住不回退（R25f）──────────────
 ...
 * · 裁定明令**不许删 caveat、不许删出处**来达标 ⇒ 只能抬阈值。
...
/** 一行表格行的字符数（`[...s].length`，与排版基线指标 13 同一把尺，不是显示宽度）。 */
 ...
 *
 * 口径与「分隔行逐格补齐」那一组共用：先剥围栏（`bodyLines`），再取首尾都是 `|` 的行，
 * **分隔行不算数据行**（它的长度由那一组的逐格恒等式决定，两组算同一行会互相打架）。
 */
...
describe("表格行与单元格的长度上限（R22e ≤ 340 / R22e2 ≤ 300）", () => {
  ...
  it("R22e：射程内没有任何一行表格行超过 340 字符", () => {
    ...
    expect(
      ...
      + "⇒ 处置是本组那一条：表里只留一句摘要，长解释移到表下的 `>` 引用块或 `> [!NOTE]`。"
      + "🔴 **不许靠删 caveat 或删出处来达标** —— `tests/ui/settings.test.ts` "
   ...
   * **变小** = API / ADMIN / REGISTRAR 那几批在推进，回来重新量一遍写进这里，并在提交正文里
  ...
  it("棘轮：射程内超过 200 字符的表格行恰好还是今天这些（变大 = 有人在塞长句，变小 = 该回来重新量）", () => {
    ...
    expect(
      ...
      + "⇒ 变小：API / ADMIN / REGISTRAR 那几批在推进，把这个数重新量一遍写回本格。",
    ).toBe(OVER_200_TODAY);
  });
  ...
  it("不许乱红：分隔行不算数据行（分隔行逐格补齐那条恒等式会把宽表的分隔行撑得很长）", () => {
...
describe("折叠块那一节：`<details>` 是射程铁律的具名例外", () => {
  ...
  it("偏离名册的具名例外：25 份非 README 文档里的 `<details>` 恰好就是名册那 5 处", () => {
  ...
  it(`那一节的最长 text-run ≤ ${QUOTA_MAX_RUN} 字符（折叠块那一组的验收）`, () => {
    ...
    expect(
      over,
      `这几段读者中间一个视觉锚点都没有：\n${over.join("\n")}\n`
      + "⇒ 处置是本组那一条：拆成 `####` 小节、拆成子列表、或在句号处另起一段。"
      + "**不许靠删句子达标**。",
    ).toEqual([]);
  });
...
/* ══════════════════════════════════════════════════════════════════════════
 * 五份 `DEPLOY.md` 的 15 节骨架落地之后的四条验收
 *
 ...
 * 「恰 6 条 × 5 语言」与「不许为凑数编造条目」那条裁定同型而方向相反——一条判据
...
function envFenceFailures(lang: Lang, where: string, body: readonly string[]): string[] {
  ...
  if (decls.length === 0) {
    out.push(`${where}：这个 \`\`\`env 围栏里一行变量声明都没有 —— 空围栏凑不出这一条要的数`);
    return out;
  }
...
describe("五份 DEPLOY.md 的 13 节骨架之下的三条验收", () => {
  ...
  it("每份 DEPLOY.md 至少 5 个 ```env 围栏（落地前是 0）", () => {
  ...
  it("每个 ```env 围栏都是「带注释的」：变量上方 1–3 行 `#`、以必填/可选起头、变量之间空行", () => {
    const failures: string[] = [];
    for (const lang of LANGS) {
  ...
  it("常见问题的三段式：`###` 条数 == `**症状**` 条数 == `**解决方案**` 条数，五语言彼此相等", () => {
  ...
  it("`## 安全建议` 真的在，而且不是一个空壳（≥4 条顶格 bullet）", () => {
    for (const lang of LANGS) {
  ...
  it("页脚节的 bullet 条数按公式（兄弟文档 + 根 README 回链 + Issues），且末节标题就是译名表同一下标的那一个（R26e'）", () => {
    // ⚠️ **这里曾经是 `.toBe(4)`**，而裁定明写「常量从字面 4 改为公式『兄弟文档数 + 1』…
    // 判官按公式实现（读 DOCS 表现算），不写死 4 也不写死 6」。写死 4 把偏离焊死了：
    // 补上根 README 回链与 GitHub Issues 反而会把本格打红（评审发现 9 / 21）。
    // 逐条目标由「页脚形态 A」那一组统一按公式查，本格只钉条数与末节标题。
    ...
    for (const lang of LANGS) {
      ...
      expect(bullets.length, `docs/${lang}/DEPLOY.md 的页脚节有 ${bullets.length} 条 bullet，公式算出来是 ${want} 条`)
        .toBe(want);
      // R26e'：末节标题 == 译名表同一下标 —— 与 API.md 那一族用的是同一个下标。
...
/* ══════════════════════════════════════════════════════════════════════════
 * 五份 `API.md` 的 13 节骨架**之下**的四条验收
 *
 * `##` 骨架那一层由译名表那组的「已接线的那几格：`##` 序列逐字 `toEqual` DOC_SECTIONS」
 * 守着（API 那一批把五格从「允许对不上」的名册搬了过去）。**本组守的是它下面**：
 *
 * · **端点从 `##` 降为协议族 `##` 之下的 `###`（模板实测：kiro / gemini 两仓的
 *   端点全写成 `### METHOD /path`，不带反引号）；`## 认证` 之下补 `### 方式 N`；
 *   `## 错误响应` 之下补一张状态码**升序**总表 `### 常见错误码`。
 * · **每个端点块的固定五件套** —— 功能说明 → `**请求体**：`+参数表 →
 *   `**请求**：`+```bash → `**响应**：`+```json/```text → 可选 alert。
 *   验收是「`**请求**` 与 `**响应**` 各 ≥10 条/份」（两个参照仓实测 42/28 与 48/18）。
 * · **参数表表头逐字固定，必填列写「是/否」不写 `✅`/`❌`**（那是 README 配置表的写法）。
 *   ⚠️ 这与 README 的配置表刻意**不同**：那一张用 `✅`/`❌`，两处不是同一条规矩。
 * · **`## 请求示例` 之下恰三个 `###`**，cURL 块内两条注释逐条在。
 *
 * ⚠️ **为什么不写死 `###` 的条数**：那个数每加一条端点就过期一次（管理接口今天 24 条，
 * 源码里再注册一条它就变）。本组钉的是**五语言彼此相等 + 不回退下限**，与常见问题那一条同一形态。
 * ══════════════════════════════════════════════════════════════════════════ */
...
/** 逐语言的双粗体标签（五件套那一条）。**必须逐语言给**：`**请求**：` 在 en 上一次都命中不了。 */
...
describe("五份 API.md 的 13 节骨架之下的四条验收", () => {
  it("射程自守：五份都读得到、各 13 个 `##`，`###` 数五份彼此相等且不回退（今天 49）", () => {
    ...
    expect(new Set(h3Counts).size, `五份的 \`###\` 数不一致：${JSON.stringify(h3Counts)} —— `
      ...
      .toBe(1);
    // **不回退下限**（与常见问题那一条同一形态，刻意不写 `toBe(49)`）：这个数每加一条端点就变，
    // 写死等于每次加端点都要回来改一个与判据无关的数。
    expect(h3Counts[0], "`###` 数掉到 40 以下 —— 端点块被整片删掉了？").toBeGreaterThanOrEqual(40);
  });

  it("端点全部降为 `###`：`## METHOD /path` 恒为 0，而 `### METHOD /path` 五份相等且 ≥ 30", () => {
    const bad: string[] = [];
    const counts: number[] = [];
    for (const lang of LANGS) {
      const heads = headingLinesOf(apiSrc(lang));
  ...
  it("`## 认证` 之下恰 4 条 `### 方式 N`，编号 1..4 严格递增（五语言同构）", () => {
    for (const lang of LANGS) {
  ...
  it("`**请求**` 与 `**响应**` 各 ≥10 条/份，且五语言条数彼此相等", () => {
  ...
  it("五件套的次序：每个 `**请求**` 之后紧跟 ```bash，每个 `**响应**` 之后紧跟 ```json / ```text", () => {
    const bad: string[] = [];
    for (const lang of LANGS) {
      const lines = apiSrc(lang).split("\n");
      lines.forEach((l, i) => {
        ...
        for (const row of tableDataRows(lines, headerIdx)) {
          ...
          if (cell !== yes && cell !== no) {
            bad.push(`docs/${lang}/API.md 的参数表里必填列写的是 ${JSON.stringify(cell)}`
              + `——只许是「${yes}」或「${no}」（README 的配置表才用 ✅/❌，这里不用）`);
          }
        }
      });
  ...
  it("五份 API.md 的正文里 `✅` / `❌` 恒为 0（那是 README 配置表的写法）", () => {
  ...
  it("cURL 块内两条注释逐条在（非流式 / 流式），且五语言各恰 1 组", () => {
    const bad: string[] = [];
    for (const lang of LANGS) {
...
/* ══════════════════════════════════════════════════════════════════════════
 * 第 1 轮评审发现 —— `## 管理 API` 那 24 条端点与源码的对应，补判据
 *
...
describe("五份 ADMIN.md 的分层、围栏与排障三段式", () => {
  ...
  /**
   * 页脚（形态 A）每条 bullet 该指向的目标，**按公式现算**：
  ...
  // 排障里「密钥表读不懂」那一条）。
  // 每-key 用量落地时 56 → 58：五份各多两个 `###`（用量那一节里的「按密钥的用量」与
  // 「密钥删掉之后，以及为什么没有『重置用量』」）。**拆成两个不是排版洁癖**：
  // 并成一个的话那一段的字符数越过 R23A 那道棘轮（实测 67 → 69），而那道闸的原话是
  // 「想让它降只能在语义边界上插标题」。
  // **同一轮里「API 密钥」那一节的 `###` 数没变**：那条「故意不做的四件事」改成三件、
  // 标题跟着改，是**改一个既有 `###` 的文字**，不是增删一个。
  ...
  it("`##` 骨架维持现状：五份各恰 15 个 `##`（骨架不动是这一条的全部内容）", () => {
    for (const lang of LANGS) {
      ...
      expect(n, `docs/${lang}/ADMIN.md 的 \`##\` 数从 ${H2_COUNT} 变成了 ${n}`
        + " —— 本组明写骨架不动，改骨架要先回来改这条判据并说明理由").toBe(H2_COUNT);
    }
  });
  ...
  it("五份各 ≥3 段代码围栏，且带语言标注率 100%（裸 ``` 开围栏一处都不许有）", () => {
    const failures: string[] = [];
    for (const lang of LANGS) {
  ...
  it("那段事件 JSON 的 level / event / msg 逐字等于源码里的字面量（期望值现算，不手抄）", () => {
  ...
  it("那段 curl 例子里的请求体字段全在 `PATCH_FIELDS` 里，且路由在 `router.ts` 上真的注册着", () => {
  ...
  it("排障节逐条是 `### {症状}` + `**原因**` + `**解决**`，两个标签各恰 1 次", () => {
    const { failures } = troubleFailures(realAdminSrc);
    expect(failures, failures.join("\n")).toEqual([]);
  });

  it("排障条数五份彼此相等，且不回退（今天各 6 条）", () => {
    ...
    for (const lang of LANGS) {
      ...
      if (h2s[h2s.length - 1] !== heading) {
        out.push(
          ...
          + `而形态 A 要求它就是译名表同一下标的 ${JSON.stringify(heading)}（裁定：五类子文档统一页脚）`,
        );
        continue;
      }
  ...
  it("页脚统一成形态 A：末节 == 译名表同一下标，bullet 条数按公式，每条目标各恰 1 条", () => {
...
/* ══════════════════════════════════════════════════════════════════════════
 * 五份 `REGISTRAR.md` 的 11 节 `##` 骨架**之下**补两级
 *
 *
 ...
 *   + 3 张表**。
 *   🔴 **五份只判「`###`/`####` 数量相等 + 层级序列相等 + 不回退下限」，不钉标题文本**
 *   （与 ADMIN 那一条同一条纪律，§1.9.4：`##` 维持现状的文档钉文本 = 拿现状当模板）。
 *   层级序列那一半由 `R2 heading 层级序列` 守着（本文件上方 R1–R6 那一组），
...
describe("五份 REGISTRAR.md 的两级分层与 Cron 那一节的拆分", () => {
  ...
  /**
   * 今天的实测值，同时是**不回退下限**（只许升不许降，与 ADMIN 那一条同一种形态）。
   *
   * ⚠️ **11 → 12 是页脚统一那条裁定的落地，不是骨架漂移**：那条裁定原文写着
   *「REGISTRAR.md 五份**完全没有页脚**（末节 = `## 排障`）…**裁定：五类子文档统一成形态 A**…
   * ADMIN / REGISTRAR 两条的措辞**修正为**『`##` 不动、补 `###`/`####`、**并统一页脚为形态 A**』」。
   * 上一版的注释引的是**修正前**那一版（「明写骨架不动」），于是同一条裁定只在 ADMIN 上
   * 做了、REGISTRAR 这一半漏了整整五份（评审发现 10 / 20）。
  ...
  it("`##` 骨架：五份各恰 12 个 `##`（原骨架 11 节 + 统一补的页脚节）", () => {
    for (const lang of LANGS) {
      const n = headingsAtLevel(realRegSrc(lang), 2).length;
      expect(n, `docs/${lang}/REGISTRAR.md 的 \`##\` 数从 ${H2_COUNT} 变成了 ${n}`
        + " —— 骨架（11 节）+ 页脚节 = 12，改它要先回来改这条判据并说明理由").toBe(H2_COUNT);
    }
  });

  it("`###`/`####` 两级：五份数量彼此相等，且不回退（今天 15 / 4）", () => {
    ...
    expect(new Set(h4).size, `五份的 \`####\` 数不一致：${JSON.stringify(Object.fromEntries(LANGS.map((l, i) => [l, h4[i]])))}`)
      .toBe(1);
    // 下限是这一格**唯一**能挡住「五份一起缩水」的东西（同 ADMIN 那一格的论证）。
    expect(h3[0], `\`###\` 掉到 ${h3[0]} 了（下限 ${H3_FLOOR}）`).toBeGreaterThanOrEqual(H3_FLOOR);
    expect(h4[0], `\`####\` 掉到 ${h4[0]} 了（下限 ${H4_FLOOR}）`).toBeGreaterThanOrEqual(H4_FLOOR);
  });
  ...
  it("Cron 墙钟那一节：五份各恰 4 个 `####` + ≥3 张表（bullet 洪流真的拆开了）", () => {
    const failures: string[] = [];
    for (const lang of LANGS) {
  ...
  it("🔴 钉死的那几条串仍然住在 Cron 那一节里 —— 挪出这一节，registrar 那一组一格都不会红", () => {
    const failures: string[] = [];
    for (const lang of LANGS) {
      const sec = cronSection(realRegSrc(lang), lang);
...
describe("五份 REGISTRAR.md 的那三条 `> [!IMPORTANT]`", () => {
  ...
  it("该红时红：alert 还在、平级承诺被换成别的话 ⇒ 内容那格红（只数条数是拦不住的）", () => {
    const read = readerWith(
      "ja",
      // ⚠️ 变异串**不带 `> **` 前缀**：两条通道改成二选一之后，那句平级承诺挪到了
      // 那条 alert 的第二行（第一行是「いずれか一方を選びます」），前缀不再是行首。
      (s) => s.replace("2 つのチャネルは完全に対等であり、", "YYDS を推奨します。"),
      "REGISTRAR",
    );
 ...
 *
 * ⚠️ **销账的另一半与本组同批**：五份补了围栏之后，`EMPTY_BY_DESIGN` 里
...
describe("五份 REGISTRAR.md 的代码围栏", () => {
   ...
   *
   * ⇒ 口径与 ADMIN 围栏那格「事件 JSON 的 level / event / msg 逐字等于源码字面量」**同族**：
  ...
  it("两段 ```text 围栏的一行日志逐字等于源码里的 event + msg 字面量（期望值现算，不手抄）", () => {
...
/* ══════════════════════════════════════════════════════════════════════════
 * 五份 `USAGE.md` 从「SDK 接线纸」扩成使用指南
 *
 *
 * · **扩容**：`##` 从 5 个扩到 12 个、`###` 从 0 补到 31 个、代码围栏从
 *   4 段扩到 11 段。素材**全部从已完工的 `API.md` / `DEPLOY.md` 与源码抽**，一条都不许编。
 * · **最小排版项**：三处 `base_url` 的坑从散文提升为 `> [!IMPORTANT]`；
 *   三种 SDK 各补一段流式围栏；页脚换成形态 A（R26e'）。
 *
 * 🔴 **本组只判「`###` 数量相等 + 层级序列相等 + 不回退下限」，一个 `##` 的文本都不钉**
 * ——§1.9.3 把 `USAGE` 明确挡在 `DOC_SECTIONS` 之外（钉文本 = 拿今天的现状当模板，
 * 那是本仓犯过的病）。这条射程登记由译名表那组的
...
describe("五份 USAGE.md 的扩容、base_url 三坑与流式围栏", () => {
  const realUsageSrc: ApiDocReader = realDoc("USAGE");

  /** 今天的实测值，同时是**不回退下限**（与 ADMIN / REGISTRAR 两条同一种形态）。 */
  const H2_COUNT = 12;
  const H3_FLOOR = 31;
  const FENCE_FLOOR = 11;

  /** 页脚（形态 A）每条 bullet 该指向的目标，**按公式现算**（同排障那一条）。 */
  const FOOTER_TARGETS: readonly string[] = [
  ...
  /** **开围栏**的语言标记序列（闭合行不算）。理由见 ADMIN 那一组里同名函数上方的注释。 */
  const openFences = (src: string): string[] => {
    const out: string[] = [];
    let inFence = false;
    for (const line of src.split("\n")) {
  ...
  it("`###` 分层：五份数量彼此相等，且不回退（今天 31，旧形态是 0）", () => {
  ...
  it("五份各 ≥11 段代码围栏，且**开围栏**语言标注率 100%（裸 ``` 开围栏一处都不许有）", () => {
    const failures: string[] = [];
    for (const lang of LANGS) {
  ...
  it("围栏语言构成五份逐份相同：6 段 ```python + 5 段 ```bash（三种 SDK 各两段、裸 HTTP 与图片视频五段）", () => {
    for (const lang of LANGS) {
      const open = openFences(realUsageSrc(lang));
  ...
  it("三处 `base_url` 各恰 1 条 `> [!IMPORTANT]`，且各自点着自己那条路径", () => {
    const failures: string[] = [];
    for (const lang of LANGS) {
  ...
  it("三种 SDK 与裸 HTTP 各自的流式开关逐份都在（形态而不是措辞）", () => {
...
/* ══════════════════════════════════════════════════════════════════════════
 * R13c' 的射程扩展：语种判定从「一张 0 行的表」扩到**全部正文行**
 *
 * ── 为什么必须扩 ────────────────────────────────────────────────────────
...
describe("R13c' 的射程扩展：40 份出货文档的**全部正文行**逐行判语种", () => {
 ...
 * 评审发现 8 / 12 的回填）是**按字面扫的**，换一个新说法就可能一个词都不撞。
 * ⚠️ **这句注释曾经是假的**：写下它的时候那张黑名单**根本不存在**，
 * 于是它在替一个不存在的判据做担保（当时登记的「注释在替判据做承诺」那一族）。现在它存在了，这句话才成立；
 ...
 *
 * **它验不了什么**：文档把这两件事**解释**得对不对；也不管别的源码事实。
 * ══════════════════════════════════════════════════════════════════════════ */
...
describe("R27 的源码锚：口令那两条门槛的数字从 `src/` 现算，不抄文档", () => {
  /**
 ...
 * 收在这里而不是散在各格里：当时记着「照模板走」是这一族缺陷的主路径，
...
/** 剥围栏之后的正文行（带行号）。与本文件源码锚那一组用的那一份口径相同。 */
...
describe("文档真值锚：key 池自愈那三句从 `applyStrike` 现算（评审发现 3）", () => {
...
describe("文档真值锚：流式那两句拿真装配现算（评审发现 1）", () => {
...
describe("文档真值锚：导入 key 的 HTTP 接口真的存在（评审发现 2）", () => {
...
describe("文档真值锚：删 key 的前置条件是**或**不是**且**（评审发现 5）", () => {
...
describe("文档真值锚：两把口令撞了是 503 不是 404（评审发现 6）", () => {
...
describe("文档真值锚：`409` / `429` 的 `reason` 闭集从 `src/http/admin/handlers/` 现算（评审发现 4）", () => {
  /**
...
/* ══════════════════════════════════════════════════════════════════════════
 * **页脚形态 A 的公式化射程**（评审发现 9 / 10 / 20 / 21 的回填）
 *
 * 一条裁定逐字写着：「常量从字面 4 改为**公式**『兄弟文档数 + 1』，agnes 落地为 6 条…
 * 模板那个 4 不是魔数，它的真实规则就是『每个兄弟文档一条 + Issues 一条』。
 * **判官按公式实现（读 DOCS 表现算），不写死 4 也不写死 6。**」
 * 另一条又裁定：「REGISTRAR.md 五份**完全没有页脚**…五类子文档统一成形态 A」。
 *
 ...
 * 三处判官（DEPLOY / ADMIN / USAGE 各一条）把这个偏离**焊死**了：`.toBe(4)` 是等号不是下限，
 * 补上那两条 bullet 反而会把判据打红。
 *
 * ⇒ 本组按公式重写，**一处字面 4 / 字面 6 都不留**：
 * · 射程 = 五类子文档 × 五语言 = **25 份**（从 `DOC_SECTIONS` 的键现算，不手抄名单）；
 * · 末节标题 == `DOC_SECTIONS.API[lang]` 的最后一项（同一族译名）；
 ...
 *
 * **它验不了什么**：bullet 的**描述语**写得贴不贴切，也不管顺序。
 * ══════════════════════════════════════════════════════════════════════════ */

describe("页脚形态 A：25 份非 README 文档，条数与目标按公式现算", () => {
  /**
   * 五类子文档 —— **从 `DOCS` 表现算**（裁定逐字：「判官按公式实现（读 DOCS 表现算）」），
      ...
      for (const doc of FOOTER_DOCS) {
        ...
        if (last[0] !== want) {
          out.push(`${path} 的末节是「${last[0]}」，形态 A 要求它是译名表同一下标的「${want}」`
            + "（裁定：五类子文档统一页脚 —— `REGISTRAR.md` 曾经整整五份都没有页脚节）");
          continue;
        }
        const bullets = lines.slice(last[1] + 1).filter((l) => /^- /.test(l));
  ...
  it("25 份非 README 文档的页脚：末节标题、条数、每条目标逐份成立", () => {
    ...
    expect(failures, `页脚形态 A 不成立：\n${failures.join("\n")}\n`
      + "⚠️ 不许把条数改回字面 4 来达标 —— 裁定明写「不写死 4 也不写死 6」，"
      + "而把根 README 回链与 Issues 删掉是内容损失，不是形态对齐").toEqual([]);
  });

  it("该红时红：把五份 `REGISTRAR.md` 的页脚整节删掉 —— 逐份点名（这正是那条裁定落地前的样子）", () => {
    const read = (p: string) => {
...
/* ══════════════════════════════════════════════════════════════════════════
 * **五链的语言顺序恒定**（R16 补①，评审发现 22 的回填）
 *
 * 规格要求 `> 📖` 指针行与语言切换行的五格顺序恒为
 * 简体中文 → 繁體中文 → English → 日本語 → 한국어，并写着「同一条模板规律，
 * **两个载体**第 1 版只落了一个」。实测两个载体**一个都没落**：
 * `lineLangLabels` 用 `Map` 只查重复 / 缺失 / 多出，**零顺序断言**。
 * · 重排①：把根 README 的 API 指针行五格重排 ⇒ `Tests 668 passed`；
 * · 重排②：把语言切换行重排 ⇒ `Tests 713 passed`。
 *
 * ⚠️ **重排②第一次做的时候「看着红了」，查下去是判据自己的反向控制崩了**：
...
describe("五链的语言顺序恒定：指针行与语言切换行两个载体一起守（R16 补①）", () => {
  ...
  it("该红时红（重排①）：把根 README 的 API 指针行五格重排 —— 必须点名那一行", () => {
    const target = "README.md";
...
/* ══════════════════════════════════════════════════════════════════════════
 * **头部徽章 / logo 引用 / 页脚 `<sub>` 的真源锚**（评审发现 14 / 23 的回填）
 *
...
describe("头部徽章 / logo 引用 / 页脚 `<sub>`：六份自等式 + 从 `package.json` 现算的锚", () => {
  ...
  it("该红时红（评审举的那条）：把 `docs/ko/README.md` 的 Hono 徽章换成 axum —— 自等式与真源锚同时红", () => {
  ...
  it("该红时红（评审举的那条）：去掉 `docs/ko/README.md` 的 `width`/`height` —— 尺寸那格红", () => {
    const at = join("docs", "ko", "README.md");
...
/* ══════════════════════════════════════════════════════════════════════════
 * **六份 README 那张 12 行配置表也要有真源锚**（评审发现 15 的回填）
 *
...
describe("六份 README 的配置表：名单 ⊆ `.env.example`，默认值从 `DEFAULTS` 现算", () => {
  const SIX: readonly string[] = ["README.md", ...LANGS.map((l) => join("docs", l, "README.md"))];
