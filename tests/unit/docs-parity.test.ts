 ...
 * 三种情形都实跑过（红/绿见 task-29-report.md 的变异表那一格）。
...
/**
 * ⑧ 五份 README 的 `## 📄` 节末段逐字节登记（第 1 轮评审回填，ADJ §63）。
 *
 * ── 它补的是哪个洞 ────────────────────────────────────────────────────────
 ...
 *
 * ⚠️ **显示宽度与排版基线指标 13/13b 的「字符数」是两把尺**（ADJ ㊾）：那两条量的是
 ...
 * 与本文件「不带切换行」/「位置不对的 `---`」/ R25f 三组同一份 `non25Pairs()`。**README 不设上限**（ADJ ㊻）：
 ...
 * ADJ ㉑ 的裁定推翻了，而且删掉时一格都不会红。偏离名册（阶段 6 的落点：一份新文档
...
describe("`### 配额账` 的折叠与分层（`<details>` 是射程铁律的具名例外）", () => {
  ...
  it("偏离名册的具名例外：25 份非 README 文档里的 `<details>` 恰好就是名册那 5 处", () => {
    ...
    expect(
      ...
      + "· 少掉 ⇒ 配额账那处**具名例外**被人顺手删了，而 ADJ ㉑「不拆 QUOTA.md」的前提"
...
describe("五份 REGISTRAR.md 的两级分层与 Cron 那一节的拆分", () => {
  ...
  it("`##` 骨架：五份各恰 12 个 `##`（原骨架 11 节 + ADJ ㊷ 统一补的页脚节）", () => {
    for (const lang of LANGS) {
...
describe("五份 USAGE.md 的扩容、base_url 三坑与流式围栏", () => {
  ...
  /** 页脚（形态 A）每条 bullet 该指向的目标，**按公式现算**（ADJ ㊶，同排障那一条）。 */
  const FOOTER_TARGETS: readonly string[] = [
...
describe("页脚形态 A：25 份非 README 文档，条数与目标按公式现算（ADJ ㊶ / ㊷）", () => {
  /**
