 ...
 *    为了有用，几乎必然把变量名写进去（今天 6 处全都写了，一处例外都没有）。
 *
...
describe("硬约束：运维配错形状的 throw 一律是 ConfigRefusal", () => {
  ...
  it.each(OPS_THROW_BLIND_SPOTS)("已知边界确实如此（边界是断言，不是散文）：$why", ({ probe }) => {
    // 四条**不是同一种**：前两条扫得到/扫不到但判不成 ops；第三条被静静截断、整处漏掉；
    // **第四条**才是让扫描器当场失步抛的那个（正则里的 `(` 永远收不了口）。
    let ops: boolean;
    try {
      const sites = throwSites(probe);
...
describe("抠注释真源的三个出口", () => {
   ...
   *
   * 复评实测：`admin-ui/index.html` 少一个 `-->` ⇒ i18n 门禁的引用数
...
describe("抠注释真源的扫描器边界", () => {
  ...
  it.each([
    ...
    // ↓ 这六条是第四轮复评实测到的**假红**（撇号与模板串那一族），本轮在真源里修掉了：
    ...
    ["复评实测：往真文件里追加的那一行完全合法的 TS", "export const zzA = 1; /* don't */ export const zzB = 2; // it's fine"],
   ...
   *
   * 第四轮复评实测，**复用档名那次变异**：在真源里新加一个**真的走得到**的抛出点、档名复用
   * `unknownPunct` ⇒ 运行期收到的档集合一个字没变 ⇒ **103/103 全绿**。
   * （同一轮另一次变异 —— 新档名 + 不加探针 —— 上面那格红且指对地方，那一半是好的。）
  ...
  it("真源里每个 fail() 调用点的档名互不重复，并集恰好是 FAIL_KINDS", () => {
    ...
    expect(
      ...
      + "运行期那一格只看档名集合，它对「同档名的第二个站点」天然失明（复评时复用档名那次变异实测全绿逃逸）。"
  ...
  it("反向控制：档名守卫认得出复用与变量传，而注释里的散文不算数", () => {
    ...
    // ① 复用已有档名的第二个可达抛出点（落点与档名跟复评时复用档名那次变异逐字相同）
    const dup = real.replace(
      "if (REGEX_AFTER_PUNCT.has(p.ch)) return true;",
      'if (p.ch === "#") fail(src, pos, "unknownPunct", "复用档名那次变异的落点");\n'
      + "    if (REGEX_AFTER_PUNCT.has(p.ch)) return true;",
    );
   ...
   * 第四轮复评在真文件上量过代价：同一行只多一个撇号 ⇒ 它一个字不吵、
   ...
   */
  it.each([
    ["本行有反引号 ⇒ 直接闭嘴（复评实测：同一行只多一个撇号）", "const of = 4; const T = `it's`; const half = of / 2; const P = \"/\"; const U = \"https://x\"; console.log(1);", 21],
...
describe("分段选择器的每个创建点都带 aria-pressed", () => {
   ...
   *
   * ⚠️ **登记：这是本仓第二个手写的字符串感知扫描器，比真源弱**（复评发现）。
   ...
   *
   * 这一格补的是 `sec-` 前缀那份**隐式豁免名册**（复评发现）：上面几格全都
 ...
 *
 * ⚠️⚠️ **这里原先写着「它假设这份 CSS 是平铺的（没有 `@media` 之类的嵌套块）。
 * 下面『射程自守』那一格盯着这个前提：哪天真加了嵌套块，切分会把外层前缀粘进选择器，
 * 那一格会红」—— 后面那两句都是假的，面板加上响应式断点那一轮当场证伪。**
 * ① 本文件里**没有**那样一格（在本文件里 `grep -n 射程自守` 只命中这一段自己）；
 * ② 加完 `@media` 之后全族一格都没红。
 * ③ 更要紧的是「粘前缀」这件事本身就不会发生：上面那条正则的 `[^{}]+` 吃不下内层
 *    规则前面那个 `{`，引擎会把起点一路推到它后面，切出来的仍旧是干净的 `.side` /
 *    `.main`（实测：三份 CSS 切出的规则里，选择器含 `@media` 的 **0 条**）。
 *
 * ⇒ **今天真正的边界是另一条，写清楚**：`@media` 里的规则会被**当成平铺规则一起收**，
 * 外层那句 `@media (…)` 则整段落空。所以给上面两个函数查得到的那几个选择器
 *（`.btn-toggle` / `.btn-toggle.active` / `.cfg-col` / `.cfg-grid` / `.cfg-label`）
 * 在断点里再写一条同名规则时，两条的声明会**拼在一起**参与判定，而"哪一条在那个
 * 视口下真的生效"这一层它分辨不了。面板那三档断点因此刻意一个都不碰这几个类
 *（理由写在 `admin-ui/css/sections.css` 末尾那一段），不是碰巧没碰。
 */
...
describe("设置页的多列：写死的 px 下限不许在窄容器里顶穿", () => {
   ...
   * 真机量到的就是这个：1920 档「上游与冷却」4 列、注册机旋钮 8 列（旋钮那张一路
...
/**
 * ── 轮询状态灯：**非文字**状态指示的对比度下限（WCAG 1.4.11，复评发现）─
 *
 ...
 * ⇒ 是 token 的事，不是文案的事。
 * ⚠️ **具体条数刻意不写**（复评发现）：它随当时渲染了几行模型目录漂
 ...
 *   逐份对账的判据，本组不去重复它。
 *   ⚠️ 文档对账那份测试**本身在射程里**（它在 `tests/` 下），
 *   只是三种拼法在它里面一条都不命中（实测：本轮逐条跑过三条正则，零处）。
 *   它的散文里留着几句带编号的**史实**叙述（说的是搬家之前、危险区刚落地那天的样子），
 *   那种句子本来就该与今天的顺序不同，刻意不去动它们 —— 史实不该跟着现状漂。
 *   ⚠️⚠️ **别把「一条都不命中」读成「那份文件里没有错编号」**：这一格只认那三种拼法，
 *   换个说法写出来的错编号照样躲得掉。**这不是假设**：复评在那份文件里逮到过一句
 *  「现在模拟的是第 N 张卡」——现在时、说的是当下行为、而且那个数当时是错的，
 *   三条正则一条都不沾。上一版这段排除理由把它一并说成「史实」，那句话本身就是一次
 *   过度声称。⇒ 那处已经改成不落编号的说法（去掉编号，再搬一次卡它就不会再漂），
 *   而这一侧的盲区照旧写在这里，别让下一个人以为它被盖住了。
 */
