 ...
 *
 * ── 主消费者已落地一半，这份判据还守什么 ────────────────────────────────
 * 这张表的主消费者是「章节骨架」判据（根 16 节 / 语言版 12 节的 `toEqual`）。
 * 根那一半今天就在跑——`tests/unit/docs-parity.test.ts` 的
 * 「⑥ R11（根）：根 README 的 16 节标题逐字命中……」拿本表 zh-CN 那一列当右操作数；
 * 语言版那一半要等阶段 5B，`docs/{lang}/README.md` 今天还是旧骨架。
 * 本文件因此守的是根判据够不着的那些：zh-CN 之外的四列、16/12 的划分、新定格子的登记。
 *
 * 于是这里做两件事，缺一不可：
